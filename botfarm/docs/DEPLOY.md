...
Код рабочий и запускается одной командой. Но «поставил на сервер — всё
продаётся» верно не полностью. Перед запуском **каждого** бота нужно:

| Что | Обязательно? | Сколько времени |
|-----|--------------|-----------------|
| Токен от @BotFather | да | 2 минуты |
| Ваш Telegram ID в `ADMIN_IDS` | да | 1 минута |
| Хотя бы один способ оплаты | да, иначе продавать нечем | см. ниже |
| Товар на складе (для магазинов аккаунтов) | да — 549 ботов из 1000 | зависит от вас |
| Контакт поддержки, ссылки выдачи, реквизиты | да | 5 минут |
| Домен с HTTPS | нет — без него работает опрос платежей | 20 минут |

Про оплату честно:

- **@CryptoBot** — 5 минут, юрлицо не нужно, работает везде. Быстрее всего
  начать с него.
- **Telegram Stars** — работает сразу, без регистраций. Уже включены там,
  где сумма помещается в лимит.
- **YooKassa** (карты и СБП) — нужны ИП или ООО и касса по 54-ФЗ. Физлицо
  не подключат.
- **Stripe** — нужна компания в поддерживаемой стране.
- **Перевод по реквизитам** — работает всегда, подтверждается вручную кнопкой
  в админке.

То есть: минимальный рабочий магазин — это токен, `ADMIN_IDS`, CryptoBot и
загруженный склад. Это реально 15–20 минут на бота.

## Сколько ботов влезет на один VPS

Важное ограничение: **один процесс = один бот**. Запустить все 1000 на одной
машине не получится и не нужно — держите запущенными те, что продаются.
...
| 8 vCPU / 16 GB | 16 GB | 250–290 | 320 |

Считайте по активным ботам, а не по всем 1000 — лежащие в архиве ничего не едят.
