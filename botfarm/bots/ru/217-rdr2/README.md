# Red Dead Redemption 2 Аккаунты

> Red Dead Redemption 2: моментальная выдача, гарантия и замена

**ID:** `ru-217-rdr2` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Red Dead Redemption 2 Аккаунты — бот по теме «аккаунты RDR2». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9216
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/217-rdr2/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 217-rdr2.service /etc/systemd/system/
sudo systemctl enable --now 217-rdr2
```
