...
> Портфолио, эскизы и запись к мастеру

**ID:** `ru-080-tattoo-book` · **Регион:** RU · **Язык:** ru ·
**Модель:** leadgen · **Тариф цен:** standard

Тату Запись — бот по теме «татуировки». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9079
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/080-tattoo-book/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 080-tattoo-book.service /etc/systemd/system/
sudo systemctl enable --now 080-tattoo-book
```
