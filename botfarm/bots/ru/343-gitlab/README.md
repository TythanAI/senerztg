...
> GitLab по цене в разы ниже подписки

**ID:** `ru-343-gitlab` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

GitLab Доступ — бот по теме «аккаунты GitLab». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9342
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/343-gitlab/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 343-gitlab.service /etc/systemd/system/
sudo systemctl enable --now 343-gitlab
```
