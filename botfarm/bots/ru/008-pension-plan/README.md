...
> Собственный пенсионный капитал

**ID:** `ru-008-pension-plan` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** standard

Пенсия Сам — бот по теме «пенсионные накопления». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9007
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/008-pension-plan/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 008-pension-plan.service /etc/systemd/system/
sudo systemctl enable --now 008-pension-plan
```
