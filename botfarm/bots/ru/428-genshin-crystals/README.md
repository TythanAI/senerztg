...
> Genshin: пополнение по выгодному курсу за 15 минут

**ID:** `ru-428-genshin-crystals` · **Регион:** RU · **Язык:** ru ·
**Модель:** topup · **Тариф цен:** budget

Genshin — бот по теме «кристаллы Genshin». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9427
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/428-genshin-crystals/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 428-genshin-crystals.service /etc/systemd/system/
sudo systemctl enable --now 428-genshin-crystals
```
