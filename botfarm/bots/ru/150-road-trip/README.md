...
> Маршруты по России с точками

**ID:** `ru-150-road-trip` · **Регион:** RU · **Язык:** ru ·
**Модель:** shop · **Тариф цен:** budget

Автопутешествия — бот по теме «автопутешествия». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9149
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/150-road-trip/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 150-road-trip.service /etc/systemd/system/
sudo systemctl enable --now 150-road-trip
```
