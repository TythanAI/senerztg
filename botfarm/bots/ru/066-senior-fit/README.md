...
> Движение и здоровье после пятидесяти

**ID:** `ru-066-senior-fit` · **Регион:** RU · **Язык:** ru ·
**Модель:** club · **Тариф цен:** budget

Актив 50+ — бот по теме «фитнес 50+». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9065
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/066-senior-fit/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 066-senior-fit.service /etc/systemd/system/
sudo systemctl enable --now 066-senior-fit
```
