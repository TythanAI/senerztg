...
> Wink дешевле официальной подписки, выдача сразу

**ID:** `ru-259-wink` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Wink Подписки — бот по теме «подписки Wink». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/259-wink/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 259-wink.service /etc/systemd/system/
sudo systemctl enable --now 259-wink
```
