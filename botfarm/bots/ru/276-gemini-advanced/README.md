# Gemini Advanced Доступ

> Gemini Advanced без карты и VPN — доступ за минуту

**ID:** `ru-276-gemini-advanced` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Gemini Advanced Доступ — бот по теме «аккаунты Gemini». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9275
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/276-gemini-advanced/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 276-gemini-advanced.service /etc/systemd/system/
sudo systemctl enable --now 276-gemini-advanced
```
