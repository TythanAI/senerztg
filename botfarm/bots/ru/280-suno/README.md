...
> Suno без карты и VPN — доступ за минуту

**ID:** `ru-280-suno` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Suno Доступ — бот по теме «аккаунты Suno». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9279
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/280-suno/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 280-suno.service /etc/systemd/system/
sudo systemctl enable --now 280-suno
```
