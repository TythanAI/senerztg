...
> Brawl Буст: результат под ключ, оплата после согласования

**ID:** `ru-459-brawl-boost` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** standard

Brawl Буст — бот по теме «прокачка Brawl Stars». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9458
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/459-brawl-boost/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 459-brawl-boost.service /etc/systemd/system/
sudo systemctl enable --now 459-brawl-boost
```
