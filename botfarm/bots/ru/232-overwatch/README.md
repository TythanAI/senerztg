# Overwatch 2 Аккаунты

> Overwatch 2: моментальная выдача, гарантия и замена

**ID:** `ru-232-overwatch` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Overwatch 2 Аккаунты — бот по теме «аккаунты Overwatch 2». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9231
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/232-overwatch/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 232-overwatch.service /etc/systemd/system/
sudo systemctl enable --now 232-overwatch
```
