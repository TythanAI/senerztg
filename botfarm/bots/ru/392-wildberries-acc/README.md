# Wildberries Аккаунты

> Wildberries: готовые аккаунты для работы

**ID:** `ru-392-wildberries-acc` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Wildberries Аккаунты — бот по теме «кабинеты Wildberries». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9391
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/392-wildberries-acc/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 392-wildberries-acc.service /etc/systemd/system/
sudo systemctl enable --now 392-wildberries-acc
```
