...
> Tuta: чистые аккаунты для работы и регистраций

**ID:** `ru-340-tutanota` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Tuta Аккаунты — бот по теме «аккаунты Tuta». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9339
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/340-tutanota/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 340-tutanota.service /etc/systemd/system/
sudo systemctl enable --now 340-tutanota
```
