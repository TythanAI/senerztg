...
> Audible дешевле официальной подписки, выдача сразу

**ID:** `ru-270-audible` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Audible Подписки — бот по теме «аккаунты Audible». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9269
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/270-audible/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 270-audible.service /etc/systemd/system/
sudo systemctl enable --now 270-audible
```
