...
> Game Pass: ключи и коды с моментальной выдачей

**ID:** `ru-423-gamepass` · **Регион:** RU · **Язык:** ru ·
**Модель:** keys · **Тариф цен:** standard

Game Pass — бот по теме «подписки Game Pass». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9422
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/423-gamepass/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 423-gamepass.service /etc/systemd/system/
sudo systemctl enable --now 423-gamepass
```
