...
> Вакансии и гранты web3

**ID:** `ru-031-web3-jobs` · **Регион:** RU · **Язык:** ru ·
**Модель:** club · **Тариф цен:** standard

Web3 Работа — бот по теме «работа в web3». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9030
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/031-web3-jobs/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 031-web3-jobs.service /etc/systemd/system/
sudo systemctl enable --now 031-web3-jobs
```
