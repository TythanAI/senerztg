...
> Nintendo: моментальная выдача, гарантия и замена

**ID:** `ru-243-nintendo` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Nintendo Аккаунты — бот по теме «аккаунты Nintendo». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/243-nintendo/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 243-nintendo.service /etc/systemd/system/
sudo systemctl enable --now 243-nintendo
```
