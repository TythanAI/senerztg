...
> Premier дешевле официальной подписки, выдача сразу

**ID:** `ru-260-premier` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Premier Подписки — бот по теме «подписки Premier». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9259
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/260-premier/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 260-premier.service /etc/systemd/system/
sudo systemctl enable --now 260-premier
```
