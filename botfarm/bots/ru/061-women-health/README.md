...
> Цикл, гормоны, самочувствие

**ID:** `ru-061-women-health` · **Регион:** RU · **Язык:** ru ·
**Модель:** club · **Тариф цен:** standard

Женское Здоровье — бот по теме «женское здоровье». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9060
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/061-women-health/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 061-women-health.service /etc/systemd/system/
sudo systemctl enable --now 061-women-health
```
