...
> Продажи на Авито системно

**ID:** `ru-085-avito-flow` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** standard

Авито Поток — бот по теме «продажи на Авито». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9084
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/085-avito-flow/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 085-avito-flow.service /etc/systemd/system/
sudo systemctl enable --now 085-avito-flow
```
