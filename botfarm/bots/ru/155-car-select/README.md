...
> Проверка и выбор машины

**ID:** `ru-155-car-select` · **Регион:** RU · **Язык:** ru ·
**Модель:** consult · **Тариф цен:** premium

Подбор Авто — бот по теме «подбор автомобиля». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9154
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/155-car-select/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 155-car-select.service /etc/systemd/system/
sudo systemctl enable --now 155-car-select
```
