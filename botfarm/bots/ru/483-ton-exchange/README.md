...
> TON Обмен: быстро, по курсу, без лишних вопросов

**ID:** `ru-483-ton-exchange` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** premium

TON Обмен — бот по теме «обмен TON». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9482
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/483-ton-exchange/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 483-ton-exchange.service /etc/systemd/system/
sudo systemctl enable --now 483-ton-exchange
```
