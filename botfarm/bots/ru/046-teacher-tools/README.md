...
> Генератор заданий и проверок

**ID:** `ru-046-teacher-tools` · **Регион:** RU · **Язык:** ru ·
**Модель:** saas · **Тариф цен:** standard

Учитель Инструменты — бот по теме «инструменты для учителей». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9045
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/046-teacher-tools/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 046-teacher-tools.service /etc/systemd/system/
sudo systemctl enable --now 046-teacher-tools
```
