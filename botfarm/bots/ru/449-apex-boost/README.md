...
> Apex Буст: результат под ключ, оплата после согласования

**ID:** `ru-449-apex-boost` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** standard

Apex Буст — бот по теме «буст ранга Apex». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9448
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/449-apex-boost/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 449-apex-boost.service /etc/systemd/system/
sudo systemctl enable --now 449-apex-boost
```
