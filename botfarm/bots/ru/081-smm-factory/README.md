...
> Ведение соцсетей под ключ

**ID:** `ru-081-smm-factory` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** premium

SMM Фабрика — бот по теме «SMM». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9080
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/081-smm-factory/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 081-smm-factory.service /etc/systemd/system/
sudo systemctl enable --now 081-smm-factory
```
