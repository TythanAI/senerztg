...
> JetBrains по цене в разы ниже подписки

**ID:** `ru-344-jetbrains` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

JetBrains Доступ — бот по теме «лицензии JetBrains». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9343
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/344-jetbrains/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 344-jetbrains.service /etc/systemd/system/
sudo systemctl enable --now 344-jetbrains
```
