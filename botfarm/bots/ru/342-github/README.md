...
> GitHub по цене в разы ниже подписки

**ID:** `ru-342-github` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

GitHub Доступ — бот по теме «аккаунты GitHub». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9341
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/342-github/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 342-github.service /etc/systemd/system/
sudo systemctl enable --now 342-github
```
