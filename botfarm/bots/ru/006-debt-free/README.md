...
> Пошаговый выход из кредитной ямы

**ID:** `ru-006-debt-free` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** budget

Долги Стоп — бот по теме «закрытие долгов». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9005
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/006-debt-free/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 006-debt-free.service /etc/systemd/system/
sudo systemctl enable --now 006-debt-free
```
