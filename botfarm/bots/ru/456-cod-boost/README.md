...
> CoD Буст: результат под ключ, оплата после согласования

**ID:** `ru-456-cod-boost` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** standard

CoD Буст — бот по теме «прокачка в Call of Duty». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9455
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/456-cod-boost/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 456-cod-boost.service /etc/systemd/system/
sudo systemctl enable --now 456-cod-boost
```
