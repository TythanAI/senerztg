...
> Ideogram без карты и VPN — доступ за минуту

**ID:** `ru-296-ideogram` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Ideogram Доступ — бот по теме «аккаунты Ideogram». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9295
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/296-ideogram/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 296-ideogram.service /etc/systemd/system/
sudo systemctl enable --now 296-ideogram
```
