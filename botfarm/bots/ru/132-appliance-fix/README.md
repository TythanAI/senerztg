...
> Стиральные, холодильники, плиты

**ID:** `ru-132-appliance-fix` · **Регион:** RU · **Язык:** ru ·
**Модель:** leadgen · **Тариф цен:** standard

Ремонт Техники — бот по теме «ремонт бытовой техники». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9131
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/132-appliance-fix/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 132-appliance-fix.service /etc/systemd/system/
sudo systemctl enable --now 132-appliance-fix
```
