...
> HTX: верифицированные аккаунты под задачи

**ID:** `ru-383-htx-acc` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** premium

HTX Аккаунты — бот по теме «аккаунты HTX». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9382
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/383-htx-acc/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 383-htx-acc.service /etc/systemd/system/
sudo systemctl enable --now 383-htx-acc
```
