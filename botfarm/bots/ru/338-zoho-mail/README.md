...
> Zoho Mail: чистые аккаунты для работы и регистраций

**ID:** `ru-338-zoho-mail` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Zoho Mail Аккаунты — бот по теме «аккаунты Zoho». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9337
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/338-zoho-mail/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 338-zoho-mail.service /etc/systemd/system/
sudo systemctl enable --now 338-zoho-mail
```
