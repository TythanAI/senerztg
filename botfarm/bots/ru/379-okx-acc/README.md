...
> OKX: верифицированные аккаунты под задачи

**ID:** `ru-379-okx-acc` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** premium

OKX Аккаунты — бот по теме «аккаунты OKX». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9378
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/379-okx-acc/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 379-okx-acc.service /etc/systemd/system/
sudo systemctl enable --now 379-okx-acc
```
