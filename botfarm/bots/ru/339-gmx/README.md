...
> GMX: чистые аккаунты для работы и регистраций

**ID:** `ru-339-gmx` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

GMX Аккаунты — бот по теме «аккаунты GMX». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9338
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/339-gmx/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 339-gmx.service /etc/systemd/system/
sudo systemctl enable --now 339-gmx
```
