...
> Destiny 2: моментальная выдача, гарантия и замена

**ID:** `ru-235-destiny2` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Destiny 2 Аккаунты — бот по теме «аккаунты Destiny 2». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/235-destiny2/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 235-destiny2.service /etc/systemd/system/
sudo systemctl enable --now 235-destiny2
```
