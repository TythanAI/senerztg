...
> Мониторинг Цен: уведомления в этот чат в реальном времени

**ID:** `ru-460-price-monitor` · **Регион:** RU · **Язык:** ru ·
**Модель:** monitor · **Тариф цен:** standard

Мониторинг Цен — бот по теме «отслеживание цен». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9459
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/460-price-monitor/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 460-price-monitor.service /etc/systemd/system/
sudo systemctl enable --now 460-price-monitor
```
