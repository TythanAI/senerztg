...
> Копирайтинг: делаем под ключ, сроки фиксируем

**ID:** `ru-496-copywriting-service` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** standard

Копирайтинг — бот по теме «написание текстов». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9495
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/496-copywriting-service/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 496-copywriting-service.service /etc/systemd/system/
sudo systemctl enable --now 496-copywriting-service
```
