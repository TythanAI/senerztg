...
> Видеостудия: делаем под ключ, сроки фиксируем

**ID:** `ru-494-video-edit-service` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** standard

Видеостудия — бот по теме «монтаж видео под ключ». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9493
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/494-video-edit-service/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 494-video-edit-service.service /etc/systemd/system/
sudo systemctl enable --now 494-video-edit-service
```
