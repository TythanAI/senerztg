...
> Календарь публикаций и идеи

**ID:** `ru-090-content-plan` · **Регион:** RU · **Язык:** ru ·
**Модель:** saas · **Тариф цен:** standard

Контент План — бот по теме «контент-маркетинг». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9089
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/090-content-plan/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 090-content-plan.service /etc/systemd/system/
sudo systemctl enable --now 090-content-plan
```
