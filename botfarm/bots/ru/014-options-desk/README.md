...
> Опционные стратегии на практике

**ID:** `ru-014-options-desk` · **Регион:** RU · **Язык:** ru ·
**Модель:** club · **Тариф цен:** elite

Опционный Стол — бот по теме «опционы». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9013

## Каталог

| SKU | Название | Цена | Тип |
|-----|----------|------|-----|
| `month` | Месяц доступа | 14 890 ₽ | subscription · 30 дн. |
| `quarter` | Три месяца | 38 730 ₽ | subscription · 90 дн. |
| `year` | Год доступа | 119 190 ₽ | subscription · 365 дн. |
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/014-options-desk/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 014-options-desk.service /etc/systemd/system/
sudo systemctl enable --now 014-options-desk
```
