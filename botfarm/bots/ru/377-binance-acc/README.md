...
> Binance: верифицированные аккаунты под задачи

**ID:** `ru-377-binance-acc` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** premium

Binance Аккаунты — бот по теме «аккаунты Binance». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9376
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/377-binance-acc/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 377-binance-acc.service /etc/systemd/system/
sudo systemctl enable --now 377-binance-acc
```
