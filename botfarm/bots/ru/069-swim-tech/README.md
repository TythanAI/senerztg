...
**ID:** `ru-069-swim-tech` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** standard

Техника Плавания — бот по теме «плавание». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9068
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/069-swim-tech/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 069-swim-tech.service /etc/systemd/system/
sudo systemctl enable --now 069-swim-tech
```
