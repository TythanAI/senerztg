...
**ID:** `ru-074-nails-pro` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** premium

Ногтевой Сервис — бот по теме «маникюр». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9073
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/074-nails-pro/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 074-nails-pro.service /etc/systemd/system/
sudo systemctl enable --now 074-nails-pro
```
