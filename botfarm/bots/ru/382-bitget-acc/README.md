...
> Bitget: верифицированные аккаунты под задачи

**ID:** `ru-382-bitget-acc` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** premium

Bitget Аккаунты — бот по теме «аккаунты Bitget». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9381
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/382-bitget-acc/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 382-bitget-acc.service /etc/systemd/system/
sudo systemctl enable --now 382-bitget-acc
```
