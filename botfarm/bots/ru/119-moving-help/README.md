...
> Грузчики и транспорт по городу

**ID:** `ru-119-moving-help` · **Регион:** RU · **Язык:** ru ·
**Модель:** leadgen · **Тариф цен:** standard

Переезд Просто — бот по теме «переезды». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9118
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/119-moving-help/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 119-moving-help.service /etc/systemd/system/
sudo systemctl enable --now 119-moving-help
```
