...
> Telegram Stars: пополнение по выгодному курсу за 15 минут

**ID:** `ru-442-telegram-stars-topup` · **Регион:** RU · **Язык:** ru ·
**Модель:** topup · **Тариф цен:** budget

Telegram Stars — бот по теме «пополнение Telegram Stars». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9441
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/442-telegram-stars-topup/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 442-telegram-stars-topup.service /etc/systemd/system/
sudo systemctl enable --now 442-telegram-stars-topup
```
