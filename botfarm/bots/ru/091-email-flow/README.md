...
**ID:** `ru-091-email-flow` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** standard

Email Воронки — бот по теме «email-маркетинг». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9090
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/091-email-flow/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 091-email-flow.service /etc/systemd/system/
sudo systemctl enable --now 091-email-flow
```
