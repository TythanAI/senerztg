...
> Поступление в вузы Европы

**ID:** `ru-044-uni-abroad` · **Регион:** RU · **Язык:** ru ·
**Модель:** consult · **Тариф цен:** elite

Универ за Рубежом — бот по теме «образование за рубежом». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9043
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/044-uni-abroad/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 044-uni-abroad.service /etc/systemd/system/
sudo systemctl enable --now 044-uni-abroad
```
