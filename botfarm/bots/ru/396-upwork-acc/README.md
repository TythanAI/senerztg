...
> Upwork: готовые аккаунты для работы

**ID:** `ru-396-upwork-acc` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Upwork Аккаунты — бот по теме «аккаунты Upwork». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9395
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/396-upwork-acc/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 396-upwork-acc.service /etc/systemd/system/
sudo systemctl enable --now 396-upwork-acc
```
