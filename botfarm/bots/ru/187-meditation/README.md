...
**ID:** `ru-187-meditation` · **Регион:** RU · **Язык:** ru ·
**Модель:** club · **Тариф цен:** budget

Медитация Каждый День — бот по теме «медитация». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9186
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/187-meditation/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 187-meditation.service /etc/systemd/system/
sudo systemctl enable --now 187-meditation
```
