...
> Эмуляторы, ромы, настройка

**ID:** `ru-200-retro-games` · **Регион:** RU · **Язык:** ru ·
**Модель:** shop · **Тариф цен:** budget

Ретро Игры — бот по теме «ретро-гейминг». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9199
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/200-retro-games/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 200-retro-games.service /etc/systemd/system/
sudo systemctl enable --now 200-retro-games
```
