...
> Amazon Gift Card: ключи и коды с моментальной выдачей

**ID:** `ru-411-amazon-card` · **Регион:** RU · **Язык:** ru ·
**Модель:** keys · **Тариф цен:** standard

Amazon Gift Card — бот по теме «карты Amazon». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9410
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/411-amazon-card/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 411-amazon-card.service /etc/systemd/system/
sudo systemctl enable --now 411-amazon-card
```
