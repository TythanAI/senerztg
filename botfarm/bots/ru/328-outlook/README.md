...
> Outlook: чистые аккаунты для работы и регистраций

**ID:** `ru-328-outlook` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Outlook Аккаунты — бот по теме «аккаунты Outlook». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9327
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/328-outlook/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 328-outlook.service /etc/systemd/system/
sudo systemctl enable --now 328-outlook
```
