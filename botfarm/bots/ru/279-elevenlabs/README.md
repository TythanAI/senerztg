...
> ElevenLabs без карты и VPN — доступ за минуту

**ID:** `ru-279-elevenlabs` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

ElevenLabs Доступ — бот по теме «аккаунты ElevenLabs». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/279-elevenlabs/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 279-elevenlabs.service /etc/systemd/system/
sudo systemctl enable --now 279-elevenlabs
```
