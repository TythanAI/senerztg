...
> Freepik по цене в разы ниже подписки

**ID:** `ru-356-freepik` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Freepik Доступ — бот по теме «аккаунты Freepik». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9355
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/356-freepik/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 356-freepik.service /etc/systemd/system/
sudo systemctl enable --now 356-freepik
```
