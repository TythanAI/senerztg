...
> Макияж на каждый день и вечер

**ID:** `ru-072-makeup-school` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** standard

Школа Макияжа — бот по теме «макияж». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9071
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/072-makeup-school/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 072-makeup-school.service /etc/systemd/system/
sudo systemctl enable --now 072-makeup-school
```
