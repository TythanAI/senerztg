...
> Связки и работа с рисками P2P

**ID:** `ru-025-p2p-arbitrage` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** premium

P2P Арбитраж — бот по теме «P2P-арбитраж». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9024
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/025-p2p-arbitrage/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 025-p2p-arbitrage.service /etc/systemd/system/
sudo systemctl enable --now 025-p2p-arbitrage
```
