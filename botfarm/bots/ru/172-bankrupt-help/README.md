...
**ID:** `ru-172-bankrupt-help` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** elite

Банкротство Физлиц — бот по теме «банкротство». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9171
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/172-bankrupt-help/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 172-bankrupt-help.service /etc/systemd/system/
sudo systemctl enable --now 172-bankrupt-help
```
