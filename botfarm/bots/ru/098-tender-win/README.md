...
> Выигрываем госзакупки

**ID:** `ru-098-tender-win` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** premium

Тендеры 44-ФЗ — бот по теме «тендеры». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9097
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/098-tender-win/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 098-tender-win.service /etc/systemd/system/
sudo systemctl enable --now 098-tender-win
```
