...
> Discord Nitro: ключи и коды с моментальной выдачей

**ID:** `ru-418-discord-nitro` · **Регион:** RU · **Язык:** ru ·
**Модель:** keys · **Тариф цен:** standard

Discord Nitro — бот по теме «Discord Nitro». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9417
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/418-discord-nitro/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 418-discord-nitro.service /etc/systemd/system/
sudo systemctl enable --now 418-discord-nitro
```
