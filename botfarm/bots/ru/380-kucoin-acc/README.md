...
> KuCoin: верифицированные аккаунты под задачи

**ID:** `ru-380-kucoin-acc` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** premium

KuCoin Аккаунты — бот по теме «аккаунты KuCoin». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9379
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/380-kucoin-acc/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 380-kucoin-acc.service /etc/systemd/system/
sudo systemctl enable --now 380-kucoin-acc
```
