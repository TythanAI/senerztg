...
> Paramount+ дешевле официальной подписки, выдача сразу

**ID:** `ru-266-paramount` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Paramount+ Подписки — бот по теме «аккаунты Paramount+». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/266-paramount/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 266-paramount.service /etc/systemd/system/
sudo systemctl enable --now 266-paramount
```
