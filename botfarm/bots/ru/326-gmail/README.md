...
> Gmail: чистые аккаунты для работы и регистраций

**ID:** `ru-326-gmail` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Gmail Аккаунты — бот по теме «аккаунты Gmail». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9325
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/326-gmail/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 326-gmail.service /etc/systemd/system/
sudo systemctl enable --now 326-gmail
```
