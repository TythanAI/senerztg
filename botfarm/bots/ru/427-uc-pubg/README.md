...
> PUBG UC: пополнение по выгодному курсу за 15 минут

**ID:** `ru-427-uc-pubg` · **Регион:** RU · **Язык:** ru ·
**Модель:** topup · **Тариф цен:** budget

PUBG UC — бот по теме «пополнение UC в PUBG». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9426
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/427-uc-pubg/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 427-uc-pubg.service /etc/systemd/system/
sudo systemctl enable --now 427-uc-pubg
```
