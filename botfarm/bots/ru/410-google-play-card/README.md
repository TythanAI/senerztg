...
> Google Play: ключи и коды с моментальной выдачей

**ID:** `ru-410-google-play-card` · **Регион:** RU · **Язык:** ru ·
**Модель:** keys · **Тариф цен:** standard

Google Play — бот по теме «карты Google Play». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9409
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/410-google-play-card/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 410-google-play-card.service /etc/systemd/system/
sudo systemctl enable --now 410-google-play-card
```
