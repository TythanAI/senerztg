...
> Инструменты и промпты для работы

**ID:** `ru-116-ai-tools` · **Регион:** RU · **Язык:** ru ·
**Модель:** club · **Тариф цен:** standard

Нейросети Практика — бот по теме «нейросети». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9115
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/116-ai-tools/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 116-ai-tools.service /etc/systemd/system/
sudo systemctl enable --now 116-ai-tools
```
