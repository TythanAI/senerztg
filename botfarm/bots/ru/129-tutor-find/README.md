...
> Преподаватель под задачу

**ID:** `ru-129-tutor-find` · **Регион:** RU · **Язык:** ru ·
**Модель:** leadgen · **Тариф цен:** standard

Подбор Репетитора — бот по теме «репетиторы». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9128
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/129-tutor-find/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 129-tutor-find.service /etc/systemd/system/
sudo systemctl enable --now 129-tutor-find
```
