...
> Peacock дешевле официальной подписки, выдача сразу

**ID:** `ru-267-peacock` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Peacock Подписки — бот по теме «аккаунты Peacock». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9266
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/267-peacock/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 267-peacock.service /etc/systemd/system/
sudo systemctl enable --now 267-peacock
```
