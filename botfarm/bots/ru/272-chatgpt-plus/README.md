...
> ChatGPT Plus без карты и VPN — доступ за минуту

**ID:** `ru-272-chatgpt-plus` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

ChatGPT Plus Доступ — бот по теме «аккаунты ChatGPT Plus». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9271
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/272-chatgpt-plus/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 272-chatgpt-plus.service /etc/systemd/system/
sudo systemctl enable --now 272-chatgpt-plus
```
