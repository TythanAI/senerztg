...
> LinkedIn: отлежавшиеся аккаунты с гарантией

**ID:** `ru-311-linkedin` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

LinkedIn Аккаунты — бот по теме «аккаунты LinkedIn». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9310
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/311-linkedin/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 311-linkedin.service /etc/systemd/system/
sudo systemctl enable --now 311-linkedin
```
