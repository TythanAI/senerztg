...
> Pika без карты и VPN — доступ за минуту

**ID:** `ru-295-pika` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Pika Доступ — бот по теме «аккаунты Pika». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9294
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/295-pika/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 295-pika.service /etc/systemd/system/
sudo systemctl enable --now 295-pika
```
