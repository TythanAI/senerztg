...
> Авито Монитор: уведомления в этот чат в реальном времени

**ID:** `ru-463-avito-monitor` · **Регион:** RU · **Язык:** ru ·
**Модель:** monitor · **Тариф цен:** standard

Авито Монитор — бот по теме «мониторинг объявлений Авито». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9462
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/463-avito-monitor/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 463-avito-monitor.service /etc/systemd/system/
sudo systemctl enable --now 463-avito-monitor
```
