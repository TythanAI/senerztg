...
**ID:** `ru-127-photo-book` · **Регион:** RU · **Язык:** ru ·
**Модель:** leadgen · **Тариф цен:** standard

Фотограф Запись — бот по теме «фотосъёмка». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9126
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/127-photo-book/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 127-photo-book.service /etc/systemd/system/
sudo systemctl enable --now 127-photo-book
```
