...
**ID:** `ru-173-ip-register` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** standard

Регистрация ИП — бот по теме «регистрация бизнеса». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9172
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/173-ip-register/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 173-ip-register.service /etc/systemd/system/
sudo systemctl enable --now 173-ip-register
```
