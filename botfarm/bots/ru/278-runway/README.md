...
> Runway без карты и VPN — доступ за минуту

**ID:** `ru-278-runway` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Runway Доступ — бот по теме «аккаунты Runway». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9277
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/278-runway/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 278-runway.service /etc/systemd/system/
sudo systemctl enable --now 278-runway
```
