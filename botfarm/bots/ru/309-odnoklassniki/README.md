# Одноклассники Аккаунты

> Одноклассники: отлежавшиеся аккаунты с гарантией

**ID:** `ru-309-odnoklassniki` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Одноклассники Аккаунты — бот по теме «аккаунты Одноклассники». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9308
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/309-odnoklassniki/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 309-odnoklassniki.service /etc/systemd/system/
sudo systemctl enable --now 309-odnoklassniki
```
