...
> VLESS: быстрая выдача, замена при проблемах

**ID:** `ru-373-vless-config` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

VLESS Доступ — бот по теме «конфиги VLESS». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9372
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/373-vless-config/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 373-vless-config.service /etc/systemd/system/
sudo systemctl enable --now 373-vless-config
```
