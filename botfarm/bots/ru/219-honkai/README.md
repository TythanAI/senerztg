# Honkai Star Rail Аккаунты

> Honkai Star Rail: моментальная выдача, гарантия и замена

**ID:** `ru-219-honkai` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Honkai Star Rail Аккаунты — бот по теме «аккаунты Honkai». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/219-honkai/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 219-honkai.service /etc/systemd/system/
sudo systemctl enable --now 219-honkai
```
