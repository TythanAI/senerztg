# Apple Music Подписки

> Apple Music дешевле официальной подписки, выдача сразу

**ID:** `ru-249-apple-music` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Apple Music Подписки — бот по теме «аккаунты Apple Music». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9248
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/249-apple-music/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 249-apple-music.service /etc/systemd/system/
sudo systemctl enable --now 249-apple-music
```
