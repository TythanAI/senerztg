...
> Reels и Shorts, которые смотрят

**ID:** `ru-134-video-edit` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** standard

Монтаж Видео — бот по теме «видеомонтаж». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9133
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/134-video-edit/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 134-video-edit.service /etc/systemd/system/
sudo systemctl enable --now 134-video-edit
```
