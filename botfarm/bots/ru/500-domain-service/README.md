...
> Домены: делаем под ключ, сроки фиксируем

**ID:** `ru-500-domain-service` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** standard

Домены — бот по теме «регистрация доменов». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9499
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/500-domain-service/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 500-domain-service.service /etc/systemd/system/
sudo systemctl enable --now 500-domain-service
```
