...
> BingX: верифицированные аккаунты под задачи

**ID:** `ru-387-bingx-acc` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** premium

BingX Аккаунты — бот по теме «аккаунты BingX». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9386
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/387-bingx-acc/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 387-bingx-acc.service /etc/systemd/system/
sudo systemctl enable --now 387-bingx-acc
```
