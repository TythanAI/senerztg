...
**ID:** `ru-104-devops-lab` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** elite

DevOps Лаб — бот по теме «DevOps». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9103
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/104-devops-lab/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 104-devops-lab.service /etc/systemd/system/
sudo systemctl enable --now 104-devops-lab
```
