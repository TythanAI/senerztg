...
> Все предметы девятого класса

**ID:** `ru-039-oge-prep` · **Регион:** RU · **Язык:** ru ·
**Модель:** club · **Тариф цен:** budget

ОГЭ Подготовка — бот по теме «ОГЭ». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9038
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/039-oge-prep/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 039-oge-prep.service /etc/systemd/system/
sudo systemctl enable --now 039-oge-prep
```
