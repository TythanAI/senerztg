...
> Synthesia без карты и VPN — доступ за минуту

**ID:** `ru-286-synthesia` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Synthesia Доступ — бот по теме «аккаунты Synthesia». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9285
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/286-synthesia/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 286-synthesia.service /etc/systemd/system/
sudo systemctl enable --now 286-synthesia
```
