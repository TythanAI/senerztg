...
> RUTUBE: отлежавшиеся аккаунты с гарантией

**ID:** `ru-322-rutube` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

RUTUBE Аккаунты — бот по теме «аккаунты RUTUBE». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9321
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/322-rutube/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 322-rutube.service /etc/systemd/system/
sudo systemctl enable --now 322-rutube
```
