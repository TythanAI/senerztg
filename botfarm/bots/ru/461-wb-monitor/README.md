...
> Wildberries Монитор: уведомления в этот чат в реальном времени

**ID:** `ru-461-wb-monitor` · **Регион:** RU · **Язык:** ru ·
**Модель:** monitor · **Тариф цен:** standard

Wildberries Монитор — бот по теме «мониторинг цен Wildberries». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9460
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/461-wb-monitor/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 461-wb-monitor.service /etc/systemd/system/
sudo systemctl enable --now 461-wb-monitor
```
