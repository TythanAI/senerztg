...
> Авторские схемы и описания

**ID:** `ru-137-knit-patterns` · **Регион:** RU · **Язык:** ru ·
**Модель:** shop · **Тариф цен:** budget

Вязание Схемы — бот по теме «вязание». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9136
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/137-knit-patterns/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 137-knit-patterns.service /etc/systemd/system/
sudo systemctl enable --now 137-knit-patterns
```
