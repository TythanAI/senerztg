...
> Подбор команды на потоке

**ID:** `ru-094-hr-hiring` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** premium

Найм Без Боли — бот по теме «подбор персонала». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9093
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/094-hr-hiring/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 094-hr-hiring.service /etc/systemd/system/
sudo systemctl enable --now 094-hr-hiring
```
