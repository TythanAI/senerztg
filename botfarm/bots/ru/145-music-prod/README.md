...
> Свой трек от идеи до релиза

**ID:** `ru-145-music-prod` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** premium

Продюсирование Музыки — бот по теме «музыкальное продюсирование». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9144
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/145-music-prod/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 145-music-prod.service /etc/systemd/system/
sudo systemctl enable --now 145-music-prod
```
