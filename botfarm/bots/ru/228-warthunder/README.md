# War Thunder Аккаунты

> War Thunder: моментальная выдача, гарантия и замена

**ID:** `ru-228-warthunder` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

War Thunder Аккаунты — бот по теме «аккаунты War Thunder». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9227
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/228-warthunder/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 228-warthunder.service /etc/systemd/system/
sudo systemctl enable --now 228-warthunder
```
