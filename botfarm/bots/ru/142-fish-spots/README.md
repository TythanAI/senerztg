...
**ID:** `ru-142-fish-spots` · **Регион:** RU · **Язык:** ru ·
**Модель:** club · **Тариф цен:** budget

Рыбные Места — бот по теме «рыбалка». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9141
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/142-fish-spots/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 142-fish-spots.service /etc/systemd/system/
sudo systemctl enable --now 142-fish-spots
```
