# Яндекс Диск Аккаунты

> Яндекс Диск: чистые аккаунты для работы и регистраций

**ID:** `ru-337-yandex-disk` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Яндекс Диск Аккаунты — бот по теме «аккаунты Яндекс Диска». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9336
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/337-yandex-disk/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 337-yandex-disk.service /etc/systemd/system/
sudo systemctl enable --now 337-yandex-disk
```
