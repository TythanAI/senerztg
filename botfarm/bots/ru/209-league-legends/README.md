# League of Legends Аккаунты

> League of Legends: моментальная выдача, гарантия и замена

**ID:** `ru-209-league-legends` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

League of Legends Аккаунты — бот по теме «аккаунты League of Legends». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9208
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/209-league-legends/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 209-league-legends.service /etc/systemd/system/
sudo systemctl enable --now 209-league-legends
```
