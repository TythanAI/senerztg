...
> Первая поставка и выход в прибыль

**ID:** `ru-083-marketplace-wb` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** premium

Wildberries Старт — бот по теме «торговля на Wildberries». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9082
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/083-marketplace-wb/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 083-marketplace-wb.service /etc/systemd/system/
sudo systemctl enable --now 083-marketplace-wb
```
