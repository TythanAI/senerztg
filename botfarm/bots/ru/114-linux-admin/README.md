...
**ID:** `ru-114-linux-admin` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** premium

Linux Админ — бот по теме «администрирование Linux». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9113
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/114-linux-admin/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 114-linux-admin.service /etc/systemd/system/
sudo systemctl enable --now 114-linux-admin
```
