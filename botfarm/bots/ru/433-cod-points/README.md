...
> Call of Duty: пополнение по выгодному курсу за 15 минут

**ID:** `ru-433-cod-points` · **Регион:** RU · **Язык:** ru ·
**Модель:** topup · **Тариф цен:** budget

Call of Duty — бот по теме «CP в Call of Duty». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9432
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/433-cod-points/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 433-cod-points.service /etc/systemd/system/
sudo systemctl enable --now 433-cod-points
```
