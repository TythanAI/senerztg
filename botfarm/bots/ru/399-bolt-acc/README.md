...
> Bolt: готовые аккаунты для работы

**ID:** `ru-399-bolt-acc` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Bolt Аккаунты — бот по теме «аккаунты Bolt». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9398
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/399-bolt-acc/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 399-bolt-acc.service /etc/systemd/system/
sudo systemctl enable --now 399-bolt-acc
```
