...
> Discord: пополнение по выгодному курсу за 15 минут

**ID:** `ru-444-discord-boost` · **Регион:** RU · **Язык:** ru ·
**Модель:** topup · **Тариф цен:** budget

Discord — бот по теме «бусты Discord-сервера». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9443
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/444-discord-boost/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 444-discord-boost.service /etc/systemd/system/
sudo systemctl enable --now 444-discord-boost
```
