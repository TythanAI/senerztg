...
> Профиль на 80+ баллов

**ID:** `ru-037-ege-math` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** standard

ЕГЭ Математика — бот по теме «ЕГЭ по математике». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9036
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/037-ege-math/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 037-ege-math.service /etc/systemd/system/
sudo systemctl enable --now 037-ege-math
```
