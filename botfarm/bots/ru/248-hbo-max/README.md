...
> HBO Max дешевле официальной подписки, выдача сразу

**ID:** `ru-248-hbo-max` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

HBO Max Подписки — бот по теме «аккаунты HBO Max». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9247
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/248-hbo-max/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 248-hbo-max.service /etc/systemd/system/
sudo systemctl enable --now 248-hbo-max
```
