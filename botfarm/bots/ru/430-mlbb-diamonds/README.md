...
> Mobile Legends: пополнение по выгодному курсу за 15 минут

**ID:** `ru-430-mlbb-diamonds` · **Регион:** RU · **Язык:** ru ·
**Модель:** topup · **Тариф цен:** budget

Mobile Legends — бот по теме «алмазы Mobile Legends». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9429
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/430-mlbb-diamonds/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 430-mlbb-diamonds.service /etc/systemd/system/
sudo systemctl enable --now 430-mlbb-diamonds
```
