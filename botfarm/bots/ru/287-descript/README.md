...
> Descript без карты и VPN — доступ за минуту

**ID:** `ru-287-descript` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Descript Доступ — бот по теме «аккаунты Descript». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9286
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/287-descript/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 287-descript.service /etc/systemd/system/
sudo systemctl enable --now 287-descript
```
