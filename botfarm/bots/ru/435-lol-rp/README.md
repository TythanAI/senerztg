...
> League of Legends: пополнение по выгодному курсу за 15 минут

**ID:** `ru-435-lol-rp` · **Регион:** RU · **Язык:** ru ·
**Модель:** topup · **Тариф цен:** budget

League of Legends — бот по теме «Riot Points». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9434
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/435-lol-rp/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 435-lol-rp.service /etc/systemd/system/
sudo systemctl enable --now 435-lol-rp
```
