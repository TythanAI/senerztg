...
> Поставщики, логистика, таможня

**ID:** `ru-097-import-china` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** premium

Импорт из Китая — бот по теме «импорт из Китая». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9096
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/097-import-china/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 097-import-china.service /etc/systemd/system/
sudo systemctl enable --now 097-import-china
```
