...
> Midjourney без карты и VPN — доступ за минуту

**ID:** `ru-274-midjourney` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Midjourney Доступ — бот по теме «аккаунты Midjourney». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9273
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/274-midjourney/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 274-midjourney.service /etc/systemd/system/
sudo systemctl enable --now 274-midjourney
```
