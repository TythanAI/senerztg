...
> Первая крипта: кошельки, биржи, безопасность

**ID:** `ru-019-crypto-base` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** standard

Крипта База — бот по теме «криптовалюты». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9018
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/019-crypto-base/docker-compose.yml up -d
```
