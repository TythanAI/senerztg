...
> Свой сервер за пять минут

**ID:** `ru-196-minecraft-serv` · **Регион:** RU · **Язык:** ru ·
**Модель:** saas · **Тариф цен:** budget

Майнкрафт Сервер — бот по теме «серверы Minecraft». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9195
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/196-minecraft-serv/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 196-minecraft-serv.service /etc/systemd/system/
sudo systemctl enable --now 196-minecraft-serv
```
