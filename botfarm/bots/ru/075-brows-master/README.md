...
> Архитектура и окрашивание

**ID:** `ru-075-brows-master` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** standard

Мастер Бровей — бот по теме «брови». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9074
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/075-brows-master/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 075-brows-master.service /etc/systemd/system/
sudo systemctl enable --now 075-brows-master
```
