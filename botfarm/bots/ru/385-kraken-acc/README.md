...
> Kraken: верифицированные аккаунты под задачи

**ID:** `ru-385-kraken-acc` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** premium

Kraken Аккаунты — бот по теме «аккаунты Kraken». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9384
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/385-kraken-acc/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 385-kraken-acc.service /etc/systemd/system/
sudo systemctl enable --now 385-kraken-acc
```
