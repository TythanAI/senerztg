...
> Proton VPN: быстрая выдача, замена при проблемах

**ID:** `ru-365-protonvpn` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Proton VPN Доступ — бот по теме «аккаунты Proton VPN». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9364
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/365-protonvpn/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 365-protonvpn.service /etc/systemd/system/
sudo systemctl enable --now 365-protonvpn
```
