...
> Asana по цене в разы ниже подписки

**ID:** `ru-352-asana` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Asana Доступ — бот по теме «аккаунты Asana». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9351
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/352-asana/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 352-asana.service /etc/systemd/system/
sudo systemctl enable --now 352-asana
```
