...
> Brawl Stars: моментальная выдача, гарантия и замена

**ID:** `ru-221-brawl-stars` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Brawl Stars Аккаунты — бот по теме «аккаунты Brawl Stars». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/221-brawl-stars/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 221-brawl-stars.service /etc/systemd/system/
sudo systemctl enable --now 221-brawl-stars
```
