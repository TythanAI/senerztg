...
> amoCRM и Битрикс24 под ключ

**ID:** `ru-087-crm-setup` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** elite

CRM Внедрение — бот по теме «внедрение CRM». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9086
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/087-crm-setup/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 087-crm-setup.service /etc/systemd/system/
sudo systemctl enable --now 087-crm-setup
```
