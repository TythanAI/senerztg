...
> Viber: отлежавшиеся аккаунты с гарантией

**ID:** `ru-318-viber` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Viber Аккаунты — бот по теме «аккаунты Viber». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9317
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/318-viber/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 318-viber.service /etc/systemd/system/
sudo systemctl enable --now 318-viber
```
