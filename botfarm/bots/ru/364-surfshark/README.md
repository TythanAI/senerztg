...
> Surfshark: быстрая выдача, замена при проблемах

**ID:** `ru-364-surfshark` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Surfshark Доступ — бот по теме «аккаунты Surfshark». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9363
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/364-surfshark/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 364-surfshark.service /etc/systemd/system/
sudo systemctl enable --now 364-surfshark
```
