...
> Разборы ароматов и находки

**ID:** `ru-077-perfume-club` · **Регион:** RU · **Язык:** ru ·
**Модель:** club · **Тариф цен:** budget

Парфюм Клуб — бот по теме «парфюмерия». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9076
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/077-perfume-club/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 077-perfume-club.service /etc/systemd/system/
sudo systemctl enable --now 077-perfume-club
```
