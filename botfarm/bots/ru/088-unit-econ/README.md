...
> Считаем прибыль до запуска

**ID:** `ru-088-unit-econ` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** premium

Юнит-Экономика — бот по теме «юнит-экономика». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9087
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/088-unit-econ/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 088-unit-econ.service /etc/systemd/system/
sudo systemctl enable --now 088-unit-econ
```
