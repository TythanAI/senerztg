...
> Wise: готовые аккаунты для работы

**ID:** `ru-402-wise-acc` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Wise Аккаунты — бот по теме «аккаунты Wise». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9401
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/402-wise-acc/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 402-wise-acc.service /etc/systemd/system/
sudo systemctl enable --now 402-wise-acc
```
