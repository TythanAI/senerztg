# PlayStation Network Аккаунты

> PlayStation Network: моментальная выдача, гарантия и замена

**ID:** `ru-241-psn` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

PlayStation Network Аккаунты — бот по теме «аккаунты PSN». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9240
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/241-psn/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 241-psn.service /etc/systemd/system/
sudo systemctl enable --now 241-psn
```
