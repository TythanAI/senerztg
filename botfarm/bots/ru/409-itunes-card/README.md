...
> iTunes Card: ключи и коды с моментальной выдачей

**ID:** `ru-409-itunes-card` · **Регион:** RU · **Язык:** ru ·
**Модель:** keys · **Тариф цен:** standard

iTunes Card — бот по теме «карты iTunes». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9408
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/409-itunes-card/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 409-itunes-card.service /etc/systemd/system/
sudo systemctl enable --now 409-itunes-card
```
