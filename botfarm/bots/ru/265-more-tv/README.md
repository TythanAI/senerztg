...
> MORE.TV дешевле официальной подписки, выдача сразу

**ID:** `ru-265-more-tv` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

MORE.TV Подписки — бот по теме «подписки MORE.TV». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/265-more-tv/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 265-more-tv.service /etc/systemd/system/
sudo systemctl enable --now 265-more-tv
```
