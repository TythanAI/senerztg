# Albion Online Аккаунты

> Albion Online: моментальная выдача, гарантия и замена

**ID:** `ru-238-albion` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Albion Online Аккаунты — бот по теме «аккаунты Albion». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/238-albion/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 238-albion.service /etc/systemd/system/
sudo systemctl enable --now 238-albion
```
