...
> Coinbase: верифицированные аккаунты под задачи

**ID:** `ru-386-coinbase-acc` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** premium

Coinbase Аккаунты — бот по теме «аккаунты Coinbase». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9385
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/386-coinbase-acc/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 386-coinbase-acc.service /etc/systemd/system/
sudo systemctl enable --now 386-coinbase-acc
```
