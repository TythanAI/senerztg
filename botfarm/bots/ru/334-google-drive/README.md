# Google Drive Аккаунты

> Google Drive: чистые аккаунты для работы и регистраций

**ID:** `ru-334-google-drive` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Google Drive Аккаунты — бот по теме «хранилище Google Drive». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9333
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/334-google-drive/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 334-google-drive.service /etc/systemd/system/
sudo systemctl enable --now 334-google-drive
```
