# Perplexity Pro Доступ

> Perplexity Pro без карты и VPN — доступ за минуту

**ID:** `ru-275-perplexity` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Perplexity Pro Доступ — бот по теме «аккаунты Perplexity». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/275-perplexity/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 275-perplexity.service /etc/systemd/system/
sudo systemctl enable --now 275-perplexity
```
