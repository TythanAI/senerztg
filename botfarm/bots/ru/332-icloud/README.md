...
> iCloud: чистые аккаунты для работы и регистраций

**ID:** `ru-332-icloud` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

iCloud Аккаунты — бот по теме «аккаунты iCloud». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9331
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/332-icloud/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 332-icloud.service /etc/systemd/system/
sudo systemctl enable --now 332-icloud
```
