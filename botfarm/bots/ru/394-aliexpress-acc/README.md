...
> AliExpress: готовые аккаунты для работы

**ID:** `ru-394-aliexpress-acc` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

AliExpress Аккаунты — бот по теме «аккаунты AliExpress». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9393
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/394-aliexpress-acc/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 394-aliexpress-acc.service /etc/systemd/system/
sudo systemctl enable --now 394-aliexpress-acc
```
