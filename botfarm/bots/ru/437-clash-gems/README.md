...
> Clash of Clans: пополнение по выгодному курсу за 15 минут

**ID:** `ru-437-clash-gems` · **Регион:** RU · **Язык:** ru ·
**Модель:** topup · **Тариф цен:** budget

Clash of Clans — бот по теме «гемы Clash of Clans». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9436
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/437-clash-gems/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 437-clash-gems.service /etc/systemd/system/
sudo systemctl enable --now 437-clash-gems
```
