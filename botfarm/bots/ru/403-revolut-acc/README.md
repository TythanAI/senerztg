...
> Revolut: готовые аккаунты для работы

**ID:** `ru-403-revolut-acc` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Revolut Аккаунты — бот по теме «аккаунты Revolut». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9402
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/403-revolut-acc/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 403-revolut-acc.service /etc/systemd/system/
sudo systemctl enable --now 403-revolut-acc
```
