...
> Разбор реплеев и стратегия

**ID:** `ru-190-dota-coach` · **Регион:** RU · **Язык:** ru ·
**Модель:** consult · **Тариф цен:** standard

Dota Тренер — бот по теме «Dota 2». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9189
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/190-dota-coach/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 190-dota-coach.service /etc/systemd/system/
sudo systemctl enable --now 190-dota-coach
```
