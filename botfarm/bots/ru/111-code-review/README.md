...
> Разбор вашего кода экспертом

**ID:** `ru-111-code-review` · **Регион:** RU · **Язык:** ru ·
**Модель:** consult · **Тариф цен:** premium

Код Ревью — бот по теме «код-ревью». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9110
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/111-code-review/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 111-code-review.service /etc/systemd/system/
sudo systemctl enable --now 111-code-review
```
