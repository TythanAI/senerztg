# Mobile Legends Аккаунты

> Mobile Legends: моментальная выдача, гарантия и замена

**ID:** `ru-224-mobile-legends-acc` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Mobile Legends Аккаунты — бот по теме «аккаунты Mobile Legends». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9223
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/224-mobile-legends-acc/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 224-mobile-legends-acc.service /etc/systemd/system/
sudo systemctl enable --now 224-mobile-legends-acc
```
