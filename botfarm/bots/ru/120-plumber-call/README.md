...
**ID:** `ru-120-plumber-call` · **Регион:** RU · **Язык:** ru ·
**Модель:** leadgen · **Тариф цен:** standard

Сантехник Вызов — бот по теме «сантехника». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9119
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/120-plumber-call/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 120-plumber-call.service /etc/systemd/system/
sudo systemctl enable --now 120-plumber-call
```
