...
> PayPal: готовые аккаунты для работы

**ID:** `ru-401-paypal-acc` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

PayPal Аккаунты — бот по теме «аккаунты PayPal». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9400
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/401-paypal-acc/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 401-paypal-acc.service /etc/systemd/system/
sudo systemctl enable --now 401-paypal-acc
```
