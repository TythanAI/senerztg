...
> Настройка Кошелька: быстро, по курсу, без лишних вопросов

**ID:** `ru-486-crypto-wallet-setup` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** premium

Настройка Кошелька — бот по теме «настройка криптокошелька». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9485
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/486-crypto-wallet-setup/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 486-crypto-wallet-setup.service /etc/systemd/system/
sudo systemctl enable --now 486-crypto-wallet-setup
```
