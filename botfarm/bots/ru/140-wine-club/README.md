...
> Дегустации и подбор вин

**ID:** `ru-140-wine-club` · **Регион:** RU · **Язык:** ru ·
**Модель:** club · **Тариф цен:** standard

Винный Клуб — бот по теме «вино». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9139
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/140-wine-club/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 140-wine-club.service /etc/systemd/system/
sudo systemctl enable --now 140-wine-club
```
