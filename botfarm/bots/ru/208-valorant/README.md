...
> Valorant: моментальная выдача, гарантия и замена

**ID:** `ru-208-valorant` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Valorant Аккаунты — бот по теме «аккаунты Valorant». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/208-valorant/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 208-valorant.service /etc/systemd/system/
sudo systemctl enable --now 208-valorant
```
