...
> Сочинение и тест без ошибок

**ID:** `ru-038-ege-russian` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** standard

ЕГЭ Русский — бот по теме «ЕГЭ по русскому». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9037
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/038-ege-russian/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 038-ege-russian.service /etc/systemd/system/
sudo systemctl enable --now 038-ege-russian
```
