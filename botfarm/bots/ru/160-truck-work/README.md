...
> Рейсы, ставки, документы

**ID:** `ru-160-truck-work` · **Регион:** RU · **Язык:** ru ·
**Модель:** club · **Тариф цен:** standard

Дальнобой Работа — бот по теме «грузоперевозки». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9159
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/160-truck-work/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 160-truck-work.service /etc/systemd/system/
sudo systemctl enable --now 160-truck-work
```
