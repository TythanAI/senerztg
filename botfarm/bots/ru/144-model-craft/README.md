...
> Схемы и наборы для сборки

**ID:** `ru-144-model-craft` · **Регион:** RU · **Язык:** ru ·
**Модель:** shop · **Тариф цен:** standard

Моделизм — бот по теме «моделизм». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9143
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/144-model-craft/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 144-model-craft.service /etc/systemd/system/
sudo systemctl enable --now 144-model-craft
```
