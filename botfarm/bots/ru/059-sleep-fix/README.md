...
**ID:** `ru-059-sleep-fix` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** budget

Сон Наладить — бот по теме «качество сна». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9058
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/059-sleep-fix/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 059-sleep-fix.service /etc/systemd/system/
sudo systemctl enable --now 059-sleep-fix
```
