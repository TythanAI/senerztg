...
**ID:** `ru-108-game-dev` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** premium

Геймдев Старт — бот по теме «разработка игр». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9107
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/108-game-dev/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 108-game-dev.service /etc/systemd/system/
sudo systemctl enable --now 108-game-dev
```
