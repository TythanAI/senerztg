...
> Notion по цене в разы ниже подписки

**ID:** `ru-348-notion` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Notion Доступ — бот по теме «аккаунты Notion». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9347
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/348-notion/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 348-notion.service /etc/systemd/system/
sudo systemctl enable --now 348-notion
```
