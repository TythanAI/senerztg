...
> Miro по цене в разы ниже подписки

**ID:** `ru-353-miro` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Miro Доступ — бот по теме «аккаунты Miro». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9352
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/353-miro/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 353-miro.service /etc/systemd/system/
sudo systemctl enable --now 353-miro
```
