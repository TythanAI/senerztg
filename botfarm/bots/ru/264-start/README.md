...
> START дешевле официальной подписки, выдача сразу

**ID:** `ru-264-start` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

START Подписки — бот по теме «подписки START». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9263
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/264-start/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 264-start.service /etc/systemd/system/
sudo systemctl enable --now 264-start
```
