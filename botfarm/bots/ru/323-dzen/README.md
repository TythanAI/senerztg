...
> Дзен: отлежавшиеся аккаунты с гарантией

**ID:** `ru-323-dzen` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Дзен Аккаунты — бот по теме «аккаунты Дзен». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9322
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/323-dzen/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 323-dzen.service /etc/systemd/system/
sudo systemctl enable --now 323-dzen
```
