...
> Valve: ключи и коды с моментальной выдачей

**ID:** `ru-421-valve-key` · **Регион:** RU · **Язык:** ru ·
**Модель:** keys · **Тариф цен:** standard

Valve — бот по теме «ключи Valve». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9420
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/421-valve-key/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 421-valve-key.service /etc/systemd/system/
sudo systemctl enable --now 421-valve-key
```
