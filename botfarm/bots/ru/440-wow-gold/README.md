...
> WoW: пополнение по выгодному курсу за 15 минут

**ID:** `ru-440-wow-gold` · **Регион:** RU · **Язык:** ru ·
**Модель:** topup · **Тариф цен:** budget

WoW — бот по теме «золото World of Warcraft». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9439
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/440-wow-gold/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 440-wow-gold.service /etc/systemd/system/
sudo systemctl enable --now 440-wow-gold
```
