...
> Находим слив бюджета за час

**ID:** `ru-100-ads-audit` · **Регион:** RU · **Язык:** ru ·
**Модель:** consult · **Тариф цен:** premium

Аудит Рекламы — бот по теме «аудит рекламы». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9099
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/100-ads-audit/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 100-ads-audit.service /etc/systemd/system/
sudo systemctl enable --now 100-ads-audit
```
