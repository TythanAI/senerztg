...
> Реклама во ВК и Telegram Ads

**ID:** `ru-082-target-ads` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** premium

Таргет Про — бот по теме «таргетированная реклама». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9081
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/082-target-ads/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 082-target-ads.service /etc/systemd/system/
sudo systemctl enable --now 082-target-ads
```
