...
> Кинопоиск дешевле официальной подписки, выдача сразу

**ID:** `ru-256-kinopoisk` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Кинопоиск Подписки — бот по теме «подписки Кинопоиск». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/256-kinopoisk/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 256-kinopoisk.service /etc/systemd/system/
sudo systemctl enable --now 256-kinopoisk
```
