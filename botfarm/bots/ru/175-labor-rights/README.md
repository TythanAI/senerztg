...
> Увольнение, зарплата, суд

**ID:** `ru-175-labor-rights` · **Регион:** RU · **Язык:** ru ·
**Модель:** consult · **Тариф цен:** premium

Трудовые Споры — бот по теме «трудовое право». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9174
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/175-labor-rights/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 175-labor-rights.service /etc/systemd/system/
sudo systemctl enable --now 175-labor-rights
```
