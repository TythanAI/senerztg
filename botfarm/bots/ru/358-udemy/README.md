...
> Udemy по цене в разы ниже подписки

**ID:** `ru-358-udemy` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Udemy Доступ — бот по теме «аккаунты Udemy». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9357
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/358-udemy/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 358-udemy.service /etc/systemd/system/
sudo systemctl enable --now 358-udemy
```
