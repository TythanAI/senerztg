...
> Карточки, реклама, юнит-экономика

**ID:** `ru-084-ozon-seller` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** premium

Ozon Продавец — бот по теме «торговля на Ozon». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9083
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/084-ozon-seller/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 084-ozon-seller.service /etc/systemd/system/
sudo systemctl enable --now 084-ozon-seller
```
