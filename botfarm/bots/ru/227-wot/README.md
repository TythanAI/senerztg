# World of Tanks Аккаунты

> World of Tanks: моментальная выдача, гарантия и замена

**ID:** `ru-227-wot` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

World of Tanks Аккаунты — бот по теме «аккаунты World of Tanks». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/227-wot/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 227-wot.service /etc/systemd/system/
sudo systemctl enable --now 227-wot
```
