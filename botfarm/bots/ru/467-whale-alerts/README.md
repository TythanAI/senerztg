...
> Whale Alerts: уведомления в этот чат в реальном времени

**ID:** `ru-467-whale-alerts` · **Регион:** RU · **Язык:** ru ·
**Модель:** monitor · **Тариф цен:** standard

Whale Alerts — бот по теме «отслеживание крупных транзакций». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9466
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/467-whale-alerts/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 467-whale-alerts.service /etc/systemd/system/
sudo systemctl enable --now 467-whale-alerts
```
