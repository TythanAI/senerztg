...
> Смета и бригада за один день

**ID:** `ru-117-repair-flat` · **Регион:** RU · **Язык:** ru ·
**Модель:** leadgen · **Тариф цен:** premium

Ремонт Квартир — бот по теме «ремонт квартир». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9116
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/117-repair-flat/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 117-repair-flat.service /etc/systemd/system/
sudo systemctl enable --now 117-repair-flat
```
