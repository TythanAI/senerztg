...
> Подготовка к IELTS на 7.0+

**ID:** `ru-034-ielts-target` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** premium

IELTS Цель — бот по теме «IELTS». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9033
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/034-ielts-target/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 034-ielts-target.service /etc/systemd/system/
sudo systemctl enable --now 034-ielts-target
```
