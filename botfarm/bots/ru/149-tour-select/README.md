...
> Отдых под бюджет и даты

**ID:** `ru-149-tour-select` · **Регион:** RU · **Язык:** ru ·
**Модель:** leadgen · **Тариф цен:** standard

Подбор Тура — бот по теме «туры». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9148
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/149-tour-select/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 149-tour-select.service /etc/systemd/system/
sudo systemctl enable --now 149-tour-select
```
