# Adobe Creative Cloud Доступ

> Adobe Creative Cloud по цене в разы ниже подписки

**ID:** `ru-347-adobe-cc` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Adobe Creative Cloud Доступ — бот по теме «аккаунты Adobe CC». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9346
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/347-adobe-cc/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 347-adobe-cc.service /etc/systemd/system/
sudo systemctl enable --now 347-adobe-cc
```
