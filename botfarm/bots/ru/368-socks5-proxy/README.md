...
> SOCKS5: быстрая выдача, замена при проблемах

**ID:** `ru-368-socks5-proxy` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

SOCKS5 Доступ — бот по теме «прокси SOCKS5». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9367
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/368-socks5-proxy/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 368-socks5-proxy.service /etc/systemd/system/
sudo systemctl enable --now 368-socks5-proxy
```
