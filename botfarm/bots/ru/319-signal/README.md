...
> Signal: отлежавшиеся аккаунты с гарантией

**ID:** `ru-319-signal` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Signal Аккаунты — бот по теме «аккаунты Signal». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9318
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/319-signal/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 319-signal.service /etc/systemd/system/
sudo systemctl enable --now 319-signal
```
