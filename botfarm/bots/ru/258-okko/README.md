...
> Okko дешевле официальной подписки, выдача сразу

**ID:** `ru-258-okko` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Okko Подписки — бот по теме «подписки Okko». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/258-okko/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 258-okko.service /etc/systemd/system/
sudo systemctl enable --now 258-okko
```
