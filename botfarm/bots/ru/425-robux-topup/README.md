...
> Robux: пополнение по выгодному курсу за 15 минут

**ID:** `ru-425-robux-topup` · **Регион:** RU · **Язык:** ru ·
**Модель:** topup · **Тариф цен:** budget

Robux — бот по теме «пополнение Robux». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9424
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/425-robux-topup/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 425-robux-topup.service /etc/systemd/system/
sudo systemctl enable --now 425-robux-topup
```
