...
> Выбор профессии по данным, а не по интуиции

**ID:** `ru-045-prof-orient` · **Регион:** RU · **Язык:** ru ·
**Модель:** consult · **Тариф цен:** standard

Профориентация — бот по теме «профориентация». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9044

## Каталог

| SKU | Название | Цена | Тип |
|-----|----------|------|-----|
| `consult30` | Консультация 30 минут | 1 480 ₽ | consult |
| `consult60` | Консультация 60 минут | 2 670 ₽ | consult |
| `pack5` | Пакет из 5 встреч | 11 170 ₽ | consult |
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/045-prof-orient/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 045-prof-orient.service /etc/systemd/system/
sudo systemctl enable --now 045-prof-orient
```
