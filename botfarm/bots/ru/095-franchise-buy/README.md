...
> Проверяем франшизу до покупки

**ID:** `ru-095-franchise-buy` · **Регион:** RU · **Язык:** ru ·
**Модель:** consult · **Тариф цен:** elite

Франшиза Аудит — бот по теме «франшизы». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9094
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/095-franchise-buy/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 095-franchise-buy.service /etc/systemd/system/
sudo systemctl enable --now 095-franchise-buy
```
