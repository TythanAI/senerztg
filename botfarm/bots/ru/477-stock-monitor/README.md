...
> Наличие Монитор: уведомления в этот чат в реальном времени

**ID:** `ru-477-stock-monitor` · **Регион:** RU · **Язык:** ru ·
**Модель:** monitor · **Тариф цен:** standard

Наличие Монитор — бот по теме «мониторинг наличия товара». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9476
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/477-stock-monitor/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 477-stock-monitor.service /etc/systemd/system/
sudo systemctl enable --now 477-stock-monitor
```
