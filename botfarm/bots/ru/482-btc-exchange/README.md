...
> BTC Обмен: быстро, по курсу, без лишних вопросов

**ID:** `ru-482-btc-exchange` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** premium

BTC Обмен — бот по теме «обмен биткоина». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9481
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/482-btc-exchange/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 482-btc-exchange.service /etc/systemd/system/
sudo systemctl enable --now 482-btc-exchange
```
