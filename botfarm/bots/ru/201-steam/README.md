...
> Steam: моментальная выдача, гарантия и замена

**ID:** `ru-201-steam` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Steam Аккаунты — бот по теме «аккаунты Steam». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/201-steam/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 201-steam.service /etc/systemd/system/
sudo systemctl enable --now 201-steam
```
