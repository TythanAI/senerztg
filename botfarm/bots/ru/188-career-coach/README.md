...
> Рост, оффер, переговоры о зарплате

**ID:** `ru-188-career-coach` · **Регион:** RU · **Язык:** ru ·
**Модель:** consult · **Тариф цен:** elite

Карьерный Коуч — бот по теме «карьерный коучинг». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9187
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/188-career-coach/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 188-career-coach.service /etc/systemd/system/
sudo systemctl enable --now 188-career-coach
```
