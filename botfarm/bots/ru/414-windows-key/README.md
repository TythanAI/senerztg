...
> Windows: ключи и коды с моментальной выдачей

**ID:** `ru-414-windows-key` · **Регион:** RU · **Язык:** ru ·
**Модель:** keys · **Тариф цен:** standard

Windows — бот по теме «ключи Windows». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9413
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/414-windows-key/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 414-windows-key.service /etc/systemd/system/
sudo systemctl enable --now 414-windows-key
```
