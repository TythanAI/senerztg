...
> Dota 2: моментальная выдача, гарантия и замена

**ID:** `ru-211-dota2` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Dota 2 Аккаунты — бот по теме «аккаунты Dota 2». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/211-dota2/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 211-dota2.service /etc/systemd/system/
sudo systemctl enable --now 211-dota2
```
