...
> Хостинг: делаем под ключ, сроки фиксируем

**ID:** `ru-499-hosting-service` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** standard

Хостинг — бот по теме «хостинг и VPS». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9498
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/499-hosting-service/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 499-hosting-service.service /etc/systemd/system/
sudo systemctl enable --now 499-hosting-service
```
