...
> Аниматоры и программа

**ID:** `ru-126-kids-party` · **Регион:** RU · **Язык:** ru ·
**Модель:** leadgen · **Тариф цен:** standard

Детский Праздник — бот по теме «детские праздники». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9125
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/126-kids-party/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 126-kids-party.service /etc/systemd/system/
sudo systemctl enable --now 126-kids-party
```
