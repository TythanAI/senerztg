# Google Workspace Аккаунты

> Google Workspace: чистые аккаунты для работы и регистраций

**ID:** `ru-327-google-workspace` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Google Workspace Аккаунты — бот по теме «аккаунты Google Workspace». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9326
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/327-google-workspace/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 327-google-workspace.service /etc/systemd/system/
sudo systemctl enable --now 327-google-workspace
```
