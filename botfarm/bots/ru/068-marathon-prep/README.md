...
> 42 км: план на 16 недель

**ID:** `ru-068-marathon-prep` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** premium

Марафон Подготовка — бот по теме «марафон». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9067
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/068-marathon-prep/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 068-marathon-prep.service /etc/systemd/system/
sudo systemctl enable --now 068-marathon-prep
```
