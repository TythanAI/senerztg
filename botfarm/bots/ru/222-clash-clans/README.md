# Clash of Clans Аккаунты

> Clash of Clans: моментальная выдача, гарантия и замена

**ID:** `ru-222-clash-clans` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Clash of Clans Аккаунты — бот по теме «аккаунты Clash of Clans». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/222-clash-clans/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 222-clash-clans.service /etc/systemd/system/
sudo systemctl enable --now 222-clash-clans
```
