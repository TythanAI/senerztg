...
> FIFA Монеты: результат под ключ, оплата после согласования

**ID:** `ru-454-fifa-coins` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** standard

FIFA Монеты — бот по теме «монеты FIFA Ultimate Team». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9453
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/454-fifa-coins/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 454-fifa-coins.service /etc/systemd/system/
sudo systemctl enable --now 454-fifa-coins
```
