...
> WhatsApp: отлежавшиеся аккаунты с гарантией

**ID:** `ru-317-whatsapp` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

WhatsApp Аккаунты — бот по теме «аккаунты WhatsApp». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9316
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/317-whatsapp/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 317-whatsapp.service /etc/systemd/system/
sudo systemctl enable --now 317-whatsapp
```
