...
> Coursera по цене в разы ниже подписки

**ID:** `ru-357-coursera` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Coursera Доступ — бот по теме «аккаунты Coursera». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9356
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/357-coursera/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 357-coursera.service /etc/systemd/system/
sudo systemctl enable --now 357-coursera
```
