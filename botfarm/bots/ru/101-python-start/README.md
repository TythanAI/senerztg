...
> От синтаксиса до первого проекта

**ID:** `ru-101-python-start` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** standard

Python Старт — бот по теме «Python». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9100

## Каталог

| SKU | Название | Цена | Тип |
|-----|----------|------|-----|
| `start` | Стартовый пакет | 1 480 ₽ | digital |
| `pro` | Расширенный курс | 4 010 ₽ | digital |
| `vip` | VIP с сопровождением | 8 930 ₽ | consult |
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/101-python-start/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 101-python-start.service /etc/systemd/system/
sudo systemctl enable --now 101-python-start
```
