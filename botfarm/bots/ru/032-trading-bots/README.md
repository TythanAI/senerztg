...
> Сетки и DCA без программирования

**ID:** `ru-032-trading-bots` · **Регион:** RU · **Язык:** ru ·
**Модель:** saas · **Тариф цен:** premium

Торговый Бот — бот по теме «торговые боты». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9031

## Каталог

| SKU | Название | Цена | Тип |
|-----|----------|------|-----|
| `lite` | Lite | 3 910 ₽ | subscription · 30 дн. |
| `pro` | Pro | 9 790 ₽ | subscription · 30 дн. |
| `team` | Team | 24 490 ₽ | subscription · 30 дн. |
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/032-trading-bots/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 032-trading-bots.service /etc/systemd/system/
sudo systemctl enable --now 032-trading-bots
```
