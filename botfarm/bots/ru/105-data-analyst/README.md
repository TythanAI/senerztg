...
**ID:** `ru-105-data-analyst` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** premium

Аналитик Данных — бот по теме «аналитика данных». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9104
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/105-data-analyst/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 105-data-analyst.service /etc/systemd/system/
sudo systemctl enable --now 105-data-analyst
```
