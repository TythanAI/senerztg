...
**ID:** `ru-168-interior-design` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** elite

Дизайн Интерьера — бот по теме «дизайн интерьера». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9167

## Каталог

| SKU | Название | Цена | Тип |
|-----|----------|------|-----|
| `basic` | Базовый пакет | 14 890 ₽ | service |
| `standard` | Стандарт | 35 750 ₽ | service |
| `premium` | Под ключ | 74 490 ₽ | service |
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/168-interior-design/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 168-interior-design.service /etc/systemd/system/
sudo systemctl enable --now 168-interior-design
```
