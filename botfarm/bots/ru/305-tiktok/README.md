...
> TikTok: отлежавшиеся аккаунты с гарантией

**ID:** `ru-305-tiktok` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

TikTok Аккаунты — бот по теме «аккаунты TikTok». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9304
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/305-tiktok/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 305-tiktok.service /etc/systemd/system/
sudo systemctl enable --now 305-tiktok
```
