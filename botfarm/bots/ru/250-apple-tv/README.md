...
> Apple TV+ дешевле официальной подписки, выдача сразу

**ID:** `ru-250-apple-tv` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Apple TV+ Подписки — бот по теме «аккаунты Apple TV+». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/250-apple-tv/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 250-apple-tv.service /etc/systemd/system/
sudo systemctl enable --now 250-apple-tv
```
