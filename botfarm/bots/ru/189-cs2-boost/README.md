...
> Апгрейд ранга с тренером

**ID:** `ru-189-cs2-boost` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** standard

CS2 Прокачка — бот по теме «CS2». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9188
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/189-cs2-boost/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 189-cs2-boost.service /etc/systemd/system/
sudo systemctl enable --now 189-cs2-boost
```
