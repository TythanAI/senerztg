...
> КБЖУ и меню под вашу цель

**ID:** `ru-055-nutrition-plan` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** standard

План Питания — бот по теме «питание». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9054
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/055-nutrition-plan/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 055-nutrition-plan.service /etc/systemd/system/
sudo systemctl enable --now 055-nutrition-plan
```
