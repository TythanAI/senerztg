...
> NordVPN: быстрая выдача, замена при проблемах

**ID:** `ru-362-nordvpn` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

NordVPN Доступ — бот по теме «аккаунты NordVPN». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9361
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/362-nordvpn/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 362-nordvpn.service /etc/systemd/system/
sudo systemctl enable --now 362-nordvpn
```
