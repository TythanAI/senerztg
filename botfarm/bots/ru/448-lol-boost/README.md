...
> LoL Буст: результат под ключ, оплата после согласования

**ID:** `ru-448-lol-boost` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** standard

LoL Буст — бот по теме «буст ранга League of Legends». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9447
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/448-lol-boost/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 448-lol-boost.service /etc/systemd/system/
sudo systemctl enable --now 448-lol-boost
```
