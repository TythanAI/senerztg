...
> Bluesky: отлежавшиеся аккаунты с гарантией

**ID:** `ru-321-bluesky` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Bluesky Аккаунты — бот по теме «аккаунты Bluesky». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9320
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/321-bluesky/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 321-bluesky.service /etc/systemd/system/
sudo systemctl enable --now 321-bluesky
```
