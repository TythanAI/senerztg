...
> Luma AI без карты и VPN — доступ за минуту

**ID:** `ru-294-luma-ai` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Luma AI Доступ — бот по теме «аккаунты Luma». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9293
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/294-luma-ai/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 294-luma-ai.service /etc/systemd/system/
sudo systemctl enable --now 294-luma-ai
```
