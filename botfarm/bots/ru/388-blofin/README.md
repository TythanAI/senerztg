...
> BloFin: верифицированные аккаунты под задачи

**ID:** `ru-388-blofin` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** premium

BloFin Аккаунты — бот по теме «аккаунты BloFin». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9387
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/388-blofin/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 388-blofin.service /etc/systemd/system/
sudo systemctl enable --now 388-blofin
```
