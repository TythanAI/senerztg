...
> Звук дешевле официальной подписки, выдача сразу

**ID:** `ru-263-zvuk` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Звук Подписки — бот по теме «подписки Звук». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/263-zvuk/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 263-zvuk.service /etc/systemd/system/
sudo systemctl enable --now 263-zvuk
```
