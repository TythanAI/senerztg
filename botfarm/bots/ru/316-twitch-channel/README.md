...
> Twitch: отлежавшиеся аккаунты с гарантией

**ID:** `ru-316-twitch-channel` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Twitch Аккаунты — бот по теме «каналы Twitch». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9315
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/316-twitch-channel/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 316-twitch-channel.service /etc/systemd/system/
sudo systemctl enable --now 316-twitch-channel
```
