...
> Яндекс Go: готовые аккаунты для работы

**ID:** `ru-400-yandex-go` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Яндекс Go Аккаунты — бот по теме «аккаунты Яндекс Go». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9399
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/400-yandex-go/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 400-yandex-go.service /etc/systemd/system/
sudo systemctl enable --now 400-yandex-go
```
