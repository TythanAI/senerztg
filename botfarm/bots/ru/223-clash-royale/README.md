# Clash Royale Аккаунты

> Clash Royale: моментальная выдача, гарантия и замена

**ID:** `ru-223-clash-royale` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Clash Royale Аккаунты — бот по теме «аккаунты Clash Royale». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9222
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/223-clash-royale/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 223-clash-royale.service /etc/systemd/system/
sudo systemctl enable --now 223-clash-royale
```
