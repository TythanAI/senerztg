...
> Разборы размещений и аллокации

**ID:** `ru-011-ipo-radar` · **Регион:** RU · **Язык:** ru ·
**Модель:** signals · **Тариф цен:** premium

IPO Радар — бот по теме «IPO». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9010
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/011-ipo-radar/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 011-ipo-radar.service /etc/systemd/system/
sudo systemctl enable --now 011-ipo-radar
```
