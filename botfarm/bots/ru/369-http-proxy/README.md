...
> HTTP-прокси: быстрая выдача, замена при проблемах

**ID:** `ru-369-http-proxy` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

HTTP-прокси Доступ — бот по теме «HTTP-прокси». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9368
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/369-http-proxy/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 369-http-proxy.service /etc/systemd/system/
sudo systemctl enable --now 369-http-proxy
```
