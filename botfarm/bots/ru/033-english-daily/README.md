...
> 10 минут в день до уровня B2

**ID:** `ru-033-english-daily` · **Регион:** RU · **Язык:** ru ·
**Модель:** club · **Тариф цен:** budget

Английский Каждый День — бот по теме «английский язык». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9032
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/033-english-daily/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 033-english-daily.service /etc/systemd/system/
sudo systemctl enable --now 033-english-daily
```
