...
> Minecraft: моментальная выдача, гарантия и замена

**ID:** `ru-215-minecraft` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Minecraft Аккаунты — бот по теме «аккаунты Minecraft». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/215-minecraft/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 215-minecraft.service /etc/systemd/system/
sudo systemctl enable --now 215-minecraft
```
