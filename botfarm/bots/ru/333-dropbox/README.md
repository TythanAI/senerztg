...
> Dropbox: чистые аккаунты для работы и регистраций

**ID:** `ru-333-dropbox` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Dropbox Аккаунты — бот по теме «аккаунты Dropbox». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9332
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/333-dropbox/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 333-dropbox.service /etc/systemd/system/
sudo systemctl enable --now 333-dropbox
```
