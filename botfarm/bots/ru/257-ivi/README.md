...
> IVI дешевле официальной подписки, выдача сразу

**ID:** `ru-257-ivi` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

IVI Подписки — бот по теме «подписки IVI». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/257-ivi/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 257-ivi.service /etc/systemd/system/
sudo systemctl enable --now 257-ivi
```
