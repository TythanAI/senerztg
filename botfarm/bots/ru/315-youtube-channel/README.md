...
> YouTube: отлежавшиеся аккаунты с гарантией

**ID:** `ru-315-youtube-channel` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

YouTube Аккаунты — бот по теме «аккаунты YouTube». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9314
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/315-youtube-channel/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 315-youtube-channel.service /etc/systemd/system/
sudo systemctl enable --now 315-youtube-channel
```
