...
> Аудит полисов и подбор защиты

**ID:** `ru-018-insurance-check` · **Регион:** RU · **Язык:** ru ·
**Модель:** consult · **Тариф цен:** standard

Страховка Чек — бот по теме «страхование». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9017
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/018-insurance-check/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 018-insurance-check.service /etc/systemd/system/
sudo systemctl enable --now 018-insurance-check
```
