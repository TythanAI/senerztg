...
> Сайты на Заказ: делаем под ключ, сроки фиксируем

**ID:** `ru-498-site-dev-service` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** standard

Сайты на Заказ — бот по теме «разработка сайтов». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9497
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/498-site-dev-service/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 498-site-dev-service.service /etc/systemd/system/
sudo systemctl enable --now 498-site-dev-service
```
