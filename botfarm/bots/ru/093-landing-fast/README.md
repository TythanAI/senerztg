...
> Продающая страница за 72 часа

**ID:** `ru-093-landing-fast` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** premium

Лендинг Быстро — бот по теме «лендинги». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9092
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/093-landing-fast/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 093-landing-fast.service /etc/systemd/system/
sudo systemctl enable --now 093-landing-fast
```
