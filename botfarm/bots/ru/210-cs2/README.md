...
> CS2: моментальная выдача, гарантия и замена

**ID:** `ru-210-cs2` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

CS2 Аккаунты — бот по теме «аккаунты CS2». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/210-cs2/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 210-cs2.service /etc/systemd/system/
sudo systemctl enable --now 210-cs2
```
