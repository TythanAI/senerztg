...
> Каталоги и подбор по VIN

**ID:** `ru-157-car-parts` · **Регион:** RU · **Язык:** ru ·
**Модель:** shop · **Тариф цен:** standard

Автозапчасти Гид — бот по теме «автозапчасти». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9156
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/157-car-parts/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 157-car-parts.service /etc/systemd/system/
sudo systemctl enable --now 157-car-parts
```
