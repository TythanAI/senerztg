...
> Авито: готовые аккаунты для работы

**ID:** `ru-391-avito-acc` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Авито Аккаунты — бот по теме «аккаунты Авито». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9390
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/391-avito-acc/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 391-avito-acc.service /etc/systemd/system/
sudo systemctl enable --now 391-avito-acc
```
