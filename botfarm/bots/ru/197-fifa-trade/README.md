...
> Заработок на трансферном рынке

**ID:** `ru-197-fifa-trade` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** standard

FIFA Трейд — бот по теме «FIFA Ultimate Team». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9196
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/197-fifa-trade/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 197-fifa-trade.service /etc/systemd/system/
sudo systemctl enable --now 197-fifa-trade
```
