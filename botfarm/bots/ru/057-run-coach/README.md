...
> От нуля до полумарафона

**ID:** `ru-057-run-coach` · **Регион:** RU · **Язык:** ru ·
**Модель:** club · **Тариф цен:** standard

Беговой Тренер — бот по теме «бег». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9056
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/057-run-coach/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 057-run-coach.service /etc/systemd/system/
sudo systemctl enable --now 057-run-coach
```
