...
> Восстановление ресурса за 6 недель

**ID:** `ru-181-burnout-fix` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** standard

Выгорание Стоп — бот по теме «выгорание». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9180
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/181-burnout-fix/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 181-burnout-fix.service /etc/systemd/system/
sudo systemctl enable --now 181-burnout-fix
```
