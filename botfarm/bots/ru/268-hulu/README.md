...
> Hulu дешевле официальной подписки, выдача сразу

**ID:** `ru-268-hulu` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Hulu Подписки — бот по теме «аккаунты Hulu». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/268-hulu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 268-hulu.service /etc/systemd/system/
sudo systemctl enable --now 268-hulu
```
