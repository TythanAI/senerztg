...
**ID:** `ru-128-wedding-plan` · **Регион:** RU · **Язык:** ru ·
**Модель:** consult · **Тариф цен:** elite

Свадебный Организатор — бот по теме «организация свадеб». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9127
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/128-wedding-plan/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 128-wedding-plan.service /etc/systemd/system/
sudo systemctl enable --now 128-wedding-plan
```
