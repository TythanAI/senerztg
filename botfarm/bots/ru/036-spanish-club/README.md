...
**ID:** `ru-036-spanish-club` · **Регион:** RU · **Язык:** ru ·
**Модель:** club · **Тариф цен:** standard

Испанский Клуб — бот по теме «испанский язык». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9035
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/036-spanish-club/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 036-spanish-club.service /etc/systemd/system/
sudo systemctl enable --now 036-spanish-club
```
