...
> Учёт денег семьи без таблиц

**ID:** `ru-007-budget-family` · **Регион:** RU · **Язык:** ru ·
**Модель:** saas · **Тариф цен:** budget

Семейный Бюджет — бот по теме «семейный бюджет». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9006
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/007-budget-family/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 007-budget-family.service /etc/systemd/system/
sudo systemctl enable --now 007-budget-family
```
