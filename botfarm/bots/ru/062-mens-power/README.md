...
> Тестостерон, сила, выносливость

**ID:** `ru-062-mens-power` · **Регион:** RU · **Язык:** ru ·
**Модель:** club · **Тариф цен:** standard

Мужская Форма — бот по теме «мужское здоровье». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9061
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/062-mens-power/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 062-mens-power.service /etc/systemd/system/
sudo systemctl enable --now 062-mens-power
```
