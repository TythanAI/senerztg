...
> Medium: отлежавшиеся аккаунты с гарантией

**ID:** `ru-320-medium` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Medium Аккаунты — бот по теме «аккаунты Medium». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9319
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/320-medium/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 320-medium.service /etc/systemd/system/
sudo systemctl enable --now 320-medium
```
