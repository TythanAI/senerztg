...
> Ключи игр: ключи и коды с моментальной выдачей

**ID:** `ru-417-game-keys-shop` · **Регион:** RU · **Язык:** ru ·
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9416
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/417-game-keys-shop/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 417-game-keys-shop.service /etc/systemd/system/
sudo systemctl enable --now 417-game-keys-shop
```
