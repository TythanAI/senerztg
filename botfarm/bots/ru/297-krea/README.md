...
> Krea AI без карты и VPN — доступ за минуту

**ID:** `ru-297-krea` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Krea AI Доступ — бот по теме «аккаунты Krea». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9296
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/297-krea/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 297-krea.service /etc/systemd/system/
sudo systemctl enable --now 297-krea
```
