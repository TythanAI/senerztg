...
> MEXC: верифицированные аккаунты под задачи

**ID:** `ru-381-mexc-acc` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** premium

MEXC Аккаунты — бот по теме «аккаунты MEXC». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9380
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/381-mexc-acc/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 381-mexc-acc.service /etc/systemd/system/
sudo systemctl enable --now 381-mexc-acc
```
