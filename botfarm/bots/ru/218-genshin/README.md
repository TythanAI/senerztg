# Genshin Impact Аккаунты

> Genshin Impact: моментальная выдача, гарантия и замена

**ID:** `ru-218-genshin` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Genshin Impact Аккаунты — бот по теме «аккаунты Genshin Impact». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9217
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/218-genshin/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 218-genshin.service /etc/systemd/system/
sudo systemctl enable --now 218-genshin
```
