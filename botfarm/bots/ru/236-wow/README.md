# World of Warcraft Аккаунты

> World of Warcraft: моментальная выдача, гарантия и замена

**ID:** `ru-236-wow` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

World of Warcraft Аккаунты — бот по теме «аккаунты WoW». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9235
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/236-wow/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 236-wow.service /etc/systemd/system/
sudo systemctl enable --now 236-wow
```
