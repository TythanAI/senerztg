...
> Tarkov Буст: результат под ключ, оплата после согласования

**ID:** `ru-451-tarkov-boost` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** standard

Tarkov Буст — бот по теме «прокачка в Escape from Tarkov». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9450
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/451-tarkov-boost/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 451-tarkov-boost.service /etc/systemd/system/
sudo systemctl enable --now 451-tarkov-boost
```
