...
> Покупка и эксплуатация в России

**ID:** `ru-162-evcar-guide` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** standard

Электромобиль Гид — бот по теме «электромобили». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9161
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/162-evcar-guide/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 162-evcar-guide.service /etc/systemd/system/
sudo systemctl enable --now 162-evcar-guide
```
