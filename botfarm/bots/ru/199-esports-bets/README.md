# Киберспорт Аналитика

> Разборы матчей и статистика

**ID:** `ru-199-esports-bets` · **Регион:** RU · **Язык:** ru ·
**Модель:** signals · **Тариф цен:** premium

Киберспорт Аналитика — бот по теме «киберспортивная аналитика». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9198
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/199-esports-bets/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 199-esports-bets.service /etc/systemd/system/
sudo systemctl enable --now 199-esports-bets
```
