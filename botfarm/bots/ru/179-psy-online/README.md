...
> Сессии в удобное время

**ID:** `ru-179-psy-online` · **Регион:** RU · **Язык:** ru ·
**Модель:** consult · **Тариф цен:** premium

Психолог Онлайн — бот по теме «психотерапия». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9178
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/179-psy-online/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 179-psy-online.service /etc/systemd/system/
sudo systemctl enable --now 179-psy-online
```
