...
> Первый сезон без падений

**ID:** `ru-161-moto-school` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** standard

Мотошкола — бот по теме «мотоциклы». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9160
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/161-moto-school/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 161-moto-school.service /etc/systemd/system/
sudo systemctl enable --now 161-moto-school
```
