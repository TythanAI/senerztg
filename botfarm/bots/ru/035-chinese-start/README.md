...
> HSK 1-3 за четыре месяца

**ID:** `ru-035-chinese-start` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** standard

Китайский Старт — бот по теме «китайский язык». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9034
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/035-chinese-start/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 035-chinese-start.service /etc/systemd/system/
sudo systemctl enable --now 035-chinese-start
```
