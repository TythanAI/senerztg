# Виртуальные номера Доступ

> Виртуальные номера: быстрая выдача, замена при проблемах

**ID:** `ru-376-sms-numbers` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Виртуальные номера Доступ — бот по теме «виртуальные номера для SMS». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9375
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/376-sms-numbers/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 376-sms-numbers.service /etc/systemd/system/
sudo systemctl enable --now 376-sms-numbers
```
