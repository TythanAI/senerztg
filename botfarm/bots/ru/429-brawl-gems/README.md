...
> Brawl Stars: пополнение по выгодному курсу за 15 минут

**ID:** `ru-429-brawl-gems` · **Регион:** RU · **Язык:** ru ·
**Модель:** topup · **Тариф цен:** budget

Brawl Stars — бот по теме «гемы Brawl Stars». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9428
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/429-brawl-gems/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 429-brawl-gems.service /etc/systemd/system/
sudo systemctl enable --now 429-brawl-gems
```
