# Envato Elements Доступ

> Envato Elements по цене в разы ниже подписки

**ID:** `ru-354-envato` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Envato Elements Доступ — бот по теме «аккаунты Envato». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9353
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/354-envato/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 354-envato.service /etc/systemd/system/
sudo systemctl enable --now 354-envato
```
