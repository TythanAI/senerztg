...
> Snapchat: отлежавшиеся аккаунты с гарантией

**ID:** `ru-312-snapchat` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Snapchat Аккаунты — бот по теме «аккаунты Snapchat». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9311
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/312-snapchat/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 312-snapchat.service /etc/systemd/system/
sudo systemctl enable --now 312-snapchat
```
