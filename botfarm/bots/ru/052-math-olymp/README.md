...
> Задачи всероса с разборами

**ID:** `ru-052-math-olymp` · **Регион:** RU · **Язык:** ru ·
**Модель:** club · **Тариф цен:** standard

Олимпиадная Математика — бот по теме «олимпиадная математика». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9051
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/052-math-olymp/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 052-math-olymp.service /etc/systemd/system/
sudo systemctl enable --now 052-math-olymp
```
