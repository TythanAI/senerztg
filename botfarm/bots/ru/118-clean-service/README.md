...
**ID:** `ru-118-clean-service` · **Регион:** RU · **Язык:** ru ·
**Модель:** leadgen · **Тариф цен:** standard

Клининг Заказ — бот по теме «клининг». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9117
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/118-clean-service/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 118-clean-service.service /etc/systemd/system/
sudo systemctl enable --now 118-clean-service
```
