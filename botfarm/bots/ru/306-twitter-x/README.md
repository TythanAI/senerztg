...
> X (Twitter): отлежавшиеся аккаунты с гарантией

**ID:** `ru-306-twitter-x` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

X (Twitter) Аккаунты — бот по теме «аккаунты X». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9305
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/306-twitter-x/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 306-twitter-x.service /etc/systemd/system/
sudo systemctl enable --now 306-twitter-x
```
