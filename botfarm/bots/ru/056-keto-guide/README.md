...
> Кето без ошибок и срывов

**ID:** `ru-056-keto-guide` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** standard

Кето Гид — бот по теме «кето-диета». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9055
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/056-keto-guide/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 056-keto-guide.service /etc/systemd/system/
sudo systemctl enable --now 056-keto-guide
```
