...
> Documents и запись в консульство

**ID:** `ru-148-visa-help` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** premium

Виза Помощь — бот по теме «визы». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9147
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/148-visa-help/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 148-visa-help.service /etc/systemd/system/
sudo systemctl enable --now 148-visa-help
```
