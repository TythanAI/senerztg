...
> Sneaker Монитор: уведомления в этот чат в реальном времени

**ID:** `ru-472-sneaker-monitor` · **Регион:** RU · **Язык:** ru ·
**Модель:** monitor · **Тариф цен:** standard

Sneaker Монитор — бот по теме «мониторинг дропов кроссовок». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9471
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/472-sneaker-monitor/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 472-sneaker-monitor.service /etc/systemd/system/
sudo systemctl enable --now 472-sneaker-monitor
```
