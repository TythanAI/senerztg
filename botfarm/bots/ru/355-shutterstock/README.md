...
> Shutterstock по цене в разы ниже подписки

**ID:** `ru-355-shutterstock` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Shutterstock Доступ — бот по теме «аккаунты Shutterstock». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9354
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/355-shutterstock/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 355-shutterstock.service /etc/systemd/system/
sudo systemctl enable --now 355-shutterstock
```
