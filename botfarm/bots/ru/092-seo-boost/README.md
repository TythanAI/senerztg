...
**ID:** `ru-092-seo-boost` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** elite

SEO Рост — бот по теме «SEO». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9091
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/092-seo-boost/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 092-seo-boost.service /etc/systemd/system/
sudo systemctl enable --now 092-seo-boost
```
