...
> Читать втрое быстрее с пониманием

**ID:** `ru-041-speed-read` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** budget

Скорочтение — бот по теме «скорочтение». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9040
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/041-speed-read/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 041-speed-read.service /etc/systemd/system/
sudo systemctl enable --now 041-speed-read
```
