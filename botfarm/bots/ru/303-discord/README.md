...
> Discord: отлежавшиеся аккаунты с гарантией

**ID:** `ru-303-discord` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Discord Аккаунты — бот по теме «аккаунты Discord». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9302
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/303-discord/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 303-discord.service /etc/systemd/system/
sudo systemctl enable --now 303-discord
```
