...
> Zoom по цене в разы ниже подписки

**ID:** `ru-350-zoom` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Zoom Доступ — бот по теме «аккаунты Zoom». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9349
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/350-zoom/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 350-zoom.service /etc/systemd/system/
sudo systemctl enable --now 350-zoom
```
