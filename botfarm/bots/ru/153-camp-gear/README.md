...
> Списки и обзоры для походов

**ID:** `ru-153-camp-gear` · **Регион:** RU · **Язык:** ru ·
**Модель:** shop · **Тариф цен:** budget

Походное Снаряжение — бот по теме «походы». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9152
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/153-camp-gear/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 153-camp-gear.service /etc/systemd/system/
sudo systemctl enable --now 153-camp-gear
```
