...
> Поднимаем скоринг перед ипотекой

**ID:** `ru-010-credit-score` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** standard

Кредитный Рейтинг — бот по теме «кредитная история». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9009
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/010-credit-score/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 010-credit-score.service /etc/systemd/system/
sudo systemctl enable --now 010-credit-score
```
