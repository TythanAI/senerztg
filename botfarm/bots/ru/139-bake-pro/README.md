...
> Торты на заказ как бизнес

**ID:** `ru-139-bake-pro` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** premium

Кондитер Про — бот по теме «кондитерское дело». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9138
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/139-bake-pro/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 139-bake-pro.service /etc/systemd/system/
sudo systemctl enable --now 139-bake-pro
```
