...
> Скидки и ошибки бронирования

**ID:** `ru-151-hotel-deals` · **Регион:** RU · **Язык:** ru ·
**Модель:** signals · **Тариф цен:** budget

Отели Выгодно — бот по теме «отели». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9150
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/151-hotel-deals/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 151-hotel-deals.service /etc/systemd/system/
sudo systemctl enable --now 151-hotel-deals
```
