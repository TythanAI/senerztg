...
> Боты на Заказ: делаем под ключ, сроки фиксируем

**ID:** `ru-497-bot-dev-service` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** standard

Боты на Заказ — бот по теме «разработка Telegram-ботов». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9496
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/497-bot-dev-service/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 497-bot-dev-service.service /etc/systemd/system/
sudo systemctl enable --now 497-bot-dev-service
```
