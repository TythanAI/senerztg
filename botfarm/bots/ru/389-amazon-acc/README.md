...
> Amazon: готовые аккаунты для работы

**ID:** `ru-389-amazon-acc` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Amazon Аккаунты — бот по теме «аккаунты Amazon». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9388
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/389-amazon-acc/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 389-amazon-acc.service /etc/systemd/system/
sudo systemctl enable --now 389-amazon-acc
```
