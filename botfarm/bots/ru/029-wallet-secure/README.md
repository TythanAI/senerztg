...
> Как не потерять крипту: безопасность

**ID:** `ru-029-wallet-secure` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** budget

Кошелёк Защита — бот по теме «безопасность криптокошельков». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9028
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/029-wallet-secure/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 029-wallet-secure.service /etc/systemd/system/
sudo systemctl enable --now 029-wallet-secure
```
