...
> Кухни и шкафы по размерам

**ID:** `ru-122-furniture-make` · **Регион:** RU · **Язык:** ru ·
**Модель:** leadgen · **Тариф цен:** premium

Мебель на Заказ — бот по теме «мебель на заказ». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9121

## Каталог

| SKU | Название | Цена | Тип |
|-----|----------|------|-----|
| `audit` | Выезд и замер | 1 710 ₽ | consult |
| `package` | Стандартный заказ | 7 830 ₽ | service |
| `turnkey` | Под ключ | 19 590 ₽ | service |
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/122-furniture-make/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 122-furniture-make.service /etc/systemd/system/
sudo systemctl enable --now 122-furniture-make
```
