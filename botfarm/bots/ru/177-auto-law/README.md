...
> ДТП, страховая, лишение прав

**ID:** `ru-177-auto-law` · **Регион:** RU · **Язык:** ru ·
**Модель:** consult · **Тариф цен:** premium

Автоюрист — бот по теме «автоправо». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9176
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/177-auto-law/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 177-auto-law.service /etc/systemd/system/
sudo systemctl enable --now 177-auto-law
```
