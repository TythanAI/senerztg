...
> Правки и риски за 24 часа

**ID:** `ru-178-contract-check` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** standard

Проверка Договора — бот по теме «проверка договоров». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9177
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/178-contract-check/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 178-contract-check.service /etc/systemd/system/
sudo systemctl enable --now 178-contract-check
```
