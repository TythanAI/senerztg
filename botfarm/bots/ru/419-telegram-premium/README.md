...
> Telegram Premium: ключи и коды с моментальной выдачей

**ID:** `ru-419-telegram-premium` · **Регион:** RU · **Язык:** ru ·
**Модель:** keys · **Тариф цен:** standard

Telegram Premium — бот по теме «Telegram Premium». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9418

## Каталог

| SKU | Название | Цена | Тип |
|-----|----------|------|-----|
| `key1` | 1 ключ | 1 480 ₽ | account |
| `key3` | 3 ключа | 4 010 ₽ | account |
| `key10` | 10 ключей | 11 910 ₽ | account |
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/419-telegram-premium/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 419-telegram-premium.service /etc/systemd/system/
sudo systemctl enable --now 419-telegram-premium
```
