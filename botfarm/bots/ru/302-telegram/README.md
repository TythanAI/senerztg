...
> Telegram: отлежавшиеся аккаунты с гарантией

**ID:** `ru-302-telegram` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Telegram Аккаунты — бот по теме «аккаунты Telegram». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9301
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/302-telegram/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 302-telegram.service /etc/systemd/system/
sudo systemctl enable --now 302-telegram
```
