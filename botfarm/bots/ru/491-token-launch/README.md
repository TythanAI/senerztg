...
> Запуск Токена: быстро, по курсу, без лишних вопросов

**ID:** `ru-491-token-launch` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** premium

Запуск Токена — бот по теме «запуск токена». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9490
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/491-token-launch/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 491-token-launch.service /etc/systemd/system/
sudo systemctl enable --now 491-token-launch
```
