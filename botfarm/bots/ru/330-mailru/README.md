...
> Mail.ru: чистые аккаунты для работы и регистраций

**ID:** `ru-330-mailru` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Mail.ru Аккаунты — бот по теме «аккаунты Mail.ru». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9329
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/330-mailru/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 330-mailru.service /etc/systemd/system/
sudo systemctl enable --now 330-mailru
```
