...
> Крипта за Наличные: быстро, по курсу, без лишних вопросов

**ID:** `ru-484-cash-crypto` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** premium

Крипта за Наличные — бот по теме «обмен крипты на наличные». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9483
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/484-cash-crypto/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 484-cash-crypto.service /etc/systemd/system/
sudo systemctl enable --now 484-cash-crypto
```
