...
> Переводы: делаем под ключ, сроки фиксируем

**ID:** `ru-495-translate-service` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** standard

Переводы — бот по теме «перевод текстов». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9494
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/495-translate-service/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 495-translate-service.service /etc/systemd/system/
sudo systemctl enable --now 495-translate-service
```
