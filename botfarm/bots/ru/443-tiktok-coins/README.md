...
> TikTok: пополнение по выгодному курсу за 15 минут

**ID:** `ru-443-tiktok-coins` · **Регион:** RU · **Язык:** ru ·
**Модель:** topup · **Тариф цен:** budget

TikTok — бот по теме «монеты TikTok». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9442
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/443-tiktok-coins/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 443-tiktok-coins.service /etc/systemd/system/
sudo systemctl enable --now 443-tiktok-coins
```
