...
> Kwai: отлежавшиеся аккаунты с гарантией

**ID:** `ru-325-kwai` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Kwai Аккаунты — бот по теме «аккаунты Kwai». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9324
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/325-kwai/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 325-kwai.service /etc/systemd/system/
sudo systemctl enable --now 325-kwai
```
