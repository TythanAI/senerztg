...
> Telegram-боты как источник дохода

**ID:** `ru-107-bot-dev` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** premium

Разработка Ботов — бот по теме «разработка ботов». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9106
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/107-bot-dev/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 107-bot-dev.service /etc/systemd/system/
sudo systemctl enable --now 107-bot-dev
```
