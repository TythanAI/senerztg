# Final Fantasy XIV Аккаунты

> Final Fantasy XIV: моментальная выдача, гарантия и замена

**ID:** `ru-237-ffxiv` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Final Fantasy XIV Аккаунты — бот по теме «аккаунты FFXIV». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9236
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/237-ffxiv/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 237-ffxiv.service /etc/systemd/system/
sudo systemctl enable --now 237-ffxiv
```
