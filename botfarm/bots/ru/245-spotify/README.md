...
> Spotify дешевле официальной подписки, выдача сразу

**ID:** `ru-245-spotify` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Spotify Подписки — бот по теме «аккаунты Spotify». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9244
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/245-spotify/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 245-spotify.service /etc/systemd/system/
sudo systemctl enable --now 245-spotify
```
