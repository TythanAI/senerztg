...
> Пассивный портфель на фондах

**ID:** `ru-012-etf-portfolio` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** standard

ETF Портфель — бот по теме «ETF». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9011
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/012-etf-portfolio/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 012-etf-portfolio.service /etc/systemd/system/
sudo systemctl enable --now 012-etf-portfolio
```
