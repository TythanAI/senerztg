...
**ID:** `ru-125-pet-care` · **Регион:** RU · **Язык:** ru ·
**Модель:** leadgen · **Тариф цен:** budget

Передержка Питомцев — бот по теме «уход за питомцами». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9124
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/125-pet-care/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 125-pet-care.service /etc/systemd/system/
sudo systemctl enable --now 125-pet-care
```
