...
> Аукцион Монитор: уведомления в этот чат в реальном времени

**ID:** `ru-479-auction-monitor` · **Регион:** RU · **Язык:** ru ·
**Модель:** monitor · **Тариф цен:** standard

Аукцион Монитор — бот по теме «мониторинг аукционов». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9478
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/479-auction-monitor/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 479-auction-monitor.service /etc/systemd/system/
sudo systemctl enable --now 479-auction-monitor
```
