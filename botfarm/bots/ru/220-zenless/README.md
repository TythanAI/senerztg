# Zenless Zone Zero Аккаунты

> Zenless Zone Zero: моментальная выдача, гарантия и замена

**ID:** `ru-220-zenless` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Zenless Zone Zero Аккаунты — бот по теме «аккаунты Zenless». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9219
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/220-zenless/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 220-zenless.service /etc/systemd/system/
sudo systemctl enable --now 220-zenless
```
