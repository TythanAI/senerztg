...
> Первые песни за 30 дней

**ID:** `ru-048-guitar-start` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** budget

Гитара Старт — бот по теме «игра на гитаре». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9047
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/048-guitar-start/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 048-guitar-start.service /etc/systemd/system/
sudo systemctl enable --now 048-guitar-start
```
