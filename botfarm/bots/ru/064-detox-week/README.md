...
> Мягкая перезагрузка организма

**ID:** `ru-064-detox-week` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** budget

Детокс Неделя — бот по теме «детокс». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9063
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/064-detox-week/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 064-detox-week.service /etc/systemd/system/
sudo systemctl enable --now 064-detox-week
```
