...
> Личный финансовый план за одну встречу

**ID:** `ru-005-finplan-pro` · **Регион:** RU · **Язык:** ru ·
**Модель:** consult · **Тариф цен:** premium

ФинПлан Про — бот по теме «финансовое планирование». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9004
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/005-finplan-pro/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 005-finplan-pro.service /etc/systemd/system/
sudo systemctl enable --now 005-finplan-pro
```
