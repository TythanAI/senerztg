...
> Курсовые и дипломы под ключ

**ID:** `ru-043-essay-help` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** premium

Помощь с Работой — бот по теме «учебные работы». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9042
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/043-essay-help/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 043-essay-help.service /etc/systemd/system/
sudo systemctl enable --now 043-essay-help
```
