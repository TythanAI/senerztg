...
> Аудит Контрактов: быстро, по курсу, без лишних вопросов

**ID:** `ru-490-smart-audit` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** premium

Аудит Контрактов — бот по теме «аудит смарт-контрактов». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9489
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/490-smart-audit/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 490-smart-audit.service /etc/systemd/system/
sudo systemctl enable --now 490-smart-audit
```
