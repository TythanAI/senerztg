...
> EA Play: ключи и коды с моментальной выдачей

**ID:** `ru-422-ea-play` · **Регион:** RU · **Язык:** ru ·
**Модель:** keys · **Тариф цен:** standard

EA Play — бот по теме «подписки EA Play». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9421
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/422-ea-play/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 422-ea-play.service /etc/systemd/system/
sudo systemctl enable --now 422-ea-play
```
