...
> Notion AI без карты и VPN — доступ за минуту

**ID:** `ru-291-notion-ai` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Notion AI Доступ — бот по теме «аккаунты Notion AI». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9290
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/291-notion-ai/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 291-notion-ai.service /etc/systemd/system/
sudo systemctl enable --now 291-notion-ai
```
