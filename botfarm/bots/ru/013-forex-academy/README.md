...
> Валютный рынок: система, а не казино

**ID:** `ru-013-forex-academy` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** premium

Форекс Академия — бот по теме «форекс». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9012
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/013-forex-academy/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 013-forex-academy.service /etc/systemd/system/
sudo systemctl enable --now 013-forex-academy
```
