...
> Rust: моментальная выдача, гарантия и замена

**ID:** `ru-230-rust` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Rust Аккаунты — бот по теме «аккаунты Rust». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/230-rust/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 230-rust.service /etc/systemd/system/
sudo systemctl enable --now 230-rust
```
