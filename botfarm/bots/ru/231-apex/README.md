# Apex Legends Аккаунты

> Apex Legends: моментальная выдача, гарантия и замена

**ID:** `ru-231-apex` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Apex Legends Аккаунты — бот по теме «аккаунты Apex Legends». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9230
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/231-apex/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 231-apex.service /etc/systemd/system/
sudo systemctl enable --now 231-apex
```
