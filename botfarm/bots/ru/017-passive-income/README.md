...
> Пять потоков дохода за год

**ID:** `ru-017-passive-income` · **Регион:** RU · **Язык:** ru ·
**Модель:** club · **Тариф цен:** standard

Пассивный Доход — бот по теме «пассивный доход». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9016
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/017-passive-income/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 017-passive-income.service /etc/systemd/system/
sudo systemctl enable --now 017-passive-income
```
