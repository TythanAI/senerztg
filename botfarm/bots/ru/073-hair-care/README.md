...
> Восстановление после окрашивания

**ID:** `ru-073-hair-care` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** budget

Уход за Волосами — бот по теме «уход за волосами». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9072
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/073-hair-care/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 073-hair-care.service /etc/systemd/system/
sudo systemctl enable --now 073-hair-care
```
