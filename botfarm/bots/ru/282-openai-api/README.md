...
> OpenAI API без карты и VPN — доступ за минуту

**ID:** `ru-282-openai-api` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

OpenAI API Доступ — бот по теме «доступ к OpenAI API». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9281
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/282-openai-api/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 282-openai-api.service /etc/systemd/system/
sudo systemctl enable --now 282-openai-api
```
