...
> OneDrive: чистые аккаунты для работы и регистраций

**ID:** `ru-336-onedrive` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

OneDrive Аккаунты — бот по теме «аккаунты OneDrive». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9335
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/336-onedrive/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 336-onedrive.service /etc/systemd/system/
sudo systemctl enable --now 336-onedrive
```
