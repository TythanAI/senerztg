...
> Instagram: отлежавшиеся аккаунты с гарантией

**ID:** `ru-304-instagram` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Instagram Аккаунты — бот по теме «аккаунты Instagram». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9303
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/304-instagram/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 304-instagram.service /etc/systemd/system/
sudo systemctl enable --now 304-instagram
```
