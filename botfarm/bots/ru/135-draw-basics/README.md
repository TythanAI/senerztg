...
**ID:** `ru-135-draw-basics` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** budget

Рисование с Нуля — бот по теме «рисование». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9134
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/135-draw-basics/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 135-draw-basics.service /etc/systemd/system/
sudo systemctl enable --now 135-draw-basics
```
