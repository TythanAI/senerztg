...
> Билды и тактика World of Tanks

**ID:** `ru-191-wot-guide` · **Регион:** RU · **Язык:** ru ·
**Модель:** club · **Тариф цен:** budget

Танки Гайды — бот по теме «World of Tanks». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9190

## Каталог

| SKU | Название | Цена | Тип |
|-----|----------|------|-----|
| `month` | Месяц доступа | 480 ₽ | subscription · 30 дн. |
| `quarter` | Три месяца | 1 260 ₽ | subscription · 90 дн. |
| `year` | Год доступа | 3 910 ₽ | subscription · 365 дн. |
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/191-wot-guide/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 191-wot-guide.service /etc/systemd/system/
sudo systemctl enable --now 191-wot-guide
```
