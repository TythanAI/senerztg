...
**ID:** `ru-154-city-guides` · **Регион:** RU · **Язык:** ru ·
**Модель:** shop · **Тариф цен:** budget

Гиды по Городам — бот по теме «городские гиды». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9153
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/154-city-guides/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 154-city-guides.service /etc/systemd/system/
sudo systemctl enable --now 154-city-guides
```
