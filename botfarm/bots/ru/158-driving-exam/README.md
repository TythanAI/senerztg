...
> Теория и площадка с первого раза

**ID:** `ru-158-driving-exam` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** budget

Экзамен ГИБДД — бот по теме «экзамен в ГИБДД». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9157
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/158-driving-exam/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 158-driving-exam.service /etc/systemd/system/
sudo systemctl enable --now 158-driving-exam
```
