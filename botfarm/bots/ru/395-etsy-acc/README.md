...
> Etsy: готовые аккаунты для работы

**ID:** `ru-395-etsy-acc` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Etsy Аккаунты — бот по теме «аккаунты Etsy». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9394
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/395-etsy-acc/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 395-etsy-acc.service /etc/systemd/system/
sudo systemctl enable --now 395-etsy-acc
```
