...
> Skillbox по цене в разы ниже подписки

**ID:** `ru-359-skillbox` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Skillbox Доступ — бот по теме «доступы Skillbox». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9358
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/359-skillbox/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 359-skillbox.service /etc/systemd/system/
sudo systemctl enable --now 359-skillbox
```
