...
> Крипто Алерты: уведомления в этот чат в реальном времени

**ID:** `ru-466-crypto-alerts` · **Регион:** RU · **Язык:** ru ·
**Модель:** monitor · **Тариф цен:** standard

Крипто Алерты — бот по теме «оповещения о курсах криптовалют». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9465
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/466-crypto-alerts/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 466-crypto-alerts.service /etc/systemd/system/
sudo systemctl enable --now 466-crypto-alerts
```
