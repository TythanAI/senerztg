...
**ID:** `ru-165-rent-flow` · **Регион:** RU · **Язык:** ru ·
**Модель:** saas · **Тариф цен:** standard

Аренда Поток — бот по теме «управление арендой». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9164
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/165-rent-flow/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 165-rent-flow.service /etc/systemd/system/
sudo systemctl enable --now 165-rent-flow
```
