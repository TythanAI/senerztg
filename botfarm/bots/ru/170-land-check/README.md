...
**ID:** `ru-170-land-check` · **Регион:** RU · **Язык:** ru ·
**Модель:** consult · **Тариф цен:** premium

Проверка Участка — бот по теме «земельные участки». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9169
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/170-land-check/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 170-land-check.service /etc/systemd/system/
sudo systemctl enable --now 170-land-check
```
