...
> Старты продаж и скидки

**ID:** `ru-166-newbuild-scan` · **Регион:** RU · **Язык:** ru ·
**Модель:** signals · **Тариф цен:** standard

Новостройки Скан — бот по теме «новостройки». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9165
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/166-newbuild-scan/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 166-newbuild-scan.service /etc/systemd/system/
sudo systemctl enable --now 166-newbuild-scan
```
