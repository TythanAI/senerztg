# Amazon Prime Подписки

> Amazon Prime дешевле официальной подписки, выдача сразу

**ID:** `ru-251-amazon-prime` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Amazon Prime Подписки — бот по теме «аккаунты Amazon Prime». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/251-amazon-prime/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 251-amazon-prime.service /etc/systemd/system/
sudo systemctl enable --now 251-amazon-prime
```
