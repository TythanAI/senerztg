...
**ID:** `ru-130-laundry-pick` · **Регион:** RU · **Язык:** ru ·
**Модель:** leadgen · **Тариф цен:** budget

Химчистка Доставка — бот по теме «химчистка». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9129

## Каталог

| SKU | Название | Цена | Тип |
|-----|----------|------|-----|
| `audit` | Выезд и замер | 160 ₽ | consult |
| `package` | Стандартный заказ | 770 ₽ | service |
| `turnkey` | Под ключ | 1 950 ₽ | service |
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/130-laundry-pick/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 130-laundry-pick.service /etc/systemd/system/
sudo systemctl enable --now 130-laundry-pick
```
