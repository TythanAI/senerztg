...
**ID:** `ru-065-mama-fit` · **Регион:** RU · **Язык:** ru ·
**Модель:** club · **Тариф цен:** standard

Мама в Форме — бот по теме «фитнес для мам». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9064
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/065-mama-fit/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 065-mama-fit.service /etc/systemd/system/
sudo systemctl enable --now 065-mama-fit
```
