...
> Декларация по криптоактивам

**ID:** `ru-028-crypto-tax-ru` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** premium

Крипта Налог — бот по теме «налоги на крипту». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9027
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/028-crypto-tax-ru/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 028-crypto-tax-ru.service /etc/systemd/system/
sudo systemctl enable --now 028-crypto-tax-ru
```
