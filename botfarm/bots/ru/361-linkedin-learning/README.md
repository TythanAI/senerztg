# LinkedIn Learning Доступ

> LinkedIn Learning по цене в разы ниже подписки

**ID:** `ru-361-linkedin-learning` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

LinkedIn Learning Доступ — бот по теме «аккаунты LinkedIn Learning». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9360

## Каталог

| SKU | Название | Цена | Тип |
|-----|----------|------|-----|
| `basic` | Базовый | 1 480 ₽ | account |
| `standard` | Стандарт | 2 670 ₽ | account |
| `premium` | Премиум | 5 060 ₽ | account |
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/361-linkedin-learning/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 361-linkedin-learning.service /etc/systemd/system/
sudo systemctl enable --now 361-linkedin-learning
```
