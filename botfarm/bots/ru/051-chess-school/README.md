...
> Разборы партий и дебютный репертуар

**ID:** `ru-051-chess-school` · **Регион:** RU · **Язык:** ru ·
**Модель:** club · **Тариф цен:** standard

Шахматная Школа — бот по теме «шахматы». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9050
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/051-chess-school/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 051-chess-school.service /etc/systemd/system/
sudo systemctl enable --now 051-chess-school
```
