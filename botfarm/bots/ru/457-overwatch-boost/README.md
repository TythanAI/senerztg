...
> Overwatch Буст: результат под ключ, оплата после согласования

**ID:** `ru-457-overwatch-boost` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** standard

Overwatch Буст — бот по теме «буст ранга Overwatch». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9456
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/457-overwatch-boost/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 457-overwatch-boost.service /etc/systemd/system/
sudo systemctl enable --now 457-overwatch-boost
```
