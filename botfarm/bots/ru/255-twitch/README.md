...
> Twitch дешевле официальной подписки, выдача сразу

**ID:** `ru-255-twitch` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Twitch Подписки — бот по теме «аккаунты Twitch». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9254
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/255-twitch/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 255-twitch.service /etc/systemd/system/
sudo systemctl enable --now 255-twitch
```
