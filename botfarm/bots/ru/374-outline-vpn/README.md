...
> Outline: быстрая выдача, замена при проблемах

**ID:** `ru-374-outline-vpn` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Outline Доступ — бот по теме «ключи Outline». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9373
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/374-outline-vpn/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 374-outline-vpn.service /etc/systemd/system/
sudo systemctl enable --now 374-outline-vpn
```
