...
> Литрес дешевле официальной подписки, выдача сразу

**ID:** `ru-271-litres` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Литрес Подписки — бот по теме «подписки Литрес». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/271-litres/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 271-litres.service /etc/systemd/system/
sudo systemctl enable --now 271-litres
```
