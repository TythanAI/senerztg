...
> Gate.io: верифицированные аккаунты под задачи

**ID:** `ru-384-gate-io` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** premium

Gate.io Аккаунты — бот по теме «аккаунты Gate.io». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9383
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/384-gate-io/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 384-gate-io.service /etc/systemd/system/
sudo systemctl enable --now 384-gate-io
```
