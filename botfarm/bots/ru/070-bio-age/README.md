...
> Расшифровка анализов и план

**ID:** `ru-070-bio-age` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** elite

Биовозраст — бот по теме «биохакинг». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9069
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/070-bio-age/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 070-bio-age.service /etc/systemd/system/
sudo systemctl enable --now 070-bio-age
```
