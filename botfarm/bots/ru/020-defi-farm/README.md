...
> Фарминг и стейкинг с расчётом рисков

**ID:** `ru-020-defi-farm` · **Регион:** RU · **Язык:** ru ·
**Модель:** club · **Тариф цен:** premium

DeFi Ферма — бот по теме «DeFi». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9019
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/020-defi-farm/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 020-defi-farm.service /etc/systemd/system/
sudo systemctl enable --now 020-defi-farm
```
