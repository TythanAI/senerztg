...
> Honkai: пополнение по выгодному курсу за 15 минут

**ID:** `ru-439-honkai-crystals` · **Регион:** RU · **Язык:** ru ·
**Модель:** topup · **Тариф цен:** budget

Honkai — бот по теме «кристаллы Honkai». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9438
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/439-honkai-crystals/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 439-honkai-crystals.service /etc/systemd/system/
sudo systemctl enable --now 439-honkai-crystals
```
