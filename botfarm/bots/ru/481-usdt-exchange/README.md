...
> USDT Обмен: быстро, по курсу, без лишних вопросов

**ID:** `ru-481-usdt-exchange` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** premium

USDT Обмен — бот по теме «обмен USDT». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9480
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/481-usdt-exchange/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 481-usdt-exchange.service /etc/systemd/system/
sudo systemctl enable --now 481-usdt-exchange
```
