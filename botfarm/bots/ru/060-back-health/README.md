...
**ID:** `ru-060-back-health` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** standard

Здоровая Спина — бот по теме «здоровье спины». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9059
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/060-back-health/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 060-back-health.service /etc/systemd/system/
sudo systemctl enable --now 060-back-health
```
