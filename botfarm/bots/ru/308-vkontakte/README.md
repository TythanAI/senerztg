...
> ВКонтакте: отлежавшиеся аккаунты с гарантией

**ID:** `ru-308-vkontakte` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

ВКонтакте Аккаунты — бот по теме «аккаунты ВКонтакте». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9307
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/308-vkontakte/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 308-vkontakte.service /etc/systemd/system/
sudo systemctl enable --now 308-vkontakte
```
