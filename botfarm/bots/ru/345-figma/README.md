...
> Figma по цене в разы ниже подписки

**ID:** `ru-345-figma` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Figma Доступ — бот по теме «аккаунты Figma». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9344
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/345-figma/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 345-figma.service /etc/systemd/system/
sudo systemctl enable --now 345-figma
```
