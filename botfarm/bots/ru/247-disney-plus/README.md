...
> Disney+ дешевле официальной подписки, выдача сразу

**ID:** `ru-247-disney-plus` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Disney+ Подписки — бот по теме «аккаунты Disney+». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9246
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/247-disney-plus/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 247-disney-plus.service /etc/systemd/system/
sudo systemctl enable --now 247-disney-plus
```
