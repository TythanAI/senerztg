...
> Genshin Фарм: результат под ключ, оплата после согласования

**ID:** `ru-452-genshin-farm` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** standard

Genshin Фарм — бот по теме «фарм в Genshin Impact». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9451
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/452-genshin-farm/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 452-genshin-farm.service /etc/systemd/system/
sudo systemctl enable --now 452-genshin-farm
```
