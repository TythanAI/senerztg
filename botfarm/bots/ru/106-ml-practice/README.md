...
> Разбор задач машинного обучения

**ID:** `ru-106-ml-practice` · **Регион:** RU · **Язык:** ru ·
**Модель:** club · **Тариф цен:** premium

ML Практика — бот по теме «машинное обучение». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9105
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/106-ml-practice/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 106-ml-practice.service /etc/systemd/system/
sudo systemctl enable --now 106-ml-practice
```
