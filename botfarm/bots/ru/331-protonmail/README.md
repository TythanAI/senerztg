# Proton Mail Аккаунты

> Proton Mail: чистые аккаунты для работы и регистраций

**ID:** `ru-331-protonmail` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Proton Mail Аккаунты — бот по теме «аккаунты Proton Mail». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.

## Что уже готово

- Каталог из 3 тарифов с автоматической выдачей после оплаты
- Приём оплаты: yookassa, sbp, cryptobot, stars, ton, manual
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9330
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/331-protonmail/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 331-protonmail.service /etc/systemd/system/
sudo systemctl enable --now 331-protonmail
```
