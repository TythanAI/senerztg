...
> GTA V: моментальная выдача, гарантия и замена

**ID:** `ru-216-gta5` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

GTA V Аккаунты — бот по теме «аккаунты GTA V». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/216-gta5/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 216-gta5.service /etc/systemd/system/
sudo systemctl enable --now 216-gta5
```
