...
> Netflix Card: ключи и коды с моментальной выдачей

**ID:** `ru-412-netflix-card` · **Регион:** RU · **Язык:** ru ·
**Модель:** keys · **Тариф цен:** standard

Netflix Card — бот по теме «карты Netflix». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9411
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/412-netflix-card/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 412-netflix-card.service /etc/systemd/system/
sudo systemctl enable --now 412-netflix-card
```
