...
> Warface: моментальная выдача, гарантия и замена

**ID:** `ru-229-warface` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Warface Аккаунты — бот по теме «аккаунты Warface». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9228
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/229-warface/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 229-warface.service /etc/systemd/system/
sudo systemctl enable --now 229-warface
```
