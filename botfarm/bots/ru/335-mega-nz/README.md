...
> MEGA: чистые аккаунты для работы и регистраций

**ID:** `ru-335-mega-nz` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

MEGA Аккаунты — бот по теме «аккаунты MEGA». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9334
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/335-mega-nz/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 335-mega-nz.service /etc/systemd/system/
sudo systemctl enable --now 335-mega-nz
```
