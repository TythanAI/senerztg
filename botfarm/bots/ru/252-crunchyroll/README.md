# Crunchyroll Подписки

> Crunchyroll дешевле официальной подписки, выдача сразу

**ID:** `ru-252-crunchyroll` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Crunchyroll Подписки — бот по теме «аккаунты Crunchyroll». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/252-crunchyroll/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 252-crunchyroll.service /etc/systemd/system/
sudo systemctl enable --now 252-crunchyroll
```
