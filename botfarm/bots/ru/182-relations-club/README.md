...
**ID:** `ru-182-relations-club` · **Регион:** RU · **Язык:** ru ·
**Модель:** club · **Тариф цен:** standard

Отношения Клуб — бот по теме «отношения». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9181
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/182-relations-club/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 182-relations-club.service /etc/systemd/system/
sudo systemctl enable --now 182-relations-club
```
