...
> Canva Pro по цене в разы ниже подписки

**ID:** `ru-346-canva-pro` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Canva Pro Доступ — бот по теме «аккаунты Canva Pro». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9345
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/346-canva-pro/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 346-canva-pro.service /etc/systemd/system/
sudo systemctl enable --now 346-canva-pro
```
