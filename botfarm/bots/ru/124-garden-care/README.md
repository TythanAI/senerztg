...
> Газон, обрезка, сезонные работы

**ID:** `ru-124-garden-care` · **Регион:** RU · **Язык:** ru ·
**Модель:** leadgen · **Тариф цен:** standard

Уход за Участком — бот по теме «уход за участком». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9123
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/124-garden-care/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 124-garden-care.service /etc/systemd/system/
sudo systemctl enable --now 124-garden-care
```
