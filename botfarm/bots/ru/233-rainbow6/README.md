# Rainbow Six Аккаунты

> Rainbow Six: моментальная выдача, гарантия и замена

**ID:** `ru-233-rainbow6` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Rainbow Six Аккаунты — бот по теме «аккаунты Rainbow Six». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9232
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/233-rainbow6/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 233-rainbow6.service /etc/systemd/system/
sudo systemctl enable --now 233-rainbow6
```
