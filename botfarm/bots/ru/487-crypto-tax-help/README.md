...
> Крипто Налоги: быстро, по курсу, без лишних вопросов

**ID:** `ru-487-crypto-tax-help` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** premium

Крипто Налоги — бот по теме «отчётность по криптоактивам». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9486
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/487-crypto-tax-help/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 487-crypto-tax-help.service /etc/systemd/system/
sudo systemctl enable --now 487-crypto-tax-help
```
