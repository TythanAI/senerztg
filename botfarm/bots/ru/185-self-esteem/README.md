...
> Опора на себя за 30 дней

**ID:** `ru-185-self-esteem` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** standard

Самооценка — бот по теме «самооценка». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9184
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/185-self-esteem/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 185-self-esteem.service /etc/systemd/system/
sudo systemctl enable --now 185-self-esteem
```
