...
> WoW Буст: результат под ключ, оплата после согласования

**ID:** `ru-450-wow-boost` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** standard

WoW Буст — бот по теме «буст в World of Warcraft». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9449
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/450-wow-boost/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 450-wow-boost.service /etc/systemd/system/
sudo systemctl enable --now 450-wow-boost
```
