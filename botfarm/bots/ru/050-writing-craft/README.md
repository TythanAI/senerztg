...
> Пишем сильные тексты каждую неделю

**ID:** `ru-050-writing-craft` · **Регион:** RU · **Язык:** ru ·
**Модель:** club · **Тариф цен:** standard

Мастерская Текста — бот по теме «копирайтинг». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9049
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/050-writing-craft/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 050-writing-craft.service /etc/systemd/system/
sudo systemctl enable --now 050-writing-craft
```
