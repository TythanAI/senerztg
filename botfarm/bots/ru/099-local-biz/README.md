...
> Клиенты из вашего района

**ID:** `ru-099-local-biz` · **Регион:** RU · **Язык:** ru ·
**Модель:** leadgen · **Тариф цен:** standard

Локальный Бизнес — бот по теме «локальный маркетинг». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9098
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/099-local-biz/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 099-local-biz.service /etc/systemd/system/
sudo systemctl enable --now 099-local-biz
```
