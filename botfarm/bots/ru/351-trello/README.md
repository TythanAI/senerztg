...
> Trello по цене в разы ниже подписки

**ID:** `ru-351-trello` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Trello Доступ — бот по теме «аккаунты Trello». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9350
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/351-trello/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 351-trello.service /etc/systemd/system/
sudo systemctl enable --now 351-trello
```
