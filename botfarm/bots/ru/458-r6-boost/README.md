...
> R6 Буст: результат под ключ, оплата после согласования

**ID:** `ru-458-r6-boost` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** standard

R6 Буст — бот по теме «буст ранга Rainbow Six». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9457
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/458-r6-boost/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 458-r6-boost.service /etc/systemd/system/
sudo systemctl enable --now 458-r6-boost
```
