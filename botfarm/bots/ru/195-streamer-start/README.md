...
> Первая тысяча зрителей

**ID:** `ru-195-streamer-start` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** standard

Стример Старт — бот по теме «стриминг». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9194
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/195-streamer-start/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 195-streamer-start.service /etc/systemd/system/
sudo systemctl enable --now 195-streamer-start
```
