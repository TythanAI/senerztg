...
> Хостинг Нод: быстро, по курсу, без лишних вопросов

**ID:** `ru-489-node-hosting` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** premium

Хостинг Нод — бот по теме «хостинг блокчейн-нод». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9488
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/489-node-hosting/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 489-node-hosting.service /etc/systemd/system/
sudo systemctl enable --now 489-node-hosting
```
