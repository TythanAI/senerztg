# Статичные прокси Доступ

> Статичные прокси: быстрая выдача, замена при проблемах

**ID:** `ru-372-static-proxy` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Статичные прокси Доступ — бот по теме «статичные прокси». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9371
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/372-static-proxy/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 372-static-proxy.service /etc/systemd/system/
sudo systemctl enable --now 372-static-proxy
```
