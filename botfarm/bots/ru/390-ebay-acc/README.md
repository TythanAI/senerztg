...
> eBay: готовые аккаунты для работы

**ID:** `ru-390-ebay-acc` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

eBay Аккаунты — бот по теме «аккаунты eBay». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9389
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/390-ebay-acc/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 390-ebay-acc.service /etc/systemd/system/
sudo systemctl enable --now 390-ebay-acc
```
