...
> Duolingo по цене в разы ниже подписки

**ID:** `ru-360-duolingo` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Duolingo Доступ — бот по теме «аккаунты Duolingo Plus». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9359
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/360-duolingo/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 360-duolingo.service /etc/systemd/system/
sudo systemctl enable --now 360-duolingo
```
