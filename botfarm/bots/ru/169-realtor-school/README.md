...
> Профессия с нуля до сделок

**ID:** `ru-169-realtor-school` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** premium

Школа Риелтора — бот по теме «работа риелтором». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9168
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/169-realtor-school/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 169-realtor-school.service /etc/systemd/system/
sudo systemctl enable --now 169-realtor-school
```
