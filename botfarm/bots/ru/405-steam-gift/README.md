...
> Steam Gift Card: ключи и коды с моментальной выдачей

**ID:** `ru-405-steam-gift` · **Регион:** RU · **Язык:** ru ·
**Модель:** keys · **Тариф цен:** standard

Steam Gift Card — бот по теме «подарочные карты Steam». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9404
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/405-steam-gift/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 405-steam-gift.service /etc/systemd/system/
sudo systemctl enable --now 405-steam-gift
```
