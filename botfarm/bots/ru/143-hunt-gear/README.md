...
> Гайды по экипировке и сезонам

**ID:** `ru-143-hunt-gear` · **Регион:** RU · **Язык:** ru ·
**Модель:** shop · **Тариф цен:** standard

Охота Снаряжение — бот по теме «охота». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9142
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/143-hunt-gear/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 143-hunt-gear.service /etc/systemd/system/
sudo systemctl enable --now 143-hunt-gear
```
