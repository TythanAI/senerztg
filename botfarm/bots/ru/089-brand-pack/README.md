...
> Логотип, фирстиль, гайдлайн

**ID:** `ru-089-brand-pack` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** premium

Бренд Пакет — бот по теме «брендинг». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9088
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/089-brand-pack/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 089-brand-pack.service /etc/systemd/system/
sudo systemctl enable --now 089-brand-pack
```
