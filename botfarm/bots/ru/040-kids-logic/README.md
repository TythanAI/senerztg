...
> Задачи для 6-10 лет каждый день

**ID:** `ru-040-kids-logic` · **Регион:** RU · **Язык:** ru ·
**Модель:** club · **Тариф цен:** budget

Детская Логика — бот по теме «развитие детей». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9039
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/040-kids-logic/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 040-kids-logic.service /etc/systemd/system/
sudo systemctl enable --now 040-kids-logic
```
