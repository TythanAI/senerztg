...
> Claude Pro без карты и VPN — доступ за минуту

**ID:** `ru-273-claude-pro` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Claude Pro Доступ — бот по теме «аккаунты Claude Pro». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/273-claude-pro/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 273-claude-pro.service /etc/systemd/system/
sudo systemctl enable --now 273-claude-pro
```
