...
> Доработки и разборы проектов

**ID:** `ru-159-car-tuning` · **Регион:** RU · **Язык:** ru ·
**Модель:** club · **Тариф цен:** standard

Тюнинг Клуб — бот по теме «тюнинг». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9158
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/159-car-tuning/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 159-car-tuning.service /etc/systemd/system/
sudo systemctl enable --now 159-car-tuning
```
