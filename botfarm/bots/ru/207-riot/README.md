...
> Riot Games: моментальная выдача, гарантия и замена

**ID:** `ru-207-riot` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Riot Games Аккаунты — бот по теме «аккаунты Riot». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9206
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/207-riot/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 207-riot.service /etc/systemd/system/
sudo systemctl enable --now 207-riot
```
