# Яндекс Почта Аккаунты

> Яндекс Почта: чистые аккаунты для работы и регистраций

**ID:** `ru-329-yandex-mail` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Яндекс Почта Аккаунты — бот по теме «аккаунты Яндекс Почты». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9328
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/329-yandex-mail/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 329-yandex-mail.service /etc/systemd/system/
sudo systemctl enable --now 329-yandex-mail
```
