...
> Slack по цене в разы ниже подписки

**ID:** `ru-349-slack` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Slack Доступ — бот по теме «аккаунты Slack». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9348
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/349-slack/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 349-slack.service /etc/systemd/system/
sudo systemctl enable --now 349-slack
```
