...
> ExpressVPN: быстрая выдача, замена при проблемах

**ID:** `ru-363-expressvpn` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

ExpressVPN Доступ — бот по теме «аккаунты ExpressVPN». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9362
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/363-expressvpn/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 363-expressvpn.service /etc/systemd/system/
sudo systemctl enable --now 363-expressvpn
```
