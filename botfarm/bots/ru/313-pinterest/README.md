...
> Pinterest: отлежавшиеся аккаунты с гарантией

**ID:** `ru-313-pinterest` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Pinterest Аккаунты — бот по теме «аккаунты Pinterest». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9312
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/313-pinterest/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 313-pinterest.service /etc/systemd/system/
sudo systemctl enable --now 313-pinterest
```
