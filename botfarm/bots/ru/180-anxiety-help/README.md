# Тревога Под Контроль

> Протокол работы с тревогой

**ID:** `ru-180-anxiety-help` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** standard

Тревога Под Контроль — бот по теме «тревожность». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9179
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/180-anxiety-help/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 180-anxiety-help.service /etc/systemd/system/
sudo systemctl enable --now 180-anxiety-help
```
