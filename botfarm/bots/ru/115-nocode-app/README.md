...
> Продукт без единой строки кода

**ID:** `ru-115-nocode-app` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** standard

Nocode Приложения — бот по теме «no-code». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9114
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/115-nocode-app/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 115-nocode-app.service /etc/systemd/system/
sudo systemctl enable --now 115-nocode-app
```
