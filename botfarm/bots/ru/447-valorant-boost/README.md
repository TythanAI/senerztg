...
> Valorant Буст: результат под ключ, оплата после согласования

**ID:** `ru-447-valorant-boost` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** standard

Valorant Буст — бот по теме «буст ранга Valorant». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9446
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/447-valorant-boost/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 447-valorant-boost.service /etc/systemd/system/
sudo systemctl enable --now 447-valorant-boost
```
