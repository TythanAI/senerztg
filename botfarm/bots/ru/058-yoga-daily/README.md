...
> Практики по 15 минут

**ID:** `ru-058-yoga-daily` · **Регион:** RU · **Язык:** ru ·
**Модель:** club · **Тариф цен:** budget

Йога Каждый День — бот по теме «йога». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9057
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/058-yoga-daily/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 058-yoga-daily.service /etc/systemd/system/
sudo systemctl enable --now 058-yoga-daily
```
