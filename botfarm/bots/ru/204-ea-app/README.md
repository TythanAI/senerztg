...
> EA App: моментальная выдача, гарантия и замена

**ID:** `ru-204-ea-app` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

EA App Аккаунты — бот по теме «аккаунты EA». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/204-ea-app/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 204-ea-app.service /etc/systemd/system/
sudo systemctl enable --now 204-ea-app
```
