# Escape from Tarkov Аккаунты

> Escape from Tarkov: моментальная выдача, гарантия и замена

**ID:** `ru-240-tarkov` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Escape from Tarkov Аккаунты — бот по теме «аккаунты Tarkov». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9239
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/240-tarkov/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 240-tarkov.service /etc/systemd/system/
sudo systemctl enable --now 240-tarkov
```
