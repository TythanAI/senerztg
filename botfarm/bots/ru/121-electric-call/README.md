...
> Проводка, розетки, щиты

**ID:** `ru-121-electric-call` · **Регион:** RU · **Язык:** ru ·
**Модель:** leadgen · **Тариф цен:** standard

Электрик Вызов — бот по теме «электрика». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9120
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/121-electric-call/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 121-electric-call.service /etc/systemd/system/
sudo systemctl enable --now 121-electric-call
```
