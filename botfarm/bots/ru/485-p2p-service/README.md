...
> P2P Сервис: быстро, по курсу, без лишних вопросов

**ID:** `ru-485-p2p-service` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** premium

P2P Сервис — бот по теме «P2P-обмен». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9484
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/485-p2p-service/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 485-p2p-service.service /etc/systemd/system/
sudo systemctl enable --now 485-p2p-service
```
