...
> От нот до собственных треков

**ID:** `ru-047-music-theory` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** standard

Теория Музыки — бот по теме «теория музыки». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9046
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/047-music-theory/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 047-music-theory.service /etc/systemd/system/
sudo systemctl enable --now 047-music-theory
```
