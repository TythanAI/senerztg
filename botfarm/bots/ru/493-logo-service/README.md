...
> Логотипы: делаем под ключ, сроки фиксируем

**ID:** `ru-493-logo-service` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** standard

Логотипы — бот по теме «разработка логотипов». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9492
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/493-logo-service/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 493-logo-service.service /etc/systemd/system/
sudo systemctl enable --now 493-logo-service
```
