...
> Microsoft Office: ключи и коды с моментальной выдачей

**ID:** `ru-415-office-key` · **Регион:** RU · **Язык:** ru ·
**Модель:** keys · **Тариф цен:** standard

Microsoft Office — бот по теме «ключи Office». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9414
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/415-office-key/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 415-office-key.service /etc/systemd/system/
sudo systemctl enable --now 415-office-key
```
