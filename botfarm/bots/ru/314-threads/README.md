...
> Threads: отлежавшиеся аккаунты с гарантией

**ID:** `ru-314-threads` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Threads Аккаунты — бот по теме «аккаунты Threads». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9313
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/314-threads/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 314-threads.service /etc/systemd/system/
sudo systemctl enable --now 314-threads
```
