...
> Мониторинг Скинов: уведомления в этот чат в реальном времени

**ID:** `ru-465-skins-monitor` · **Регион:** RU · **Язык:** ru ·
**Модель:** monitor · **Тариф цен:** standard

Мониторинг Скинов — бот по теме «мониторинг цен на скины». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9464
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/465-skins-monitor/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 465-skins-monitor.service /etc/systemd/system/
sudo systemctl enable --now 465-skins-monitor
```
