...
**ID:** `ru-030-stables-yield` · **Регион:** RU · **Язык:** ru ·
**Модель:** club · **Тариф цен:** standard

Стейбл Доход — бот по теме «стейблкоины». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9029
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/030-stables-yield/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 030-stables-yield.service /etc/systemd/system/
sudo systemctl enable --now 030-stables-yield
```
