...
> Flutter от нуля до релиза

**ID:** `ru-109-mobile-dev` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** premium

Мобильная Разработка — бот по теме «мобильная разработка». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9108
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/109-mobile-dev/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 109-mobile-dev.service /etc/systemd/system/
sudo systemctl enable --now 109-mobile-dev
```
