...
> Одобрение и снижение ставки

**ID:** `ru-163-mortgage-calc` · **Регион:** RU · **Язык:** ru ·
**Модель:** consult · **Тариф цен:** premium

Ипотека Консультант — бот по теме «ипотека». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9162
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/163-mortgage-calc/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 163-mortgage-calc.service /etc/systemd/system/
sudo systemctl enable --now 163-mortgage-calc
```
