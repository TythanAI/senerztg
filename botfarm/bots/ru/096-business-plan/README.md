...
> План для банка и инвестора

**ID:** `ru-096-business-plan` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** premium

Бизнес-План — бот по теме «бизнес-планы». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9095
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/096-business-plan/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 096-business-plan.service /etc/systemd/system/
sudo systemctl enable --now 096-business-plan
```
