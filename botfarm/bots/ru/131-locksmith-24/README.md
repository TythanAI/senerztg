...
**ID:** `ru-131-locksmith-24` · **Регион:** RU · **Язык:** ru ·
**Модель:** leadgen · **Тариф цен:** standard

Вскрытие Замков — бот по теме «вскрытие замков». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9130
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/131-locksmith-24/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 131-locksmith-24.service /etc/systemd/system/
sudo systemctl enable --now 131-locksmith-24
```
