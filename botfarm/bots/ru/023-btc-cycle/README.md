...
> Циклический анализ биткоина

**ID:** `ru-023-btc-cycle` · **Регион:** RU · **Язык:** ru ·
**Модель:** signals · **Тариф цен:** premium

BTC Цикл — бот по теме «биткоин». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9022
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/023-btc-cycle/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 023-btc-cycle.service /etc/systemd/system/
sudo systemctl enable --now 023-btc-cycle
```
