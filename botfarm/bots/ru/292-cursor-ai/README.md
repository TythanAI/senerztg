...
> Cursor без карты и VPN — доступ за минуту

**ID:** `ru-292-cursor-ai` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Cursor Доступ — бот по теме «аккаунты Cursor». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9291
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/292-cursor-ai/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 292-cursor-ai.service /etc/systemd/system/
sudo systemctl enable --now 292-cursor-ai
```
