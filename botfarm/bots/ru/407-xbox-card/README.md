...
> Xbox Gift Card: ключи и коды с моментальной выдачей

**ID:** `ru-407-xbox-card` · **Регион:** RU · **Язык:** ru ·
**Модель:** keys · **Тариф цен:** standard

Xbox Gift Card — бот по теме «карты Xbox». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9406
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/407-xbox-card/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 407-xbox-card.service /etc/systemd/system/
sudo systemctl enable --now 407-xbox-card
```
