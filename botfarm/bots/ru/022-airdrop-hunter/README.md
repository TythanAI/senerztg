...
> Ретродропы: чек-листы и активности

**ID:** `ru-022-airdrop-hunter` · **Регион:** RU · **Язык:** ru ·
**Модель:** club · **Тариф цен:** standard

Аирдроп Хантер — бот по теме «аирдропы». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9021
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/022-airdrop-hunter/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 022-airdrop-hunter.service /etc/systemd/system/
sudo systemctl enable --now 022-airdrop-hunter
```
