...
> Standoff 2: моментальная выдача, гарантия и замена

**ID:** `ru-226-standoff2` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Standoff 2 Аккаунты — бот по теме «аккаунты Standoff 2». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/226-standoff2/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 226-standoff2.service /etc/systemd/system/
sudo systemctl enable --now 226-standoff2
```
