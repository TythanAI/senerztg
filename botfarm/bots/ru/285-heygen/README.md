...
> HeyGen без карты и VPN — доступ за минуту

**ID:** `ru-285-heygen` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

HeyGen Доступ — бот по теме «аккаунты HeyGen». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/285-heygen/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 285-heygen.service /etc/systemd/system/
sudo systemctl enable --now 285-heygen
```
