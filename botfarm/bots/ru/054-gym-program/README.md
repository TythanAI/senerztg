...
> Персональная программа на 12 недель

**ID:** `ru-054-gym-program` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** standard

Зал Программа — бот по теме «тренировки в зале». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9053
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/054-gym-program/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 054-gym-program.service /etc/systemd/system/
sudo systemctl enable --now 054-gym-program
```
