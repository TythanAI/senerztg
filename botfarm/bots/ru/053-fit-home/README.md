...
> Тренировки без зала, 20 минут в день

**ID:** `ru-053-fit-home` · **Регион:** RU · **Язык:** ru ·
**Модель:** club · **Тариф цен:** budget

Фит Дома — бот по теме «домашние тренировки». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9052
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/053-fit-home/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 053-fit-home.service /etc/systemd/system/
sudo systemctl enable --now 053-fit-home
```
