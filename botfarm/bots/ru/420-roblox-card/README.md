...
> Roblox Card: ключи и коды с моментальной выдачей

**ID:** `ru-420-roblox-card` · **Регион:** RU · **Язык:** ru ·
**Модель:** keys · **Тариф цен:** standard

Roblox Card — бот по теме «карты Roblox». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9419
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/420-roblox-card/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 420-roblox-card.service /etc/systemd/system/
sudo systemctl enable --now 420-roblox-card
```
