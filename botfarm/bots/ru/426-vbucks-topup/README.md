...
> V-Bucks: пополнение по выгодному курсу за 15 минут

**ID:** `ru-426-vbucks-topup` · **Регион:** RU · **Язык:** ru ·
**Модель:** topup · **Тариф цен:** budget

V-Bucks — бот по теме «пополнение V-Bucks». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9425
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/426-vbucks-topup/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 426-vbucks-topup.service /etc/systemd/system/
sudo systemctl enable --now 426-vbucks-topup
```
