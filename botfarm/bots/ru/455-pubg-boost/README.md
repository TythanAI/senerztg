...
> PUBG Буст: результат под ключ, оплата после согласования

**ID:** `ru-455-pubg-boost` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** standard

PUBG Буст — бот по теме «буст ранга PUBG». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9454
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/455-pubg-boost/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 455-pubg-boost.service /etc/systemd/system/
sudo systemctl enable --now 455-pubg-boost
```
