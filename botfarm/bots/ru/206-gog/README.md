...
> GOG: моментальная выдача, гарантия и замена

**ID:** `ru-206-gog` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

GOG Аккаунты — бот по теме «аккаунты GOG». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/206-gog/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 206-gog.service /etc/systemd/system/
sudo systemctl enable --now 206-gog
```
