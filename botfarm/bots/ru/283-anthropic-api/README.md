...
> Anthropic API без карты и VPN — доступ за минуту

**ID:** `ru-283-anthropic-api` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Anthropic API Доступ — бот по теме «доступ к Anthropic API». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9282
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/283-anthropic-api/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 283-anthropic-api.service /etc/systemd/system/
sudo systemctl enable --now 283-anthropic-api
```
