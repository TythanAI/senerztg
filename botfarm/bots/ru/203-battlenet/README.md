...
> Battle.net: моментальная выдача, гарантия и замена

**ID:** `ru-203-battlenet` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Battle.net Аккаунты — бот по теме «аккаунты Battle.net». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/203-battlenet/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 203-battlenet.service /etc/systemd/system/
sudo systemctl enable --now 203-battlenet
```
