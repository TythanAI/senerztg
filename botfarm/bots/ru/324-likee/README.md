...
> Likee: отлежавшиеся аккаунты с гарантией

**ID:** `ru-324-likee` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Likee Аккаунты — бот по теме «аккаунты Likee». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9323
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/324-likee/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 324-likee.service /etc/systemd/system/
sudo systemctl enable --now 324-likee
```
