...
> Leonardo AI без карты и VPN — доступ за минуту

**ID:** `ru-281-leonardo-ai` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Leonardo AI Доступ — бот по теме «аккаунты Leonardo». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9280
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/281-leonardo-ai/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 281-leonardo-ai.service /etc/systemd/system/
sudo systemctl enable --now 281-leonardo-ai
```
