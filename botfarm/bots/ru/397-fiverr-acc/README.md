...
> Fiverr: готовые аккаунты для работы

**ID:** `ru-397-fiverr-acc` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Fiverr Аккаунты — бот по теме «аккаунты Fiverr». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9396
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/397-fiverr-acc/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 397-fiverr-acc.service /etc/systemd/system/
sudo systemctl enable --now 397-fiverr-acc
```
