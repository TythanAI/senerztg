# Резидентные прокси Доступ

> Резидентные прокси: быстрая выдача, замена при проблемах

**ID:** `ru-371-residential-proxy` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Резидентные прокси Доступ — бот по теме «резидентные прокси». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9370
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/371-residential-proxy/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 371-residential-proxy.service /etc/systemd/system/
sudo systemctl enable --now 371-residential-proxy
```
