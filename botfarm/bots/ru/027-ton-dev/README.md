...
> Смарт-контракты и мини-аппы TON

**ID:** `ru-027-ton-dev` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** premium

TON Разработка — бот по теме «разработка на TON». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9026
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/027-ton-dev/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 027-ton-dev.service /etc/systemd/system/
sudo systemctl enable --now 027-ton-dev
```
