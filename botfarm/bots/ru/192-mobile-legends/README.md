...
**ID:** `ru-192-mobile-legends` · **Регион:** RU · **Язык:** ru ·
**Модель:** club · **Тариф цен:** budget

Мобильные Легенды — бот по теме «мобильные MOBA». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9191
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/192-mobile-legends/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 192-mobile-legends.service /etc/systemd/system/
sudo systemctl enable --now 192-mobile-legends
```
