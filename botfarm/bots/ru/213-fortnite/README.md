...
> Fortnite: моментальная выдача, гарантия и замена

**ID:** `ru-213-fortnite` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Fortnite Аккаунты — бот по теме «аккаунты Fortnite». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9212
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/213-fortnite/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 213-fortnite.service /etc/systemd/system/
sudo systemctl enable --now 213-fortnite
```
