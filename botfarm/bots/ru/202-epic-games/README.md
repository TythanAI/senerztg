...
> Epic Games: моментальная выдача, гарантия и замена

**ID:** `ru-202-epic-games` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Epic Games Аккаунты — бот по теме «аккаунты Epic Games». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9201
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/202-epic-games/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 202-epic-games.service /etc/systemd/system/
sudo systemctl enable --now 202-epic-games
```
