...
> VK Музыка дешевле официальной подписки, выдача сразу

**ID:** `ru-262-vk-music` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

VK Музыка Подписки — бот по теме «подписки VK Музыка». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/262-vk-music/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 262-vk-music.service /etc/systemd/system/
sudo systemctl enable --now 262-vk-music
```
