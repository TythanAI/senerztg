...
> Заказы, чеки, переговоры

**ID:** `ru-112-freelance-it` · **Регион:** RU · **Язык:** ru ·
**Модель:** club · **Тариф цен:** standard

IT Фриланс — бот по теме «IT-фриланс». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9111

## Каталог

| SKU | Название | Цена | Тип |
|-----|----------|------|-----|
| `month` | Месяц доступа | 1 480 ₽ | subscription · 30 дн. |
| `quarter` | Три месяца | 3 860 ₽ | subscription · 90 дн. |
| `year` | Год доступа | 11 910 ₽ | subscription · 365 дн. |
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/112-freelance-it/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 112-freelance-it.service /etc/systemd/system/
sudo systemctl enable --now 112-freelance-it
```
