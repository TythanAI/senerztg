...
> Gamma без карты и VPN — доступ за минуту

**ID:** `ru-298-gamma-app` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Gamma Доступ — бот по теме «аккаунты Gamma». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9297
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/298-gamma-app/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 298-gamma-app.service /etc/systemd/system/
sudo systemctl enable --now 298-gamma-app
```
