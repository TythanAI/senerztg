...
> Uber: готовые аккаунты для работы

**ID:** `ru-398-uber-acc` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Uber Аккаунты — бот по теме «аккаунты Uber». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9397
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/398-uber-acc/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 398-uber-acc.service /etc/systemd/system/
sudo systemctl enable --now 398-uber-acc
```
