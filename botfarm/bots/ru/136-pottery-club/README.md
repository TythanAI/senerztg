...
> Гончарное дело для начинающих

**ID:** `ru-136-pottery-club` · **Регион:** RU · **Язык:** ru ·
**Модель:** club · **Тариф цен:** standard

Керамика Клуб — бот по теме «керамика». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9135
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/136-pottery-club/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 136-pottery-club.service /etc/systemd/system/
sudo systemctl enable --now 136-pottery-club
```
