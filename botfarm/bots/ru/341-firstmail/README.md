...
> Firstmail: чистые аккаунты для работы и регистраций

**ID:** `ru-341-firstmail` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Firstmail Аккаунты — бот по теме «аккаунты Firstmail». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9340
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/341-firstmail/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 341-firstmail.service /etc/systemd/system/
sudo systemctl enable --now 341-firstmail
```
