...
> Домен Монитор: уведомления в этот чат в реальном времени

**ID:** `ru-470-domain-monitor` · **Регион:** RU · **Язык:** ru ·
**Модель:** monitor · **Тариф цен:** standard

Домен Монитор — бот по теме «мониторинг освобождения доменов». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9469
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/470-domain-monitor/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 470-domain-monitor.service /etc/systemd/system/
sudo systemctl enable --now 470-domain-monitor
```
