...
> CyberGhost: быстрая выдача, замена при проблемах

**ID:** `ru-366-cyberghost` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

CyberGhost Доступ — бот по теме «аккаунты CyberGhost». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9365
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/366-cyberghost/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 366-cyberghost.service /etc/systemd/system/
sudo systemctl enable --now 366-cyberghost
```
