...
> Nintendo eShop: ключи и коды с моментальной выдачей

**ID:** `ru-408-nintendo-card` · **Регион:** RU · **Язык:** ru ·
**Модель:** keys · **Тариф цен:** standard

Nintendo eShop — бот по теме «карты Nintendo eShop». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9407
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/408-nintendo-card/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 408-nintendo-card.service /etc/systemd/system/
sudo systemctl enable --now 408-nintendo-card
```
