...
> Возврат НДФЛ под ключ

**ID:** `ru-009-tax-refund` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** standard

Налог Возврат — бот по теме «налоговые вычеты». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9008
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/009-tax-refund/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 009-tax-refund.service /etc/systemd/system/
sudo systemctl enable --now 009-tax-refund
```
