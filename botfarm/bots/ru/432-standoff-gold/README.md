...
> Standoff 2: пополнение по выгодному курсу за 15 минут

**ID:** `ru-432-standoff-gold` · **Регион:** RU · **Язык:** ru ·
**Модель:** topup · **Тариф цен:** budget

Standoff 2 — бот по теме «золото Standoff 2». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9431
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/432-standoff-gold/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 432-standoff-gold.service /etc/systemd/system/
sudo systemctl enable --now 432-standoff-gold
```
