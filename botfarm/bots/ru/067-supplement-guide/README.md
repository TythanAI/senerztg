...
> Подбор добавок по анализам

**ID:** `ru-067-supplement-guide` · **Регион:** RU · **Язык:** ru ·
**Модель:** consult · **Тариф цен:** premium

Нутрициолог Онлайн — бот по теме «нутрициология». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9066

## Каталог

| SKU | Название | Цена | Тип |
|-----|----------|------|-----|
| `consult30` | Консультация 30 минут | 4 890 ₽ | consult |
| `consult60` | Консультация 60 минут | 8 810 ₽ | consult |
| `pack5` | Пакет из 5 встреч | 36 740 ₽ | consult |
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/067-supplement-guide/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 067-supplement-guide.service /etc/systemd/system/
sudo systemctl enable --now 067-supplement-guide
```
