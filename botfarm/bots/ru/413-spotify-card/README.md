...
> Spotify Card: ключи и коды с моментальной выдачей

**ID:** `ru-413-spotify-card` · **Регион:** RU · **Язык:** ru ·
**Модель:** keys · **Тариф цен:** standard

Spotify Card — бот по теме «карты Spotify». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9412
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/413-spotify-card/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 413-spotify-card.service /etc/systemd/system/
sudo systemctl enable --now 413-spotify-card
```
