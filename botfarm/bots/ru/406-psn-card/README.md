...
> PSN Card: ключи и коды с моментальной выдачей

**ID:** `ru-406-psn-card` · **Регион:** RU · **Язык:** ru ·
**Модель:** keys · **Тариф цен:** standard

PSN Card — бот по теме «карты PlayStation Store». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9405
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/406-psn-card/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 406-psn-card.service /etc/systemd/system/
sudo systemctl enable --now 406-psn-card
```
