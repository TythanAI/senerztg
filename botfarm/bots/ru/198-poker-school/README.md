...
> Матанализ вместо интуиции

**ID:** `ru-198-poker-school` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** premium

Покерная Школа — бот по теме «покер». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9197
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/198-poker-school/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 198-poker-school.service /etc/systemd/system/
sudo systemctl enable --now 198-poker-school
```
