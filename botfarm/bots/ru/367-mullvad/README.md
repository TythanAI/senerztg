...
> Mullvad: быстрая выдача, замена при проблемах

**ID:** `ru-367-mullvad` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Mullvad Доступ — бот по теме «аккаунты Mullvad». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9366
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/367-mullvad/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 367-mullvad.service /etc/systemd/system/
sudo systemctl enable --now 367-mullvad
```
