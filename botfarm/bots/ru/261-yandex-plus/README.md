...
> Яндекс Плюс дешевле официальной подписки, выдача сразу

**ID:** `ru-261-yandex-plus` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Яндекс Плюс Подписки — бот по теме «подписки Яндекс Плюс». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/261-yandex-plus/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 261-yandex-plus.service /etc/systemd/system/
sudo systemctl enable --now 261-yandex-plus
```
