...
> Новое блюдо каждую неделю

**ID:** `ru-138-cook-school` · **Регион:** RU · **Язык:** ru ·
**Модель:** club · **Тариф цен:** budget

Кулинарная Школа — бот по теме «кулинария». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9137
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/138-cook-school/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 138-cook-school.service /etc/systemd/system/
sudo systemctl enable --now 138-cook-school
```
