...
> Uptime Монитор: уведомления в этот чат в реальном времени

**ID:** `ru-469-uptime-monitor` · **Регион:** RU · **Язык:** ru ·
**Модель:** monitor · **Тариф цен:** standard

Uptime Монитор — бот по теме «мониторинг доступности сайтов». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9468
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/469-uptime-monitor/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 469-uptime-monitor.service /etc/systemd/system/
sudo systemctl enable --now 469-uptime-monitor
```
