...
> Тендер Монитор: уведомления в этот чат в реальном времени

**ID:** `ru-476-tender-monitor` · **Регион:** RU · **Язык:** ru ·
**Модель:** monitor · **Тариф цен:** standard

Тендер Монитор — бот по теме «мониторинг тендеров». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9475
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/476-tender-monitor/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 476-tender-monitor.service /etc/systemd/system/
sudo systemctl enable --now 476-tender-monitor
```
