...
> Съёмка и обработка на телефон

**ID:** `ru-133-photo-school` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** standard

Школа Фото — бот по теме «фотография». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9132
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/133-photo-school/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 133-photo-school.service /etc/systemd/system/
sudo systemctl enable --now 133-photo-school
```
