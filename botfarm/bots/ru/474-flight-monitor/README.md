...
> Авиа Монитор: уведомления в этот чат в реальном времени

**ID:** `ru-474-flight-monitor` · **Регион:** RU · **Язык:** ru ·
**Модель:** monitor · **Тариф цен:** standard

Авиа Монитор — бот по теме «мониторинг цен на авиабилеты». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9473
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/474-flight-monitor/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 474-flight-monitor.service /etc/systemd/system/
sudo systemctl enable --now 474-flight-monitor
```
