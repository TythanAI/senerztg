...
> Payoneer: готовые аккаунты для работы

**ID:** `ru-404-payoneer-acc` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Payoneer Аккаунты — бот по теме «аккаунты Payoneer». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9403
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/404-payoneer-acc/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 404-payoneer-acc.service /etc/systemd/system/
sudo systemctl enable --now 404-payoneer-acc
```
