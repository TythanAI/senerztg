...
> CS2 Буст: результат под ключ, оплата после согласования

**ID:** `ru-445-cs2-rank-boost` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** standard

CS2 Буст — бот по теме «прокачка ранга CS2». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9444
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/445-cs2-rank-boost/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 445-cs2-rank-boost.service /etc/systemd/system/
sudo systemctl enable --now 445-cs2-rank-boost
```
