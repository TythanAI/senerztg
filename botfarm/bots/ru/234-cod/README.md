# Call of Duty Аккаунты

> Call of Duty: моментальная выдача, гарантия и замена

**ID:** `ru-234-cod` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Call of Duty Аккаунты — бот по теме «аккаунты Call of Duty». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/234-cod/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 234-cod.service /etc/systemd/system/
sudo systemctl enable --now 234-cod
```
