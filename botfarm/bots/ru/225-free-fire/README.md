...
> Free Fire: моментальная выдача, гарантия и замена

**ID:** `ru-225-free-fire` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Free Fire Аккаунты — бот по теме «аккаунты Free Fire». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9224
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/225-free-fire/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 225-free-fire.service /etc/systemd/system/
sudo systemctl enable --now 225-free-fire
```
