...
> Гардероб, который работает

**ID:** `ru-076-style-guide` · **Регион:** RU · **Язык:** ru ·
**Модель:** consult · **Тариф цен:** premium

Стилист Онлайн — бот по теме «стиль и гардероб». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9075
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/076-style-guide/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 076-style-guide.service /etc/systemd/system/
sudo systemctl enable --now 076-style-guide
```
