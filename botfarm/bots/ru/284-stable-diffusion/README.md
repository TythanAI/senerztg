# Stable Diffusion Доступ

> Stable Diffusion без карты и VPN — доступ за минуту

**ID:** `ru-284-stable-diffusion` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Stable Diffusion Доступ — бот по теме «доступ к Stable Diffusion». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9283
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/284-stable-diffusion/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 284-stable-diffusion.service /etc/systemd/system/
sudo systemctl enable --now 284-stable-diffusion
```
