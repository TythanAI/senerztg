...
> Хореография по видеоурокам

**ID:** `ru-146-dance-home` · **Регион:** RU · **Язык:** ru ·
**Модель:** club · **Тариф цен:** budget

Танцы Дома — бот по теме «танцы». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9145
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/146-dance-home/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 146-dance-home.service /etc/systemd/system/
sudo systemctl enable --now 146-dance-home
```
