...
> Портфель на дивидендах: разборы и сделки

**ID:** `ru-002-dividend-club` · **Регион:** RU · **Язык:** ru ·
**Модель:** club · **Тариф цен:** premium

Дивидендный клуб — бот по теме «дивидендные акции». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9001

## Каталог

| SKU | Название | Цена | Тип |
|-----|----------|------|-----|
| `month` | Месяц доступа | 4 890 ₽ | subscription · 30 дн. |
| `quarter` | Три месяца | 12 730 ₽ | subscription · 90 дн. |
| `year` | Год доступа | 39 190 ₽ | subscription · 365 дн. |
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/002-dividend-club/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 002-dividend-club.service /etc/systemd/system/
sudo systemctl enable --now 002-dividend-club
```
