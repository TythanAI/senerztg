# YouTube Premium Подписки

> YouTube Premium дешевле официальной подписки, выдача сразу

**ID:** `ru-246-youtube-premium` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

YouTube Premium Подписки — бот по теме «аккаунты YouTube Premium». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/246-youtube-premium/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 246-youtube-premium.service /etc/systemd/system/
sudo systemctl enable --now 246-youtube-premium
```
