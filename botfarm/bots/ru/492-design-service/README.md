...
> Дизайн на Заказ: делаем под ключ, сроки фиксируем

**ID:** `ru-492-design-service` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** standard

Дизайн на Заказ — бот по теме «графический дизайн». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9491
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/492-design-service/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 492-design-service.service /etc/systemd/system/
sudo systemctl enable --now 492-design-service
```
