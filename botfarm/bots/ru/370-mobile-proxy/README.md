# Мобильные прокси Доступ

> Мобильные прокси: быстрая выдача, замена при проблемах

**ID:** `ru-370-mobile-proxy` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Мобильные прокси Доступ — бот по теме «мобильные прокси». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9369
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/370-mobile-proxy/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 370-mobile-proxy.service /etc/systemd/system/
sudo systemctl enable --now 370-mobile-proxy
```
