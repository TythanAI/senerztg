...
> Облигации и фонды: доход выше вклада

**ID:** `ru-003-bond-lab` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** standard

Облигации Лаб — бот по теме «облигации». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9002
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/003-bond-lab/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 003-bond-lab.service /etc/systemd/system/
sudo systemctl enable --now 003-bond-lab
```
