...
> Otter.ai без карты и VPN — доступ за минуту

**ID:** `ru-300-otter-ai` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Otter.ai Доступ — бот по теме «аккаунты Otter.ai». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9299
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/300-otter-ai/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 300-otter-ai.service /etc/systemd/system/
sudo systemctl enable --now 300-otter-ai
```
