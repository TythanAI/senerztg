...
> DeepL Pro без карты и VPN — доступ за минуту

**ID:** `ru-299-deepl` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

DeepL Pro Доступ — бот по теме «аккаунты DeepL». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/299-deepl/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 299-deepl.service /etc/systemd/system/
sudo systemctl enable --now 299-deepl
```
