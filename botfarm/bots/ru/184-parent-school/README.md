...
> Возрастные кризисы без криков

**ID:** `ru-184-parent-school` · **Регион:** RU · **Язык:** ru ·
**Модель:** club · **Тариф цен:** standard

Школа Родителей — бот по теме «воспитание детей». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9183
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/184-parent-school/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 184-parent-school.service /etc/systemd/system/
sudo systemctl enable --now 184-parent-school
```
