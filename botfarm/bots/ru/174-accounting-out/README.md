...
> Отчётность и зарплата

**ID:** `ru-174-accounting-out` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** premium

Бухгалтерия Аутсорс — бот по теме «бухгалтерия». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9173
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/174-accounting-out/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 174-accounting-out.service /etc/systemd/system/
sudo systemctl enable --now 174-accounting-out
```
