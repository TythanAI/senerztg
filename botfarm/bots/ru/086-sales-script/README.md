...
> Готовые скрипты для отдела продаж

**ID:** `ru-086-sales-script` · **Регион:** RU · **Язык:** ru ·
**Модель:** shop · **Тариф цен:** standard

Скрипты Продаж — бот по теме «скрипты продаж». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9085
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/086-sales-script/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 086-sales-script.service /etc/systemd/system/
sudo systemctl enable --now 086-sales-script
```
