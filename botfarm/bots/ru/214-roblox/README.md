...
> Roblox: моментальная выдача, гарантия и замена

**ID:** `ru-214-roblox` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Roblox Аккаунты — бот по теме «аккаунты Roblox». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9213
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/214-roblox/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 214-roblox.service /etc/systemd/system/
sudo systemctl enable --now 214-roblox
```
