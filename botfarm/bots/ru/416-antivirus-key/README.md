...
> Антивирусы: ключи и коды с моментальной выдачей

**ID:** `ru-416-antivirus-key` · **Регион:** RU · **Язык:** ru ·
**Модель:** keys · **Тариф цен:** standard

Антивирусы — бот по теме «ключи антивирусов». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9415
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/416-antivirus-key/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 416-antivirus-key.service /etc/systemd/system/
sudo systemctl enable --now 416-antivirus-key
```
