...
> Растения, которые не гибнут

**ID:** `ru-141-garden-home` · **Регион:** RU · **Язык:** ru ·
**Модель:** club · **Тариф цен:** budget

Домашний Сад — бот по теме «комнатные растения». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9140
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/141-garden-home/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 141-garden-home.service /etc/systemd/system/
sudo systemctl enable --now 141-garden-home
```
