...
> Steam: пополнение по выгодному курсу за 15 минут

**ID:** `ru-441-steam-wallet` · **Регион:** RU · **Язык:** ru ·
**Модель:** topup · **Тариф цен:** budget

Steam — бот по теме «пополнение кошелька Steam». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9440
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/441-steam-wallet/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 441-steam-wallet.service /etc/systemd/system/
sudo systemctl enable --now 441-steam-wallet
```
