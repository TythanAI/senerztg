...
> Речь, которая держит зал

**ID:** `ru-049-public-speak` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** premium

Публичные Выступления — бот по теме «ораторское мастерство». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9048
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/049-public-speak/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 049-public-speak.service /etc/systemd/system/
sudo systemctl enable --now 049-public-speak
```
