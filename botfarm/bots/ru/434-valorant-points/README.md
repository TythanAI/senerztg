...
> Valorant: пополнение по выгодному курсу за 15 минут

**ID:** `ru-434-valorant-points` · **Регион:** RU · **Язык:** ru ·
**Модель:** topup · **Тариф цен:** budget

Valorant — бот по теме «Valorant Points». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9433
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/434-valorant-points/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 434-valorant-points.service /etc/systemd/system/
sudo systemctl enable --now 434-valorant-points
```
