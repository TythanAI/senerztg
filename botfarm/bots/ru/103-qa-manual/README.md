...
**ID:** `ru-103-qa-manual` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** standard

QA Тестировщик — бот по теме «тестирование ПО». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9102
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/103-qa-manual/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 103-qa-manual.service /etc/systemd/system/
sudo systemctl enable --now 103-qa-manual
```
