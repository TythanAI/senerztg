...
> Destiny 2 Carry: результат под ключ, оплата после согласования

**ID:** `ru-453-destiny-carry` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** standard

Destiny 2 Carry — бот по теме «рейды в Destiny 2». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9452
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/453-destiny-carry/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 453-destiny-carry.service /etc/systemd/system/
sudo systemctl enable --now 453-destiny-carry
```
