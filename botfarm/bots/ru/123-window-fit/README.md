...
> Замер, монтаж, гарантия

**ID:** `ru-123-window-fit` · **Регион:** RU · **Язык:** ru ·
**Модель:** leadgen · **Тариф цен:** premium

Окна Установка — бот по теме «остекление». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9122
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/123-window-fit/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 123-window-fit.service /etc/systemd/system/
sudo systemctl enable --now 123-window-fit
```
