...
> EVE Online: моментальная выдача, гарантия и замена

**ID:** `ru-239-eve` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

EVE Online Аккаунты — бот по теме «аккаунты EVE Online». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/239-eve/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 239-eve.service /etc/systemd/system/
sudo systemctl enable --now 239-eve
```
