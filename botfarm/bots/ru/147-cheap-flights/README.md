...
> Ошибочные тарифы и распродажи

**ID:** `ru-147-cheap-flights` · **Регион:** RU · **Язык:** ru ·
**Модель:** signals · **Тариф цен:** budget

Дешёвые Билеты — бот по теме «авиабилеты». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9146
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/147-cheap-flights/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 147-cheap-flights.service /etc/systemd/system/
sudo systemctl enable --now 147-cheap-flights
```
