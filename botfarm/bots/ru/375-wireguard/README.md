...
> WireGuard: быстрая выдача, замена при проблемах

**ID:** `ru-375-wireguard` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

WireGuard Доступ — бот по теме «конфиги WireGuard». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9374
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/375-wireguard/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 375-wireguard.service /etc/systemd/system/
sudo systemctl enable --now 375-wireguard
```
