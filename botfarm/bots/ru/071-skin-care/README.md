...
> Индивидуальный уход по типу кожи

**ID:** `ru-071-skin-care` · **Регион:** RU · **Язык:** ru ·
**Модель:** consult · **Тариф цен:** standard

Косметолог Онлайн — бот по теме «уход за кожей». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9070
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/071-skin-care/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 071-skin-care.service /etc/systemd/system/
sudo systemctl enable --now 071-skin-care
```
