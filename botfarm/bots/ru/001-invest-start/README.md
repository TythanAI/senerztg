...
> Инвестиции с нуля: от первого счёта до портфеля

**ID:** `ru-001-invest-start` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** standard

ИнвестСтарт — бот по теме «инвестиции». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9000
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/001-invest-start/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 001-invest-start.service /etc/systemd/system/
sudo systemctl enable --now 001-invest-start
```
