...
> Салонные процедуры своими руками

**ID:** `ru-078-spa-home` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** budget

SPA Дома — бот по теме «домашний уход». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9077
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/078-spa-home/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 078-spa-home.service /etc/systemd/system/
sudo systemctl enable --now 078-spa-home
```
