...
> React-разработчик за 6 месяцев

**ID:** `ru-102-frontend-job` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** premium

Фронтенд Работа — бот по теме «фронтенд». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9101
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/102-frontend-job/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 102-frontend-job.service /etc/systemd/system/
sudo systemctl enable --now 102-frontend-job
```
