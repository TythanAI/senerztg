...
> Tidal дешевле официальной подписки, выдача сразу

**ID:** `ru-254-tidal` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Tidal Подписки — бот по теме «аккаунты Tidal». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9253
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/254-tidal/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 254-tidal.service /etc/systemd/system/
sudo systemctl enable --now 254-tidal
```
