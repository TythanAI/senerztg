...
> Резюме и LinkedIn, которые зовут

**ID:** `ru-110-it-resume` · **Регион:** RU · **Язык:** ru ·
**Модель:** service · **Тариф цен:** standard

IT Резюме — бот по теме «трудоустройство в IT». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9109
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/110-it-resume/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 110-it-resume.service /etc/systemd/system/
sudo systemctl enable --now 110-it-resume
```
