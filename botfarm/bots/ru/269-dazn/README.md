...
> DAZN дешевле официальной подписки, выдача сразу

**ID:** `ru-269-dazn` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

DAZN Подписки — бот по теме «аккаунты DAZN». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/269-dazn/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 269-dazn.service /etc/systemd/system/
sudo systemctl enable --now 269-dazn
```
