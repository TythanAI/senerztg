...
> Онлайн-дейтинг без иллюзий

**ID:** `ru-183-dating-guide` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** budget

Знакомства Гид — бот по теме «знакомства». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9182
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/183-dating-guide/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 183-dating-guide.service /etc/systemd/system/
sudo systemctl enable --now 183-dating-guide
```
