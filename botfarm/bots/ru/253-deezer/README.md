...
> Deezer дешевле официальной подписки, выдача сразу

**ID:** `ru-253-deezer` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Deezer Подписки — бот по теме «аккаунты Deezer». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/253-deezer/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 253-deezer.service /etc/systemd/system/
sudo systemctl enable --now 253-deezer
```
