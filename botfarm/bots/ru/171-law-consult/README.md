...
**ID:** `ru-171-law-consult` · **Регион:** RU · **Язык:** ru ·
**Модель:** consult · **Тариф цен:** premium

Юрист Онлайн — бот по теме «юридические консультации». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9170
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/171-law-consult/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 171-law-consult.service /etc/systemd/system/
sudo systemctl enable --now 171-law-consult
```
