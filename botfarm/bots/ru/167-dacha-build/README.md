...
> Каркасник своими силами

**ID:** `ru-167-dacha-build` · **Регион:** RU · **Язык:** ru ·
**Модель:** course · **Тариф цен:** premium

Дом Построить — бот по теме «строительство дома». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9166
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/167-dacha-build/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 167-dacha-build.service /etc/systemd/system/
sudo systemctl enable --now 167-dacha-build
```
