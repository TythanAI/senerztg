...
> Facebook: отлежавшиеся аккаунты с гарантией

**ID:** `ru-307-facebook` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Facebook Аккаунты — бот по теме «аккаунты Facebook». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9306
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/307-facebook/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 307-facebook.service /etc/systemd/system/
sudo systemctl enable --now 307-facebook
```
