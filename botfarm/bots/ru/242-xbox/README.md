...
> Xbox: моментальная выдача, гарантия и замена

**ID:** `ru-242-xbox` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Xbox Аккаунты — бот по теме «аккаунты Xbox». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/242-xbox/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 242-xbox.service /etc/systemd/system/
sudo systemctl enable --now 242-xbox
```
