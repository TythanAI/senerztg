...
> Развод, алименты, раздел

**ID:** `ru-176-family-law` · **Регион:** RU · **Язык:** ru ·
**Модель:** consult · **Тариф цен:** premium

Семейный Юрист — бот по теме «семейное право». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9175
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/176-family-law/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 176-family-law.service /etc/systemd/system/
sudo systemctl enable --now 176-family-law
```
