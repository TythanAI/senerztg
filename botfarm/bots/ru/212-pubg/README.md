...
> PUBG: моментальная выдача, гарантия и замена

**ID:** `ru-212-pubg` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

PUBG Аккаунты — бот по теме «аккаунты PUBG». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/212-pubg/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 212-pubg.service /etc/systemd/system/
sudo systemctl enable --now 212-pubg
```
