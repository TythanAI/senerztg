...
> Ozon: готовые аккаунты для работы

**ID:** `ru-393-ozon-acc` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

Ozon Аккаунты — бот по теме «кабинеты Ozon». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9392
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/393-ozon-acc/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 393-ozon-acc.service /etc/systemd/system/
sudo systemctl enable --now 393-ozon-acc
```
