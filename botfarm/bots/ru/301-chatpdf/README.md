...
> ChatPDF без карты и VPN — доступ за минуту

**ID:** `ru-301-chatpdf` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** standard

ChatPDF Доступ — бот по теме «аккаунты ChatPDF». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9300
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/301-chatpdf/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 301-chatpdf.service /etc/systemd/system/
sudo systemctl enable --now 301-chatpdf
```
