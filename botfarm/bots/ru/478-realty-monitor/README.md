# Недвижимость Монитор

> Недвижимость Монитор: уведомления в этот чат в реальном времени

**ID:** `ru-478-realty-monitor` · **Регион:** RU · **Язык:** ru ·
**Модель:** monitor · **Тариф цен:** standard

Недвижимость Монитор — бот по теме «мониторинг объявлений недвижимости». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9477
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/478-realty-monitor/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 478-realty-monitor.service /etc/systemd/system/
sudo systemctl enable --now 478-realty-monitor
```
