...
> Мнемотехники и ежедневные тренировки

**ID:** `ru-042-memory-gym` · **Регион:** RU · **Язык:** ru ·
**Модель:** saas · **Тариф цен:** budget

Тренажёр Памяти — бот по теме «тренировка памяти». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9041
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/042-memory-gym/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 042-memory-gym.service /etc/systemd/system/
sudo systemctl enable --now 042-memory-gym
```
