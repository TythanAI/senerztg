...
> Netflix дешевле официальной подписки, выдача сразу

**ID:** `ru-244-netflix` · **Регион:** RU · **Язык:** ru ·
**Модель:** accounts · **Тариф цен:** budget

Netflix Подписки — бот по теме «аккаунты Netflix». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/244-netflix/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 244-netflix.service /etc/systemd/system/
sudo systemctl enable --now 244-netflix
```
