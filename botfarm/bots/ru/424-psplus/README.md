...
> PS Plus: ключи и коды с моментальной выдачей

**ID:** `ru-424-psplus` · **Регион:** RU · **Язык:** ru ·
**Модель:** keys · **Тариф цен:** standard

PS Plus — бот по теме «подписки PS Plus». Каталог с тарифами, приём оплаты картой, через СБП и криптовалютой, автоматическая выдача после оплаты, партнёрская программа и админ-панель с аналитикой.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9423
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/ru/424-psplus/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 424-psplus.service /etc/systemd/system/
sudo systemctl enable --now 424-psplus
```
