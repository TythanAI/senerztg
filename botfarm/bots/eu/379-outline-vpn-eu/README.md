...
> Outline: fast delivery, replaced if anything fails

**ID:** `eu-379-outline-vpn-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Outline Access — a Outline privacy access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9878
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/379-outline-vpn-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 379-outline-vpn-eu.service /etc/systemd/system/
sudo systemctl enable --now 379-outline-vpn-eu
```
