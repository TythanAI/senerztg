...
> League of Legends accounts — instant delivery, warranty included

**ID:** `eu-238-league-legends-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

League of Legends Accounts — a League of Legends game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9737
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/238-league-legends-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 238-league-legends-eu.service /etc/systemd/system/
sudo systemctl enable --now 238-league-legends-eu
```
