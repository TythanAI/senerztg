...
> Headway course access at a fraction of the price

**ID:** `eu-199-headway-app-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Headway Access — a Headway summaries access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9698
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/199-headway-app-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 199-headway-app-eu.service /etc/systemd/system/
sudo systemctl enable --now 199-headway-app-eu
```
