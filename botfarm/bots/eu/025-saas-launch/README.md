...
**ID:** `eu-025-saas-launch` · **Регион:** EU · **Язык:** en ·
**Модель:** course · **Тариф цен:** premium

SaaS Launch Kit — a SaaS launch bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9524
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/025-saas-launch/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 025-saas-launch.service /etc/systemd/system/
sudo systemctl enable --now 025-saas-launch
```
