...
> Midjourney access in under a minute, no card needed

**ID:** `eu-290-midjourney-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Midjourney Access — a Midjourney AI access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9789
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/290-midjourney-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 290-midjourney-eu.service /etc/systemd/system/
sudo systemctl enable --now 290-midjourney-eu
```
