...
> SOCKS5: fast delivery, replaced if anything fails

**ID:** `eu-373-socks5-proxy-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

SOCKS5 Access — a SOCKS5 privacy access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9872
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/373-socks5-proxy-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 373-socks5-proxy-eu.service /etc/systemd/system/
sudo systemctl enable --now 373-socks5-proxy-eu
```
