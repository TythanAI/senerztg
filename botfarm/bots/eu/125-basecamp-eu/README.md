...
> Basecamp access without a company card

**ID:** `eu-125-basecamp-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Basecamp Access — a Basecamp project tool access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9624
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/125-basecamp-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 125-basecamp-eu.service /etc/systemd/system/
sudo systemctl enable --now 125-basecamp-eu
```
