...
> Fortnite accounts — instant delivery, warranty included

**ID:** `eu-242-fortnite-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Fortnite Accounts — a Fortnite game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9741
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/242-fortnite-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 242-fortnite-eu.service /etc/systemd/system/
sudo systemctl enable --now 242-fortnite-eu
```
