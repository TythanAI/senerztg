...
> Job Alerts: real-time alerts straight into this chat

**ID:** `eu-475-job-monitor-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** monitor · **Тариф цен:** standard

Job Alerts Monitor — a Job Alerts alerts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9974
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/475-job-monitor-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 475-job-monitor-eu.service /etc/systemd/system/
sudo systemctl enable --now 475-job-monitor-eu
```
