...
> Twitch: aged accounts with a replacement warranty

**ID:** `eu-330-twitch-channel-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Twitch Accounts — a Twitch social accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9829
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/330-twitch-channel-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 330-twitch-channel-eu.service /etc/systemd/system/
sudo systemctl enable --now 330-twitch-channel-eu
```
