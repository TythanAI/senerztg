...
> HeyGen access in under a minute, no card needed

**ID:** `eu-301-heygen-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

HeyGen Access — a HeyGen AI access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9800
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/301-heygen-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 301-heygen-eu.service /etc/systemd/system/
sudo systemctl enable --now 301-heygen-eu
```
