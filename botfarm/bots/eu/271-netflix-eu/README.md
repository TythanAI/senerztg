...
> Netflix for less than the official plan, delivered instantly

**ID:** `eu-271-netflix-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Netflix Subs — a Netflix subscriptions bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9770
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/271-netflix-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 271-netflix-eu.service /etc/systemd/system/
sudo systemctl enable --now 271-netflix-eu
```
