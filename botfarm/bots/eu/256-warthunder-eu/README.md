...
> War Thunder accounts — instant delivery, warranty included

**ID:** `eu-256-warthunder-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

War Thunder Accounts — a War Thunder game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9755
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/256-warthunder-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 256-warthunder-eu.service /etc/systemd/system/
sudo systemctl enable --now 256-warthunder-eu
```
