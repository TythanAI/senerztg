...
> Perplexity Pro access in under a minute, no card needed

**ID:** `eu-291-perplexity-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Perplexity Pro Access — a Perplexity Pro AI access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9790
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/291-perplexity-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 291-perplexity-eu.service /etc/systemd/system/
sudo systemctl enable --now 291-perplexity-eu
```
