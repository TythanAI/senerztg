...
> CS2 accounts — instant delivery, warranty included

**ID:** `eu-239-cs2-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

CS2 Accounts — a CS2 game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9738
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/239-cs2-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 239-cs2-eu.service /etc/systemd/system/
sudo systemctl enable --now 239-cs2-eu
```
