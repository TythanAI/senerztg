# Coursera Plus Access

> Coursera Plus course access at a fraction of the price

**ID:** `eu-202-coursera-plus-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Coursera Plus Access — a Coursera Plus course access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9701
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/202-coursera-plus-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 202-coursera-plus-eu.service /etc/systemd/system/
sudo systemctl enable --now 202-coursera-plus-eu
```
