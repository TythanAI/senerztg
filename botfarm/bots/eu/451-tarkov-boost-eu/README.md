...
> Tarkov Boosting: done for you, agreed before you pay

**ID:** `eu-451-tarkov-boost-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** standard

Tarkov Boosting — a Tarkov Boosting boosting services bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9950
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/451-tarkov-boost-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 451-tarkov-boost-eu.service /etc/systemd/system/
sudo systemctl enable --now 451-tarkov-boost-eu
```
