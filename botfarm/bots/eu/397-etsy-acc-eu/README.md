...
> Etsy: ready-to-use accounts for sellers

**ID:** `eu-397-etsy-acc-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Etsy Accounts — a Etsy marketplace accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9896
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/397-etsy-acc-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 397-etsy-acc-eu.service /etc/systemd/system/
sudo systemctl enable --now 397-etsy-acc-eu
```
