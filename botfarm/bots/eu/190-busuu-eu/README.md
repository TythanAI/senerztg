...
> Busuu course access at a fraction of the price

**ID:** `eu-190-busuu-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Busuu Access — a Busuu language access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9689
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/190-busuu-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 190-busuu-eu.service /etc/systemd/system/
sudo systemctl enable --now 190-busuu-eu
```
