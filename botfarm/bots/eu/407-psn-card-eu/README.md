...
> PSN Card: keys and codes delivered instantly

**ID:** `eu-407-psn-card-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** keys · **Тариф цен:** standard

PSN Card Keys — a PSN Card keys and gift cards bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9906
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/407-psn-card-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 407-psn-card-eu.service /etc/systemd/system/
sudo systemctl enable --now 407-psn-card-eu
```
