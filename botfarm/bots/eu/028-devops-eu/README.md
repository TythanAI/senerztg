...
> Docker, CI/CD, and IaC in practice

**ID:** `eu-028-devops-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** course · **Тариф цен:** elite

DevOps Pipeline — a DevOps bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9527
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/028-devops-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 028-devops-eu.service /etc/systemd/system/
sudo systemctl enable --now 028-devops-eu
```
