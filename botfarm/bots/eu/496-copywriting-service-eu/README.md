...
> Copywriting: delivered turnkey on a fixed timeline

**ID:** `eu-496-copywriting-service-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** standard

Copywriting — a Copywriting digital services bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9995
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/496-copywriting-service-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 496-copywriting-service-eu.service /etc/systemd/system/
sudo systemctl enable --now 496-copywriting-service-eu
```
