# Ticket Watch Monitor

> Ticket Watch: real-time alerts straight into this chat

**ID:** `eu-473-ticket-monitor-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** monitor · **Тариф цен:** standard

Ticket Watch Monitor — a Ticket Watch alerts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9972
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/473-ticket-monitor-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 473-ticket-monitor-eu.service /etc/systemd/system/
sudo systemctl enable --now 473-ticket-monitor-eu
```
