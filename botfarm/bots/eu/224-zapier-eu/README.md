...
> Zapier without a yearly commitment

**ID:** `eu-224-zapier-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Zapier Access — a Zapier automation access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9723
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/224-zapier-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 224-zapier-eu.service /etc/systemd/system/
sudo systemctl enable --now 224-zapier-eu
```
