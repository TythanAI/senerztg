...
> Intercom access without a company card

**ID:** `eu-114-intercom-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Intercom Access — a Intercom support tool access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9613
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/114-intercom-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 114-intercom-eu.service /etc/systemd/system/
sudo systemctl enable --now 114-intercom-eu
```
