...
> CS2 Boosting: done for you, agreed before you pay

**ID:** `eu-445-cs2-rank-boost-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** standard

CS2 Boosting — a CS2 Boosting boosting services bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9944
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/445-cs2-rank-boost-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 445-cs2-rank-boost-eu.service /etc/systemd/system/
sudo systemctl enable --now 445-cs2-rank-boost-eu
```
