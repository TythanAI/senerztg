...
> MEXC: verified accounts, ready to trade

**ID:** `eu-386-mexc-acc-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** premium

MEXC Accounts — a MEXC exchange accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9885
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/386-mexc-acc-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 386-mexc-acc-eu.service /etc/systemd/system/
sudo systemctl enable --now 386-mexc-acc-eu
```
