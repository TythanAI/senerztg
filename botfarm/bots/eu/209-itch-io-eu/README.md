...
> itch.io: ready accounts, instant delivery

**ID:** `eu-209-itch-io-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

itch.io Accounts — a itch.io store accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9708
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/209-itch-io-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 209-itch-io-eu.service /etc/systemd/system/
sudo systemctl enable --now 209-itch-io-eu
```
