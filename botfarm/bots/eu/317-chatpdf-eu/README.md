...
> ChatPDF access in under a minute, no card needed

**ID:** `eu-317-chatpdf-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

ChatPDF Access — a ChatPDF AI access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9816
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/317-chatpdf-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 317-chatpdf-eu.service /etc/systemd/system/
sudo systemctl enable --now 317-chatpdf-eu
```
