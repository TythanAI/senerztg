...
> 1Password licence keys, delivered instantly

**ID:** `eu-171-1password-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** keys · **Тариф цен:** budget

1Password Keys — a 1Password password manager access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9670
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/171-1password-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 171-1password-eu.service /etc/systemd/system/
sudo systemctl enable --now 171-1password-eu
```
