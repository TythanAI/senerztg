...
> Hotjar access without a company card

**ID:** `eu-110-hotjar-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Hotjar Access — a Hotjar analytics access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9609
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/110-hotjar-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 110-hotjar-eu.service /etc/systemd/system/
sudo systemctl enable --now 110-hotjar-eu
```
