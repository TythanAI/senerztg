...
> OneDrive: clean accounts for work and sign-ups

**ID:** `eu-344-onedrive-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

OneDrive Accounts — a OneDrive email and cloud accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9843
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/344-onedrive-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 344-onedrive-eu.service /etc/systemd/system/
sudo systemctl enable --now 344-onedrive-eu
```
