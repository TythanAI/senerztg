...
> Typeform access without a company card

**ID:** `eu-118-typeform-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Typeform Access — a Typeform form tool access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9617
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/118-typeform-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 118-typeform-eu.service /etc/systemd/system/
sudo systemctl enable --now 118-typeform-eu
```
