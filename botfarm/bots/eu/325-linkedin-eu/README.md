...
> LinkedIn: aged accounts with a replacement warranty

**ID:** `eu-325-linkedin-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

LinkedIn Accounts — a LinkedIn social accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9824
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/325-linkedin-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 325-linkedin-eu.service /etc/systemd/system/
sudo systemctl enable --now 325-linkedin-eu
```
