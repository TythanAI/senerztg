...
> ESET licence keys, delivered instantly

**ID:** `eu-178-eset-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** keys · **Тариф цен:** budget

ESET Keys — a ESET security licences bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9677
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/178-eset-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 178-eset-eu.service /etc/systemd/system/
sudo systemctl enable --now 178-eset-eu
```
