...
**ID:** `eu-061-seo-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** elite

SEO Growth — a SEO bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9560
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/061-seo-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 061-seo-eu.service /etc/systemd/system/
sudo systemctl enable --now 061-seo-eu
```
