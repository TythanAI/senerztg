...
> Weekly premium-selling setups

**ID:** `eu-004-options-income` · **Регион:** EU · **Язык:** en ·
**Модель:** signals · **Тариф цен:** premium

Options Income Desk — a options trading bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9203
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/004-options-income/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 004-options-income.service /etc/systemd/system/
sudo systemctl enable --now 004-options-income
```
