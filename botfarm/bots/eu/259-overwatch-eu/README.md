# Overwatch 2 Accounts

> Overwatch 2 accounts — instant delivery, warranty included

**ID:** `eu-259-overwatch-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Overwatch 2 Accounts — a Overwatch 2 game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9758
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/259-overwatch-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 259-overwatch-eu.service /etc/systemd/system/
sudo systemctl enable --now 259-overwatch-eu
```
