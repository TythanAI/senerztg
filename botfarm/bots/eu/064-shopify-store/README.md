...
> Launch-ready store in 10 days

**ID:** `eu-064-shopify-store` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** premium

Shopify Store Build — a Shopify bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9563
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/064-shopify-store/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 064-shopify-store.service /etc/systemd/system/
sudo systemctl enable --now 064-shopify-store
```
