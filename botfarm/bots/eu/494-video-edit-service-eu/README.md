...
> Video Studio: delivered turnkey on a fixed timeline

**ID:** `eu-494-video-edit-service-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** standard

Video Studio — a Video Studio digital services bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9993
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/494-video-edit-service-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 494-video-edit-service-eu.service /etc/systemd/system/
sudo systemctl enable --now 494-video-edit-service-eu
```
