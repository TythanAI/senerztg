# Discord Boosts Top-Up

> Discord Boosts top-ups at a better rate, credited in 15 minutes

**ID:** `eu-444-discord-boost-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** topup · **Тариф цен:** budget

Discord Boosts Top-Up — a Discord Boosts in-game currency bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9943
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/444-discord-boost-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 444-discord-boost-eu.service /etc/systemd/system/
sudo systemctl enable --now 444-discord-boost-eu
```
