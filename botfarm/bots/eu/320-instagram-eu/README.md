...
> Instagram: aged accounts with a replacement warranty

**ID:** `eu-320-instagram-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Instagram Accounts — a Instagram social accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9819
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/320-instagram-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 320-instagram-eu.service /etc/systemd/system/
sudo systemctl enable --now 320-instagram-eu
```
