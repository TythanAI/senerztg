...
> Logo Design: delivered turnkey on a fixed timeline

**ID:** `eu-493-logo-service-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** standard

Logo Design — a Logo Design digital services bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9992
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/493-logo-service-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 493-logo-service-eu.service /etc/systemd/system/
sudo systemctl enable --now 493-logo-service-eu
```
