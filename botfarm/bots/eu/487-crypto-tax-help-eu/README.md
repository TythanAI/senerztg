...
> Crypto Tax Reports: fast, fair rate, no runaround

**ID:** `eu-487-crypto-tax-help-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** premium

Crypto Tax Reports — a Crypto Tax Reports crypto services bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9986
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/487-crypto-tax-help-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 487-crypto-tax-help-eu.service /etc/systemd/system/
sudo systemctl enable --now 487-crypto-tax-help-eu
```
