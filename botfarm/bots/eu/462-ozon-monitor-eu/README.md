# Retail Price Watch Monitor

> Retail Price Watch: real-time alerts straight into this chat

**ID:** `eu-462-ozon-monitor-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** monitor · **Тариф цен:** standard

Retail Price Watch Monitor — a Retail Price Watch alerts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9961
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/462-ozon-monitor-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 462-ozon-monitor-eu.service /etc/systemd/system/
sudo systemctl enable --now 462-ozon-monitor-eu
```
