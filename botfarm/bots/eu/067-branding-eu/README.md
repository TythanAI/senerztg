...
**ID:** `eu-067-branding-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** premium

Brand Identity — a branding bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9566
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/067-branding-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 067-branding-eu.service /etc/systemd/system/
sudo systemctl enable --now 067-branding-eu
```
