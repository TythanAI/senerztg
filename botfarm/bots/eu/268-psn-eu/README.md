...
> PlayStation Network accounts — instant delivery, warranty included

**ID:** `eu-268-psn-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

PlayStation Network Accounts — a PlayStation Network game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9767
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/268-psn-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 268-psn-eu.service /etc/systemd/system/
sudo systemctl enable --now 268-psn-eu
```
