# Rainbow Six Accounts

> Rainbow Six accounts — instant delivery, warranty included

**ID:** `eu-260-rainbow6-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Rainbow Six Accounts — a Rainbow Six game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9759
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/260-rainbow6-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 260-rainbow6-eu.service /etc/systemd/system/
sudo systemctl enable --now 260-rainbow6-eu
```
