...
**ID:** `eu-092-digital-nomad` · **Регион:** EU · **Язык:** en ·
**Модель:** course · **Тариф цен:** premium

Digital Nomad Setup — a digital nomad life bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/092-digital-nomad/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 092-digital-nomad.service /etc/systemd/system/
sudo systemctl enable --now 092-digital-nomad
```
