# Humble Bundle Accounts

> Humble Bundle: ready accounts, instant delivery

**ID:** `eu-206-humble-bundle-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Humble Bundle Accounts — a Humble Bundle bundle accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9705
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/206-humble-bundle-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 206-humble-bundle-eu.service /etc/systemd/system/
sudo systemctl enable --now 206-humble-bundle-eu
```
