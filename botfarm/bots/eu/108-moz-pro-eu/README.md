...
> Moz Pro access without a company card

**ID:** `eu-108-moz-pro-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Moz Pro Access — a Moz Pro SEO tool access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9607
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/108-moz-pro-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 108-moz-pro-eu.service /etc/systemd/system/
sudo systemctl enable --now 108-moz-pro-eu
```
