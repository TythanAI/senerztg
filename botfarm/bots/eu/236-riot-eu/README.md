...
> Riot Games accounts — instant delivery, warranty included

**ID:** `eu-236-riot-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Riot Games Accounts — a Riot Games game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9735
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/236-riot-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 236-riot-eu.service /etc/systemd/system/
sudo systemctl enable --now 236-riot-eu
```
