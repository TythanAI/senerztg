...
> Azure: ready accounts and credits

**ID:** `eu-128-azure-acc-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Azure Accounts — a Azure cloud accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9627
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/128-azure-acc-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 128-azure-acc-eu.service /etc/systemd/system/
sudo systemctl enable --now 128-azure-acc-eu
```
