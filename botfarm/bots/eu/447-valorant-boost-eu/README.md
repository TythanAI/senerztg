...
> Valorant Boosting: done for you, agreed before you pay

**ID:** `eu-447-valorant-boost-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** standard

Valorant Boosting — a Valorant Boosting boosting services bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9946
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/447-valorant-boost-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 447-valorant-boost-eu.service /etc/systemd/system/
sudo systemctl enable --now 447-valorant-boost-eu
```
