...
> Structured plans for every distance

**ID:** `eu-090-running-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** club · **Тариф цен:** budget

Running Club — a running bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9289

## Каталог

| SKU | Название | Цена | Тип |
|-----|----------|------|-----|
| `month` | One month | 8 € | subscription · 30 дн. |
| `quarter` | Three months | 22 € | subscription · 90 дн. |
| `year` | Twelve months | 71 € | subscription · 365 дн. |
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/090-running-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 090-running-eu.service /etc/systemd/system/
sudo systemctl enable --now 090-running-eu
```
