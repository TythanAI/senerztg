...
> Crypto to Cash: fast, fair rate, no runaround

**ID:** `eu-484-cash-crypto-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** premium

Crypto to Cash — a Crypto to Cash crypto services bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9983
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/484-cash-crypto-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 484-cash-crypto-eu.service /etc/systemd/system/
sudo systemctl enable --now 484-cash-crypto-eu
```
