...
> Umzug planen und buchen

**ID:** `eu-100-umzug-de` · **Регион:** EU · **Язык:** de ·
**Модель:** leadgen · **Тариф цен:** standard

Umzugsservice — Bot rund um Umzüge. Katalog mit Tarifen, Zahlung per Karte und Krypto, automatische Auslieferung, Partnerprogramm und Admin-Panel.
...
|-----|----------|------|-----|
| `audit` | Besichtigung und Angebot | 9 € | consult |
| `package` | Standardauftrag | 45 € | service |
| `turnkey` | Komplettservice | 115 € | service |
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/100-umzug-de/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 100-umzug-de.service /etc/systemd/system/
sudo systemctl enable --now 100-umzug-de
```
