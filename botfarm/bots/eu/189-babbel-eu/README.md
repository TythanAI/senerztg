...
> Babbel course access at a fraction of the price

**ID:** `eu-189-babbel-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Babbel Access — a Babbel language access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9688
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/189-babbel-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 189-babbel-eu.service /etc/systemd/system/
sudo systemctl enable --now 189-babbel-eu
```
