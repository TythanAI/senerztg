...
> Descript access in under a minute, no card needed

**ID:** `eu-303-descript-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Descript Access — a Descript AI access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9802
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/303-descript-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 303-descript-eu.service /etc/systemd/system/
sudo systemctl enable --now 303-descript-eu
```
