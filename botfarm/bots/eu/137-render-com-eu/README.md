...
> Render: ready accounts and credits

**ID:** `eu-137-render-com-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Render Accounts — a Render hosting accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9636
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/137-render-com-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 137-render-com-eu.service /etc/systemd/system/
sudo systemctl enable --now 137-render-com-eu
```
