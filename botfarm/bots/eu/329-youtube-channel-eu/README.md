...
> YouTube: aged accounts with a replacement warranty

**ID:** `eu-329-youtube-channel-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

YouTube Accounts — a YouTube social accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9828
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/329-youtube-channel-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 329-youtube-channel-eu.service /etc/systemd/system/
sudo systemctl enable --now 329-youtube-channel-eu
```
