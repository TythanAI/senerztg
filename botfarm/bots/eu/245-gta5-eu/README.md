...
> GTA V accounts — instant delivery, warranty included

**ID:** `eu-245-gta5-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

GTA V Accounts — a GTA V game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9744
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/245-gta5-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 245-gta5-eu.service /etc/systemd/system/
sudo systemctl enable --now 245-gta5-eu
```
