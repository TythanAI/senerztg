# Frontend Masters Access

> Frontend Masters course access at a fraction of the price

**ID:** `eu-185-frontend-masters-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Frontend Masters Access — a Frontend Masters course access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9684
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/185-frontend-masters-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 185-frontend-masters-eu.service /etc/systemd/system/
sudo systemctl enable --now 185-frontend-masters-eu
```
