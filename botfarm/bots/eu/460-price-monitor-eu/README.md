...
> Price Watch: real-time alerts straight into this chat

**ID:** `eu-460-price-monitor-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** monitor · **Тариф цен:** standard

Price Watch Monitor — a Price Watch alerts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9959
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/460-price-monitor-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 460-price-monitor-eu.service /etc/systemd/system/
sudo systemctl enable --now 460-price-monitor-eu
```
