...
> Grammarly access in under a minute, no card needed

**ID:** `eu-306-grammarly-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Grammarly Access — a Grammarly AI access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9805
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/306-grammarly-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 306-grammarly-eu.service /etc/systemd/system/
sudo systemctl enable --now 306-grammarly-eu
```
