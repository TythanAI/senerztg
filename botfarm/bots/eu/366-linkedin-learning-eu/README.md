...
> LinkedIn Learning at a fraction of the subscription price

**ID:** `eu-366-linkedin-learning-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

LinkedIn Learning Access — a LinkedIn Learning software access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9865
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/366-linkedin-learning-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 366-linkedin-learning-eu.service /etc/systemd/system/
sudo systemctl enable --now 366-linkedin-learning-eu
```
