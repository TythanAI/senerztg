...
> Second and third pillar, explained

**ID:** `eu-008-pension-eu` · **Регион:** EU · **Язык:** de ·
**Модель:** course · **Тариф цен:** standard

Private Pension EU — Bot rund um pension planning. Katalog mit Tarifen, Zahlung per Karte und Krypto, automatische Auslieferung, Partnerprogramm und Admin-Panel.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9507

## Каталог

| SKU | Название | Цена | Тип |
|-----|----------|------|-----|
| `start` | Starter-Paket | 28 € | digital |
| `pro` | Komplettkurs | 77 € | digital |
| `vip` | VIP mit Betreuung | 173 € | consult |
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/008-pension-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 008-pension-eu.service /etc/systemd/system/
sudo systemctl enable --now 008-pension-eu
```
