...
> Filmora for a fraction of the licence price

**ID:** `eu-149-filmora-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Filmora Access — a Filmora editor access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9648
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/149-filmora-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 149-filmora-eu.service /etc/systemd/system/
sudo systemctl enable --now 149-filmora-eu
```
