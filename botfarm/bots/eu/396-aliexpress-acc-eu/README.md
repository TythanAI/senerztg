...
> AliExpress: ready-to-use accounts for sellers

**ID:** `eu-396-aliexpress-acc-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

AliExpress Accounts — a AliExpress marketplace accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9895
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/396-aliexpress-acc-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 396-aliexpress-acc-eu.service /etc/systemd/system/
sudo systemctl enable --now 396-aliexpress-acc-eu
```
