...
> Free Fire accounts — instant delivery, warranty included

**ID:** `eu-254-free-fire-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Free Fire Accounts — a Free Fire game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9753
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/254-free-fire-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 254-free-fire-eu.service /etc/systemd/system/
sudo systemctl enable --now 254-free-fire-eu
```
