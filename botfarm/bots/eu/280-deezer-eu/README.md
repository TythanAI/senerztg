...
> Deezer for less than the official plan, delivered instantly

**ID:** `eu-280-deezer-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Deezer Subs — a Deezer subscriptions bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9779
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/280-deezer-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 280-deezer-eu.service /etc/systemd/system/
sudo systemctl enable --now 280-deezer-eu
```
