...
> Snapchat: aged accounts with a replacement warranty

**ID:** `eu-326-snapchat-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Snapchat Accounts — a Snapchat social accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9825
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/326-snapchat-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 326-snapchat-eu.service /etc/systemd/system/
sudo systemctl enable --now 326-snapchat-eu
```
