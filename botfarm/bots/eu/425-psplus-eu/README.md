...
> PS Plus: keys and codes delivered instantly

**ID:** `eu-425-psplus-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** keys · **Тариф цен:** standard

PS Plus Keys — a PS Plus keys and gift cards bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9924
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/425-psplus-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 425-psplus-eu.service /etc/systemd/system/
sudo systemctl enable --now 425-psplus-eu
```
