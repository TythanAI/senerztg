...
> Cloudflare: ready accounts and credits

**ID:** `eu-133-cloudflare-acc-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Cloudflare Accounts — a Cloudflare CDN accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9632
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/133-cloudflare-acc-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 133-cloudflare-acc-eu.service /etc/systemd/system/
sudo systemctl enable --now 133-cloudflare-acc-eu
```
