# SSL Expiry Watch Monitor

> SSL Expiry Watch: real-time alerts straight into this chat

**ID:** `eu-471-ssl-monitor-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** monitor · **Тариф цен:** standard

SSL Expiry Watch Monitor — a SSL Expiry Watch alerts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9970
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/471-ssl-monitor-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 471-ssl-monitor-eu.service /etc/systemd/system/
sudo systemctl enable --now 471-ssl-monitor-eu
```
