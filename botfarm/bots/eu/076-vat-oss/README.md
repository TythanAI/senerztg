...
> Sell across borders, compliantly

**ID:** `eu-076-vat-oss` · **Регион:** EU · **Язык:** en ·
**Модель:** consult · **Тариф цен:** premium

EU VAT & OSS — a EU VAT bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/076-vat-oss/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 076-vat-oss.service /etc/systemd/system/
sudo systemctl enable --now 076-vat-oss
```
