...
> Build a lazy portfolio that beats most funds

**ID:** `eu-001-index-investing` · **Регион:** EU · **Язык:** en ·
**Модель:** course · **Тариф цен:** standard

Index Investing 101 — a index investing bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9200
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/001-index-investing/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 001-index-investing.service /etc/systemd/system/
sudo systemctl enable --now 001-index-investing
```
