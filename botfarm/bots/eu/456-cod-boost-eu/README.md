...
> CoD Boosting: done for you, agreed before you pay

**ID:** `eu-456-cod-boost-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** standard

CoD Boosting — a CoD Boosting boosting services bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9955
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/456-cod-boost-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 456-cod-boost-eu.service /etc/systemd/system/
sudo systemctl enable --now 456-cod-boost-eu
```
