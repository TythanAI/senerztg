# Classifieds Watch Monitor

> Classifieds Watch: real-time alerts straight into this chat

**ID:** `eu-463-avito-monitor-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** monitor · **Тариф цен:** standard

Classifieds Watch Monitor — a Classifieds Watch alerts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9962
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/463-avito-monitor-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 463-avito-monitor-eu.service /etc/systemd/system/
sudo systemctl enable --now 463-avito-monitor-eu
```
