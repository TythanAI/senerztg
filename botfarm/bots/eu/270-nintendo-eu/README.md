...
> Nintendo accounts — instant delivery, warranty included

**ID:** `eu-270-nintendo-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Nintendo Accounts — a Nintendo game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9769
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/270-nintendo-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 270-nintendo-eu.service /etc/systemd/system/
sudo systemctl enable --now 270-nintendo-eu
```
