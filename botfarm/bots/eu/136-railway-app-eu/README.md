...
> Railway: ready accounts and credits

**ID:** `eu-136-railway-app-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Railway Accounts — a Railway hosting accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9635
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/136-railway-app-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 136-railway-app-eu.service /etc/systemd/system/
sudo systemctl enable --now 136-railway-app-eu
```
