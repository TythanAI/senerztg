...
> Facebook: aged accounts with a replacement warranty

**ID:** `eu-323-facebook-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Facebook Accounts — a Facebook social accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9822
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/323-facebook-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 323-facebook-eu.service /etc/systemd/system/
sudo systemctl enable --now 323-facebook-eu
```
