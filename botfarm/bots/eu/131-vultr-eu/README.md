...
> Vultr: ready accounts and credits

**ID:** `eu-131-vultr-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Vultr Accounts — a Vultr VPS accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9630
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/131-vultr-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 131-vultr-eu.service /etc/systemd/system/
sudo systemctl enable --now 131-vultr-eu
```
