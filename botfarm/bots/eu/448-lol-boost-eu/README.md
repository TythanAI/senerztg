...
> LoL Boosting: done for you, agreed before you pay

**ID:** `eu-448-lol-boost-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** standard

LoL Boosting — a LoL Boosting boosting services bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9947
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/448-lol-boost-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 448-lol-boost-eu.service /etc/systemd/system/
sudo systemctl enable --now 448-lol-boost-eu
```
