...
> Google Drive: clean accounts for work and sign-ups

**ID:** `eu-342-google-drive-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Google Drive Accounts — a Google Drive email and cloud accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9841
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/342-google-drive-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 342-google-drive-eu.service /etc/systemd/system/
sudo systemctl enable --now 342-google-drive-eu
```
