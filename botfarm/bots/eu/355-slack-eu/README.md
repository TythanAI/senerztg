...
> Slack at a fraction of the subscription price

**ID:** `eu-355-slack-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Slack Access — a Slack software access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9854
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/355-slack-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 355-slack-eu.service /etc/systemd/system/
sudo systemctl enable --now 355-slack-eu
```
