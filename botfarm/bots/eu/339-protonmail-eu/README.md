...
> Proton Mail: clean accounts for work and sign-ups

**ID:** `eu-339-protonmail-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Proton Mail Accounts — a Proton Mail email and cloud accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.

## Что уже готово

- Каталог из 3 тарифов с автоматической выдачей после оплаты
- Приём оплаты: stripe, cryptobot, stars, ton, manual
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9838
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/339-protonmail-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 339-protonmail-eu.service /etc/systemd/system/
sudo systemctl enable --now 339-protonmail-eu
```
