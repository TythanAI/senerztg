...
> TikTok top-ups at a better rate, credited in 15 minutes

**ID:** `eu-443-tiktok-coins-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** topup · **Тариф цен:** budget

TikTok Top-Up — a TikTok in-game currency bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9942
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/443-tiktok-coins-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 443-tiktok-coins-eu.service /etc/systemd/system/
sudo systemctl enable --now 443-tiktok-coins-eu
```
