...
> Low-volatility crypto income

**ID:** `eu-021-stablecoin-yield` · **Регион:** EU · **Язык:** en ·
**Модель:** club · **Тариф цен:** standard

Stablecoin Yield — a stablecoin yield bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9520

## Каталог

| SKU | Название | Цена | Тип |
|-----|----------|------|-----|
| `month` | One month | 28 € | subscription · 30 дн. |
| `quarter` | Three months | 74 € | subscription · 90 дн. |
| `year` | Twelve months | 231 € | subscription · 365 дн. |
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/021-stablecoin-yield/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 021-stablecoin-yield.service /etc/systemd/system/
sudo systemctl enable --now 021-stablecoin-yield
```
