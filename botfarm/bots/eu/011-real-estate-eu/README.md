# EU Property Investing

> Buy-to-let across the eurozone

**ID:** `eu-011-real-estate-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** course · **Тариф цен:** premium

EU Property Investing — a property investing bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9210

## Каталог

| SKU | Название | Цена | Тип |
|-----|----------|------|-----|
| `start` | Starter pack | 78 € | digital |
| `pro` | Full course | 212 € | digital |
| `vip` | VIP with coaching | 473 € | consult |
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/011-real-estate-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 011-real-estate-eu.service /etc/systemd/system/
sudo systemctl enable --now 011-real-estate-eu
```
