...
> Wise: ready-to-use accounts for sellers

**ID:** `eu-403-wise-acc-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Wise Accounts — a Wise marketplace accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9902
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/403-wise-acc-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 403-wise-acc-eu.service /etc/systemd/system/
sudo systemctl enable --now 403-wise-acc-eu
```
