...
> Morningstar: research and data access on demand

**ID:** `eu-163-morningstar-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** premium

Morningstar Access — a Morningstar research access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9662
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/163-morningstar-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 163-morningstar-eu.service /etc/systemd/system/
sudo systemctl enable --now 163-morningstar-eu
```
