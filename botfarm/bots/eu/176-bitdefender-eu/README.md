...
> Bitdefender licence keys, delivered instantly

**ID:** `eu-176-bitdefender-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** keys · **Тариф цен:** budget

Bitdefender Keys — a Bitdefender security licences bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9675

## Каталог

| SKU | Название | Цена | Тип |
|-----|----------|------|-----|
| `key1` | 1 key | 8 € | account |
| `key3` | 3 keys | 23 € | account |
| `key10` | 10 keys | 71 € | account |
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/176-bitdefender-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 176-bitdefender-eu.service /etc/systemd/system/
sudo systemctl enable --now 176-bitdefender-eu
```
