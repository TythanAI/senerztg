...
> FUT Coins: done for you, agreed before you pay

**ID:** `eu-454-fifa-coins-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** standard

FUT Coins — a FUT Coins boosting services bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9953
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/454-fifa-coins-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 454-fifa-coins-eu.service /etc/systemd/system/
sudo systemctl enable --now 454-fifa-coins-eu
```
