...
> HubSpot access without a company card

**ID:** `eu-101-hubspot-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

HubSpot Access — a HubSpot CRM access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9600
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/101-hubspot-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 101-hubspot-eu.service /etc/systemd/system/
sudo systemctl enable --now 101-hubspot-eu
```
