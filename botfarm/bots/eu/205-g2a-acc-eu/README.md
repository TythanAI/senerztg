...
> G2A: ready accounts, instant delivery

**ID:** `eu-205-g2a-acc-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

G2A Accounts — a G2A marketplace accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9704
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/205-g2a-acc-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 205-g2a-acc-eu.service /etc/systemd/system/
sudo systemctl enable --now 205-g2a-acc-eu
```
