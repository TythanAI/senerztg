...
> Anthropic API access in under a minute, no card needed

**ID:** `eu-299-anthropic-api-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Anthropic API Access — a Anthropic API AI access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9798
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/299-anthropic-api-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 299-anthropic-api-eu.service /etc/systemd/system/
sudo systemctl enable --now 299-anthropic-api-eu
```
