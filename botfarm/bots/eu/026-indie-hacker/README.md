...
> Build in public with a real cohort

**ID:** `eu-026-indie-hacker` · **Регион:** EU · **Язык:** en ·
**Модель:** club · **Тариф цен:** standard

Indie Hacker Club — a indie hacking bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9525
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/026-indie-hacker/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 026-indie-hacker.service /etc/systemd/system/
sudo systemctl enable --now 026-indie-hacker
```
