...
> Pinterest: aged accounts with a replacement warranty

**ID:** `eu-327-pinterest-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Pinterest Accounts — a Pinterest social accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9826
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/327-pinterest-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 327-pinterest-eu.service /etc/systemd/system/
sudo systemctl enable --now 327-pinterest-eu
```
