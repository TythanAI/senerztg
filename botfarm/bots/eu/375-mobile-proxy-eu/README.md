# Mobile Proxies Access

> Mobile Proxies: fast delivery, replaced if anything fails

**ID:** `eu-375-mobile-proxy-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Mobile Proxies Access — a Mobile Proxies privacy access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9874
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/375-mobile-proxy-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 375-mobile-proxy-eu.service /etc/systemd/system/
sudo systemctl enable --now 375-mobile-proxy-eu
```
