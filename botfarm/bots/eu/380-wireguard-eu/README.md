...
> WireGuard: fast delivery, replaced if anything fails

**ID:** `eu-380-wireguard-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

WireGuard Access — a WireGuard privacy access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9879
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/380-wireguard-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 380-wireguard-eu.service /etc/systemd/system/
sudo systemctl enable --now 380-wireguard-eu
```
