...
> Steam accounts — instant delivery, warranty included

**ID:** `eu-230-steam-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Steam Accounts — a Steam game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9729
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/230-steam-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 230-steam-eu.service /etc/systemd/system/
sudo systemctl enable --now 230-steam-eu
```
