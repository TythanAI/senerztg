...
> Cursor access in under a minute, no card needed

**ID:** `eu-308-cursor-ai-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Cursor Access — a Cursor AI access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9807
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/308-cursor-ai-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 308-cursor-ai-eu.service /etc/systemd/system/
sudo systemctl enable --now 308-cursor-ai-eu
```
