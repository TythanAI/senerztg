...
> Custom Bots: delivered turnkey on a fixed timeline

**ID:** `eu-497-bot-dev-service-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** standard

Custom Bots — a Custom Bots digital services bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9996
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/497-bot-dev-service-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 497-bot-dev-service-eu.service /etc/systemd/system/
sudo systemctl enable --now 497-bot-dev-service-eu
```
