...
> Pika access in under a minute, no card needed

**ID:** `eu-311-pika-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Pika Access — a Pika AI access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9810
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/311-pika-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 311-pika-eu.service /etc/systemd/system/
sudo systemctl enable --now 311-pika-eu
```
