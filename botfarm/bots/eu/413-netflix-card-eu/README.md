...
> Netflix Card: keys and codes delivered instantly

**ID:** `eu-413-netflix-card-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** keys · **Тариф цен:** standard

Netflix Card Keys — a Netflix Card keys and gift cards bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9912
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/413-netflix-card-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 413-netflix-card-eu.service /etc/systemd/system/
sudo systemctl enable --now 413-netflix-card-eu
```
