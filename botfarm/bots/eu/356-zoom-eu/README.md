...
> Zoom at a fraction of the subscription price

**ID:** `eu-356-zoom-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Zoom Access — a Zoom software access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9855
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/356-zoom-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 356-zoom-eu.service /etc/systemd/system/
sudo systemctl enable --now 356-zoom-eu
```
