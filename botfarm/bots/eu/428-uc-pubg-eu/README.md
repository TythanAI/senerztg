...
> PUBG UC top-ups at a better rate, credited in 15 minutes

**ID:** `eu-428-uc-pubg-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** topup · **Тариф цен:** budget

PUBG UC Top-Up — a PUBG UC in-game currency bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9927
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/428-uc-pubg-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 428-uc-pubg-eu.service /etc/systemd/system/
sudo systemctl enable --now 428-uc-pubg-eu
```
