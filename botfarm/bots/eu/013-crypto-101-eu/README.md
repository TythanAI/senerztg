...
> Self-custody, exchanges, and risk

**ID:** `eu-013-crypto-101-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** course · **Тариф цен:** standard

Crypto Foundations — a cryptocurrency bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9512
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/013-crypto-101-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 013-crypto-101-eu.service /etc/systemd/system/
sudo systemctl enable --now 013-crypto-101-eu
```
