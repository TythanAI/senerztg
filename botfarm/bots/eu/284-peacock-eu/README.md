...
> Peacock for less than the official plan, delivered instantly

**ID:** `eu-284-peacock-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Peacock Subs — a Peacock subscriptions bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9783
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/284-peacock-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 284-peacock-eu.service /etc/systemd/system/
sudo systemctl enable --now 284-peacock-eu
```
