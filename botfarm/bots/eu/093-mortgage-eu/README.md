...
**ID:** `eu-093-mortgage-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** consult · **Тариф цен:** elite

EU Mortgage Advice — a mortgages bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9592
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/093-mortgage-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 093-mortgage-eu.service /etc/systemd/system/
sudo systemctl enable --now 093-mortgage-eu
```
