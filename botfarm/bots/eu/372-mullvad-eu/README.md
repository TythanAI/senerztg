...
> Mullvad: fast delivery, replaced if anything fails

**ID:** `eu-372-mullvad-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Mullvad Access — a Mullvad privacy access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9871
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/372-mullvad-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 372-mullvad-eu.service /etc/systemd/system/
sudo systemctl enable --now 372-mullvad-eu
```
