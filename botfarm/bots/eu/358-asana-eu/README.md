...
> Asana at a fraction of the subscription price

**ID:** `eu-358-asana-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Asana Access — a Asana software access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9857
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/358-asana-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 358-asana-eu.service /etc/systemd/system/
sudo systemctl enable --now 358-asana-eu
```
