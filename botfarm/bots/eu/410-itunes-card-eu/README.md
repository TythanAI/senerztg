...
> iTunes Card: keys and codes delivered instantly

**ID:** `eu-410-itunes-card-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** keys · **Тариф цен:** standard

iTunes Card Keys — a iTunes Card keys and gift cards bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9909
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/410-itunes-card-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 410-itunes-card-eu.service /etc/systemd/system/
sudo systemctl enable --now 410-itunes-card-eu
```
