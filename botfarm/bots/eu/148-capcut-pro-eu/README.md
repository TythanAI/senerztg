...
> CapCut Pro for a fraction of the licence price

**ID:** `eu-148-capcut-pro-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

CapCut Pro Access — a CapCut Pro editor access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9647
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/148-capcut-pro-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 148-capcut-pro-eu.service /etc/systemd/system/
sudo systemctl enable --now 148-capcut-pro-eu
```
