...
> Suppliers, VAT, and fulfilment

**ID:** `eu-071-dropship-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** course · **Тариф цен:** premium

Dropshipping EU — a dropshipping bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9270
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/071-dropship-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 071-dropship-eu.service /etc/systemd/system/
sudo systemctl enable --now 071-dropship-eu
```
