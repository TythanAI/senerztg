...
> Similarweb access without a company card

**ID:** `eu-109-similarweb-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Similarweb Access — a Similarweb analytics access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9608
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/109-similarweb-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 109-similarweb-eu.service /etc/systemd/system/
sudo systemctl enable --now 109-similarweb-eu
```
