...
> GMX: clean accounts for work and sign-ups

**ID:** `eu-346-gmx-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

GMX Accounts — a GMX email and cloud accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9845
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/346-gmx-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 346-gmx-eu.service /etc/systemd/system/
sudo systemctl enable --now 346-gmx-eu
```
