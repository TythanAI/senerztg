...
> Miro at a fraction of the subscription price

**ID:** `eu-359-miro-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Miro Access — a Miro software access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9858
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/359-miro-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 359-miro-eu.service /etc/systemd/system/
sudo systemctl enable --now 359-miro-eu
```
