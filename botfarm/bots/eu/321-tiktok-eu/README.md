...
> TikTok: aged accounts with a replacement warranty

**ID:** `eu-321-tiktok-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

TikTok Accounts — a TikTok social accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9820
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/321-tiktok-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 321-tiktok-eu.service /etc/systemd/system/
sudo systemctl enable --now 321-tiktok-eu
```
