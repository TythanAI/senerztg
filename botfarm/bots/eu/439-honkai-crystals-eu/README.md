...
> Honkai top-ups at a better rate, credited in 15 minutes

**ID:** `eu-439-honkai-crystals-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** topup · **Тариф цен:** budget

Honkai Top-Up — a Honkai in-game currency bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9938
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/439-honkai-crystals-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 439-honkai-crystals-eu.service /etc/systemd/system/
sudo systemctl enable --now 439-honkai-crystals-eu
```
