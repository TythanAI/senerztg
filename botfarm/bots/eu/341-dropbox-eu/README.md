...
> Dropbox: clean accounts for work and sign-ups

**ID:** `eu-341-dropbox-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Dropbox Accounts — a Dropbox email and cloud accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9840
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/341-dropbox-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 341-dropbox-eu.service /etc/systemd/system/
sudo systemctl enable --now 341-dropbox-eu
```
