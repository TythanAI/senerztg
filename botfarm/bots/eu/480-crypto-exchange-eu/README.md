...
> Crypto Exchange: fast, fair rate, no runaround

**ID:** `eu-480-crypto-exchange-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** premium

Crypto Exchange — a Crypto Exchange crypto services bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9979
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/480-crypto-exchange-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 480-crypto-exchange-eu.service /etc/systemd/system/
sudo systemctl enable --now 480-crypto-exchange-eu
```
