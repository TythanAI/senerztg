...
> EVE Online accounts — instant delivery, warranty included

**ID:** `eu-266-eve-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

EVE Online Accounts — a EVE Online game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9765
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/266-eve-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 266-eve-eu.service /etc/systemd/system/
sudo systemctl enable --now 266-eve-eu
```
