...
> Albion Online accounts — instant delivery, warranty included

**ID:** `eu-265-albion-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Albion Online Accounts — a Albion Online game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9764
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/265-albion-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 265-albion-eu.service /etc/systemd/system/
sudo systemctl enable --now 265-albion-eu
```
