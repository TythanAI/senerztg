...
> Readwise without a yearly commitment

**ID:** `eu-228-readwise-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Readwise Access — a Readwise reading access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9727
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/228-readwise-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 228-readwise-eu.service /etc/systemd/system/
sudo systemctl enable --now 228-readwise-eu
```
