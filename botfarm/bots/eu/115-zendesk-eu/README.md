...
> Zendesk access without a company card

**ID:** `eu-115-zendesk-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Zendesk Access — a Zendesk helpdesk access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9614
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/115-zendesk-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 115-zendesk-eu.service /etc/systemd/system/
sudo systemctl enable --now 115-zendesk-eu
```
