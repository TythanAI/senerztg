...
> Managed search campaigns

**ID:** `eu-060-google-ads` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** elite

Google Ads Management — a Google Ads bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/060-google-ads/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 060-google-ads.service /etc/systemd/system/
sudo systemctl enable --now 060-google-ads
```
