# Tender Alerts Monitor

> Tender Alerts: real-time alerts straight into this chat

**ID:** `eu-476-tender-monitor-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** monitor · **Тариф цен:** standard

Tender Alerts Monitor — a Tender Alerts alerts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9975
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/476-tender-monitor-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 476-tender-monitor-eu.service /etc/systemd/system/
sudo systemctl enable --now 476-tender-monitor-eu
```
