...
**ID:** `eu-009-etf-sparplan` · **Регион:** EU · **Язык:** de ·
**Модель:** course · **Тариф цен:** standard

ETF Sparplan — Bot rund um ETF-Sparpläne. Katalog mit Tarifen, Zahlung per Karte und Krypto, automatische Auslieferung, Partnerprogramm und Admin-Panel.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/009-etf-sparplan/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 009-etf-sparplan.service /etc/systemd/system/
sudo systemctl enable --now 009-etf-sparplan
```
