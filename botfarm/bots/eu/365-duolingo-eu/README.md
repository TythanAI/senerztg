...
> Duolingo at a fraction of the subscription price

**ID:** `eu-365-duolingo-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Duolingo Access — a Duolingo software access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9864
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/365-duolingo-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 365-duolingo-eu.service /etc/systemd/system/
sudo systemctl enable --now 365-duolingo-eu
```
