...
> The Economist: research and data access on demand

**ID:** `eu-168-economist-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** premium

The Economist Access — a The Economist news subscriptions bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9667
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/168-economist-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 168-economist-eu.service /etc/systemd/system/
sudo systemctl enable --now 168-economist-eu
```
