...
> Confluence access without a company card

**ID:** `eu-124-confluence-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Confluence Access — a Confluence wiki access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9623
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/124-confluence-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 124-confluence-eu.service /etc/systemd/system/
sudo systemctl enable --now 124-confluence-eu
```
