...
> Coursera at a fraction of the subscription price

**ID:** `eu-363-coursera-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Coursera Access — a Coursera software access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9862
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/363-coursera-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 363-coursera-eu.service /etc/systemd/system/
sudo systemctl enable --now 363-coursera-eu
```
