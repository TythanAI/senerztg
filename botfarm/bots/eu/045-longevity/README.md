...
> Blood panel analysis and protocol

**ID:** `eu-045-longevity` · **Регион:** EU · **Язык:** en ·
**Модель:** consult · **Тариф цен:** elite

Longevity Review — a longevity bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/045-longevity/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 045-longevity.service /etc/systemd/system/
sudo systemctl enable --now 045-longevity
```
