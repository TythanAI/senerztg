...
> Rust accounts — instant delivery, warranty included

**ID:** `eu-257-rust-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Rust Accounts — a Rust game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9756
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/257-rust-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 257-rust-eu.service /etc/systemd/system/
sudo systemctl enable --now 257-rust-eu
```
