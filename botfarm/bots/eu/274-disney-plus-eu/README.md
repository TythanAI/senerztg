...
> Disney+ for less than the official plan, delivered instantly

**ID:** `eu-274-disney-plus-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Disney+ Subs — a Disney+ subscriptions bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9773
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/274-disney-plus-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 274-disney-plus-eu.service /etc/systemd/system/
sudo systemctl enable --now 274-disney-plus-eu
```
