...
> Discord Nitro: keys and codes delivered instantly

**ID:** `eu-419-discord-nitro-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** keys · **Тариф цен:** standard

Discord Nitro Keys — a Discord Nitro keys and gift cards bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9918
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/419-discord-nitro-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 419-discord-nitro-eu.service /etc/systemd/system/
sudo systemctl enable --now 419-discord-nitro-eu
```
