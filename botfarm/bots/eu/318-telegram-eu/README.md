...
> Telegram: aged accounts with a replacement warranty

**ID:** `eu-318-telegram-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Telegram Accounts — a Telegram social accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9817
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/318-telegram-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 318-telegram-eu.service /etc/systemd/system/
sudo systemctl enable --now 318-telegram-eu
```
