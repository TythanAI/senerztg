...
> PUBG accounts — instant delivery, warranty included

**ID:** `eu-241-pubg-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

PUBG Accounts — a PUBG game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9740
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/241-pubg-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 241-pubg-eu.service /etc/systemd/system/
sudo systemctl enable --now 241-pubg-eu
```
