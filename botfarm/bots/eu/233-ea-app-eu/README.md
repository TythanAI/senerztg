...
> EA App accounts — instant delivery, warranty included

**ID:** `eu-233-ea-app-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

EA App Accounts — a EA App game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9732
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/233-ea-app-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 233-ea-app-eu.service /etc/systemd/system/
sudo systemctl enable --now 233-ea-app-eu
```
