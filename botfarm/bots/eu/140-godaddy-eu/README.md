...
> GoDaddy: ready accounts and credits

**ID:** `eu-140-godaddy-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

GoDaddy Accounts — a GoDaddy domain accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9639
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/140-godaddy-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 140-godaddy-eu.service /etc/systemd/system/
sudo systemctl enable --now 140-godaddy-eu
```
