...
> Apex Legends accounts — instant delivery, warranty included

**ID:** `eu-258-apex-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Apex Legends Accounts — a Apex Legends game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9757
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/258-apex-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 258-apex-eu.service /etc/systemd/system/
sudo systemctl enable --now 258-apex-eu
```
