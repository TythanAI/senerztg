...
> MEGA: clean accounts for work and sign-ups

**ID:** `eu-343-mega-nz-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

MEGA Accounts — a MEGA email and cloud accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9842
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/343-mega-nz-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 343-mega-nz-eu.service /etc/systemd/system/
sudo systemctl enable --now 343-mega-nz-eu
```
