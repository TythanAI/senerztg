...
> Honkai Star Rail accounts — instant delivery, warranty included

**ID:** `eu-248-honkai-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Honkai Star Rail Accounts — a Honkai Star Rail game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9747
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/248-honkai-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 248-honkai-eu.service /etc/systemd/system/
sudo systemctl enable --now 248-honkai-eu
```
