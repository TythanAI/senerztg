...
**ID:** `eu-082-video-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** course · **Тариф цен:** standard

Video Editing — a video editing bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/082-video-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 082-video-eu.service /etc/systemd/system/
sudo systemctl enable --now 082-video-eu
```
