...
> Shelf-ready packaging artwork

**ID:** `eu-077-packaging-design` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** premium

Packaging Design — a packaging design bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/077-packaging-design/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 077-packaging-design.service /etc/systemd/system/
sudo systemctl enable --now 077-packaging-design
```
