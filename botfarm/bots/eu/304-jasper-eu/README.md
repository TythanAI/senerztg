...
> Jasper AI access in under a minute, no card needed

**ID:** `eu-304-jasper-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Jasper AI Access — a Jasper AI AI access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9803
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/304-jasper-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 304-jasper-eu.service /etc/systemd/system/
sudo systemctl enable --now 304-jasper-eu
```
