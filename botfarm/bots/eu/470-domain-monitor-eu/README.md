# Domain Drop Watch Monitor

> Domain Drop Watch: real-time alerts straight into this chat

**ID:** `eu-470-domain-monitor-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** monitor · **Тариф цен:** standard

Domain Drop Watch Monitor — a Domain Drop Watch alerts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9969
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/470-domain-monitor-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 470-domain-monitor-eu.service /etc/systemd/system/
sudo systemctl enable --now 470-domain-monitor-eu
```
