...
> Whale flows and exchange reserves

**ID:** `eu-015-onchain-signals` · **Регион:** EU · **Язык:** en ·
**Модель:** signals · **Тариф цен:** premium

On-Chain Signals — a on-chain analysis bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9514

## Каталог

| SKU | Название | Цена | Тип |
|-----|----------|------|-----|
| `week` | One week | 35 € | subscription · 7 дн. |
| `month` | One month | 110 € | subscription · 30 дн. |
| `quarter` | One quarter | 275 € | subscription · 90 дн. |
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/015-onchain-signals/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 015-onchain-signals.service /etc/systemd/system/
sudo systemctl enable --now 015-onchain-signals
```
