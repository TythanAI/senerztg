...
> HTX: verified accounts, ready to trade

**ID:** `eu-388-htx-acc-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** premium

HTX Accounts — a HTX exchange accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9887
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/388-htx-acc-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 388-htx-acc-eu.service /etc/systemd/system/
sudo systemctl enable --now 388-htx-acc-eu
```
