...
> Retire two decades early on a normal salary

**ID:** `eu-003-fire-plan` · **Регион:** EU · **Язык:** en ·
**Модель:** course · **Тариф цен:** premium

FIRE Blueprint — a financial independence bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9502
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/003-fire-plan/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 003-fire-plan.service /etc/systemd/system/
sudo systemctl enable --now 003-fire-plan
```
