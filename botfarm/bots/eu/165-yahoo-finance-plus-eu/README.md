# Yahoo Finance Plus Access

> Yahoo Finance Plus: research and data access on demand

**ID:** `eu-165-yahoo-finance-plus-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** premium

Yahoo Finance Plus Access — a Yahoo Finance Plus research access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9664

## Каталог

| SKU | Название | Цена | Тип |
|-----|----------|------|-----|
| `basic` | Basic | 78 € | account |
| `standard` | Standard | 141 € | account |
| `premium` | Premium | 268 € | account |
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/165-yahoo-finance-plus-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 165-yahoo-finance-plus-eu.service /etc/systemd/system/
sudo systemctl enable --now 165-yahoo-finance-plus-eu
```
