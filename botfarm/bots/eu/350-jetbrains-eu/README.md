...
> JetBrains at a fraction of the subscription price

**ID:** `eu-350-jetbrains-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

JetBrains Access — a JetBrains software access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9849
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/350-jetbrains-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 350-jetbrains-eu.service /etc/systemd/system/
sudo systemctl enable --now 350-jetbrains-eu
```
