...
> Raycast Pro without a yearly commitment

**ID:** `eu-229-raycast-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Raycast Pro Access — a Raycast Pro productivity access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9728
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/229-raycast-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 229-raycast-eu.service /etc/systemd/system/
sudo systemctl enable --now 229-raycast-eu
```
