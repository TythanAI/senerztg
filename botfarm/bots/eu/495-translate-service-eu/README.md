...
> Translation: delivered turnkey on a fixed timeline

**ID:** `eu-495-translate-service-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** standard

Translation — a Translation digital services bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9994
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/495-translate-service-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 495-translate-service-eu.service /etc/systemd/system/
sudo systemctl enable --now 495-translate-service-eu
```
