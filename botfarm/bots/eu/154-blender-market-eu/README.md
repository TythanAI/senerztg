# Blender Market Access

> Blender Market for a fraction of the licence price

**ID:** `eu-154-blender-market-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Blender Market Access — a Blender Market asset access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9653
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/154-blender-market-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 154-blender-market-eu.service /etc/systemd/system/
sudo systemctl enable --now 154-blender-market-eu
```
