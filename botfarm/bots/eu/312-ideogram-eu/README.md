...
> Ideogram access in under a minute, no card needed

**ID:** `eu-312-ideogram-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Ideogram Access — a Ideogram AI access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9811
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/312-ideogram-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 312-ideogram-eu.service /etc/systemd/system/
sudo systemctl enable --now 312-ideogram-eu
```
