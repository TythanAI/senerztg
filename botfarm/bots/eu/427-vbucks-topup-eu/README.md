...
> V-Bucks top-ups at a better rate, credited in 15 minutes

**ID:** `eu-427-vbucks-topup-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** topup · **Тариф цен:** budget

V-Bucks Top-Up — a V-Bucks in-game currency bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9926
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/427-vbucks-topup-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 427-vbucks-topup-eu.service /etc/systemd/system/
sudo systemctl enable --now 427-vbucks-topup-eu
```
