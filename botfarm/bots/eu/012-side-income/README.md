...
> A tested income experiment every month

**ID:** `eu-012-side-income` · **Регион:** EU · **Язык:** en ·
**Модель:** club · **Тариф цен:** standard

Side Income Lab — a side income bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9511
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/012-side-income/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 012-side-income.service /etc/systemd/system/
sudo systemctl enable --now 012-side-income
```
