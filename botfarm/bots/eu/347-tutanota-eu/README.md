...
> Tuta: clean accounts for work and sign-ups

**ID:** `eu-347-tutanota-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Tuta Accounts — a Tuta email and cloud accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9846
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/347-tutanota-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 347-tutanota-eu.service /etc/systemd/system/
sudo systemctl enable --now 347-tutanota-eu
```
