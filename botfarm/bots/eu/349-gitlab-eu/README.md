...
> GitLab at a fraction of the subscription price

**ID:** `eu-349-gitlab-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

GitLab Access — a GitLab software access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9848
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/349-gitlab-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 349-gitlab-eu.service /etc/systemd/system/
sudo systemctl enable --now 349-gitlab-eu
```
