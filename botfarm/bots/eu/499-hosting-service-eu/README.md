...
> Managed Hosting: delivered turnkey on a fixed timeline

**ID:** `eu-499-hosting-service-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** standard

Managed Hosting — a Managed Hosting digital services bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9998
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/499-hosting-service-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 499-hosting-service-eu.service /etc/systemd/system/
sudo systemctl enable --now 499-hosting-service-eu
```
