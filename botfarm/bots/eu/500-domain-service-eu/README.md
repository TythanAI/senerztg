...
> Domain Registration: delivered turnkey on a fixed timeline

**ID:** `eu-500-domain-service-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** standard

Domain Registration — a Domain Registration digital services bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9999
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/500-domain-service-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 500-domain-service-eu.service /etc/systemd/system/
sudo systemctl enable --now 500-domain-service-eu
```
