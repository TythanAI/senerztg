...
> Artlist for a fraction of the licence price

**ID:** `eu-144-artlist-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Artlist Access — a Artlist music licence access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9643
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/144-artlist-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 144-artlist-eu.service /etc/systemd/system/
sudo systemctl enable --now 144-artlist-eu
```
