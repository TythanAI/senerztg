...
> Steam Wallet top-ups at a better rate, credited in 15 minutes

**ID:** `eu-441-steam-wallet-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** topup · **Тариф цен:** budget

Steam Wallet Top-Up — a Steam Wallet in-game currency bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9940
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/441-steam-wallet-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 441-steam-wallet-eu.service /etc/systemd/system/
sudo systemctl enable --now 441-steam-wallet-eu
```
