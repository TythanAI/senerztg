...
> Paramount+ for less than the official plan, delivered instantly

**ID:** `eu-283-paramount-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Paramount+ Subs — a Paramount+ subscriptions bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9782
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/283-paramount-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 283-paramount-eu.service /etc/systemd/system/
sudo systemctl enable --now 283-paramount-eu
```
