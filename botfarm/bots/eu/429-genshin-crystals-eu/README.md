...
> Genshin top-ups at a better rate, credited in 15 minutes

**ID:** `eu-429-genshin-crystals-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** topup · **Тариф цен:** budget

Genshin Top-Up — a Genshin in-game currency bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9928
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/429-genshin-crystals-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 429-genshin-crystals-eu.service /etc/systemd/system/
sudo systemctl enable --now 429-genshin-crystals-eu
```
