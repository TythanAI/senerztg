# Virtual Numbers Access

> Virtual Numbers: fast delivery, replaced if anything fails

**ID:** `eu-381-sms-numbers-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Virtual Numbers Access — a Virtual Numbers privacy access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9880
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/381-sms-numbers-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 381-sms-numbers-eu.service /etc/systemd/system/
sudo systemctl enable --now 381-sms-numbers-eu
```
