...
> Envato Elements at a fraction of the subscription price

**ID:** `eu-360-envato-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Envato Elements Access — a Envato Elements software access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9859
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/360-envato-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 360-envato-eu.service /etc/systemd/system/
sudo systemctl enable --now 360-envato-eu
```
