...
> 16-week plan with pacing charts

**ID:** `eu-038-marathon-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** course · **Тариф цен:** premium

Marathon Sub-4 — a marathon training bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9237
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/038-marathon-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 038-marathon-eu.service /etc/systemd/system/
sudo systemctl enable --now 038-marathon-eu
```
