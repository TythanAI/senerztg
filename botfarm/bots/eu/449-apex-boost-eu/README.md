...
> Apex Boosting: done for you, agreed before you pay

**ID:** `eu-449-apex-boost-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** standard

Apex Boosting — a Apex Boosting boosting services bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9948
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/449-apex-boost-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 449-apex-boost-eu.service /etc/systemd/system/
sudo systemctl enable --now 449-apex-boost-eu
```
