...
> Openings, tactics, and game reviews

**ID:** `eu-086-chess-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** club · **Тариф цен:** standard

Chess Improvement — a chess bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9585
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/086-chess-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 086-chess-eu.service /etc/systemd/system/
sudo systemctl enable --now 086-chess-eu
```
