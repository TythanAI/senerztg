...
> Leonardo AI access in under a minute, no card needed

**ID:** `eu-297-leonardo-ai-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Leonardo AI Access — a Leonardo AI AI access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9796
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/297-leonardo-ai-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 297-leonardo-ai-eu.service /etc/systemd/system/
sudo systemctl enable --now 297-leonardo-ai-eu
```
