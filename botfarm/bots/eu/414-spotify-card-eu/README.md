...
> Spotify Card: keys and codes delivered instantly

**ID:** `eu-414-spotify-card-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** keys · **Тариф цен:** standard

Spotify Card Keys — a Spotify Card keys and gift cards bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9913
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/414-spotify-card-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 414-spotify-card-eu.service /etc/systemd/system/
sudo systemctl enable --now 414-spotify-card-eu
```
