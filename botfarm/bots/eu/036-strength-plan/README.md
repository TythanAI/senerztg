...
> 12 weeks to a real deadlift

**ID:** `eu-036-strength-plan` · **Регион:** EU · **Язык:** en ·
**Модель:** course · **Тариф цен:** standard

Strength Programme — a strength training bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9535
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/036-strength-plan/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 036-strength-plan.service /etc/systemd/system/
sudo systemctl enable --now 036-strength-plan
```
