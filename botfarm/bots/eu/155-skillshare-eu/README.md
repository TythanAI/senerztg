...
> Skillshare for a fraction of the licence price

**ID:** `eu-155-skillshare-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Skillshare Access — a Skillshare course access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9654
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/155-skillshare-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 155-skillshare-eu.service /etc/systemd/system/
sudo systemctl enable --now 155-skillshare-eu
```
