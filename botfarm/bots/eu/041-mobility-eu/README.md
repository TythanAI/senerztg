...
> Undo eight hours of sitting

**ID:** `eu-041-mobility-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** course · **Тариф цен:** standard

Desk Worker Mobility — a mobility training bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/041-mobility-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 041-mobility-eu.service /etc/systemd/system/
sudo systemctl enable --now 041-mobility-eu
```
