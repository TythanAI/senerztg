...
> 20-minute sessions, no equipment

**ID:** `eu-035-home-hiit` · **Регион:** EU · **Язык:** en ·
**Модель:** club · **Тариф цен:** budget

Home HIIT — a home workouts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9234
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/035-home-hiit/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 035-home-hiit.service /etc/systemd/system/
sudo systemctl enable --now 035-home-hiit
```
