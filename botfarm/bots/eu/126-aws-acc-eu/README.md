...
> AWS: ready accounts and credits

**ID:** `eu-126-aws-acc-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

AWS Accounts — a AWS cloud accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9625
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/126-aws-acc-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 126-aws-acc-eu.service /etc/systemd/system/
sudo systemctl enable --now 126-aws-acc-eu
```
