...
> Make without a yearly commitment

**ID:** `eu-225-make-com-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Make Access — a Make automation access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9724
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/225-make-com-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 225-make-com-eu.service /etc/systemd/system/
sudo systemctl enable --now 225-make-com-eu
```
