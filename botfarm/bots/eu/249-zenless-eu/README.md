...
> Zenless Zone Zero accounts — instant delivery, warranty included

**ID:** `eu-249-zenless-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Zenless Zone Zero Accounts — a Zenless Zone Zero game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9748
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/249-zenless-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 249-zenless-eu.service /etc/systemd/system/
sudo systemctl enable --now 249-zenless-eu
```
