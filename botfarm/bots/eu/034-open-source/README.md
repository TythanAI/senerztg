...
> Land your first maintainer role

**ID:** `eu-034-open-source` · **Регион:** EU · **Язык:** en ·
**Модель:** club · **Тариф цен:** standard

Open Source Path — a open source bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9533
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/034-open-source/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 034-open-source.service /etc/systemd/system/
sudo systemctl enable --now 034-open-source
```
