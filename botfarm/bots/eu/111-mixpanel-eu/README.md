...
> Mixpanel access without a company card

**ID:** `eu-111-mixpanel-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Mixpanel Access — a Mixpanel product analytics access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9610
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/111-mixpanel-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 111-mixpanel-eu.service /etc/systemd/system/
sudo systemctl enable --now 111-mixpanel-eu
```
