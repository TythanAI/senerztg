...
> Pluralsight course access at a fraction of the price

**ID:** `eu-182-pluralsight-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Pluralsight Access — a Pluralsight course access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9681
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/182-pluralsight-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 182-pluralsight-eu.service /etc/systemd/system/
sudo systemctl enable --now 182-pluralsight-eu
```
