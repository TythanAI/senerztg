...
> Apple Music for less than the official plan, delivered instantly

**ID:** `eu-276-apple-music-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Apple Music Subs — a Apple Music subscriptions bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9775
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/276-apple-music-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 276-apple-music-eu.service /etc/systemd/system/
sudo systemctl enable --now 276-apple-music-eu
```
