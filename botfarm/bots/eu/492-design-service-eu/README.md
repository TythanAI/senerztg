...
> Graphic Design: delivered turnkey on a fixed timeline

**ID:** `eu-492-design-service-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** standard

Graphic Design — a Graphic Design digital services bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9991
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/492-design-service-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 492-design-service-eu.service /etc/systemd/system/
sudo systemctl enable --now 492-design-service-eu
```
