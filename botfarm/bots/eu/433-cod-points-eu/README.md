...
> Call of Duty top-ups at a better rate, credited in 15 minutes

**ID:** `eu-433-cod-points-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** topup · **Тариф цен:** budget

Call of Duty Top-Up — a Call of Duty in-game currency bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9932
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/433-cod-points-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 433-cod-points-eu.service /etc/systemd/system/
sudo systemctl enable --now 433-cod-points-eu
```
