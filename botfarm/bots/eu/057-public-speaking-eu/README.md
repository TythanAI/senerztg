...
**ID:** `eu-057-public-speaking-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** course · **Тариф цен:** premium

Speak with Impact — a public speaking bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9556
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/057-public-speaking-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 057-public-speaking-eu.service /etc/systemd/system/
sudo systemctl enable --now 057-public-speaking-eu
```
