...
> Koyfin: research and data access on demand

**ID:** `eu-161-koyfin-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** premium

Koyfin Access — a Koyfin market data access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9660
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/161-koyfin-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 161-koyfin-eu.service /etc/systemd/system/
sudo systemctl enable --now 161-koyfin-eu
```
