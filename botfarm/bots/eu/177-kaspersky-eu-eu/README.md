...
> Kaspersky licence keys, delivered instantly

**ID:** `eu-177-kaspersky-eu-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** keys · **Тариф цен:** budget

Kaspersky Keys — a Kaspersky security licences bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9676
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/177-kaspersky-eu-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 177-kaspersky-eu-eu.service /etc/systemd/system/
sudo systemctl enable --now 177-kaspersky-eu-eu
```
