...
> Roblox accounts — instant delivery, warranty included

**ID:** `eu-243-roblox-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Roblox Accounts — a Roblox game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9742
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/243-roblox-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 243-roblox-eu.service /etc/systemd/system/
sudo systemctl enable --now 243-roblox-eu
```
