...
> TradingView: research and data access on demand

**ID:** `eu-160-tradingview-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** premium

TradingView Access — a TradingView charting access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9659
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/160-tradingview-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 160-tradingview-eu.service /etc/systemd/system/
sudo systemctl enable --now 160-tradingview-eu
```
