...
> Token Launch: fast, fair rate, no runaround

**ID:** `eu-491-token-launch-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** premium

Token Launch — a Token Launch crypto services bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9990
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/491-token-launch-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 491-token-launch-eu.service /etc/systemd/system/
sudo systemctl enable --now 491-token-launch-eu
```
