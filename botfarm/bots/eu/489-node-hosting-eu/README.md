...
> Node Hosting: fast, fair rate, no runaround

**ID:** `eu-489-node-hosting-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** premium

Node Hosting — a Node Hosting crypto services bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9988
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/489-node-hosting-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 489-node-hosting-eu.service /etc/systemd/system/
sudo systemctl enable --now 489-node-hosting-eu
```
