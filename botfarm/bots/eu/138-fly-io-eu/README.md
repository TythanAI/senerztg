...
> Fly.io: ready accounts and credits

**ID:** `eu-138-fly-io-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Fly.io Accounts — a Fly.io hosting accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9637
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/138-fly-io-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 138-fly-io-eu.service /etc/systemd/system/
sudo systemctl enable --now 138-fly-io-eu
```
