...
> Google Play: keys and codes delivered instantly

**ID:** `eu-411-google-play-card-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** keys · **Тариф цен:** standard

Google Play Keys — a Google Play keys and gift cards bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9910
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/411-google-play-card-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 411-google-play-card-eu.service /etc/systemd/system/
sudo systemctl enable --now 411-google-play-card-eu
```
