...
> Audible for less than the official plan, delivered instantly

**ID:** `eu-287-audible-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Audible Subs — a Audible subscriptions bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9786
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/287-audible-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 287-audible-eu.service /etc/systemd/system/
sudo systemctl enable --now 287-audible-eu
```
