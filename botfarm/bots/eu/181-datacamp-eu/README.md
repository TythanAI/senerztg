...
> DataCamp course access at a fraction of the price

**ID:** `eu-181-datacamp-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

DataCamp Access — a DataCamp course access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9680
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/181-datacamp-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 181-datacamp-eu.service /etc/systemd/system/
sudo systemctl enable --now 181-datacamp-eu
```
