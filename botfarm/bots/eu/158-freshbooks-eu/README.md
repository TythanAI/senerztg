...
> FreshBooks: research and data access on demand

**ID:** `eu-158-freshbooks-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** premium

FreshBooks Access — a FreshBooks invoicing access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9657
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/158-freshbooks-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 158-freshbooks-eu.service /etc/systemd/system/
sudo systemctl enable --now 158-freshbooks-eu
```
