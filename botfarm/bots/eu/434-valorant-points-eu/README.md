...
> Valorant top-ups at a better rate, credited in 15 minutes

**ID:** `eu-434-valorant-points-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** topup · **Тариф цен:** budget

Valorant Top-Up — a Valorant in-game currency bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9933
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/434-valorant-points-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 434-valorant-points-eu.service /etc/systemd/system/
sudo systemctl enable --now 434-valorant-points-eu
```
