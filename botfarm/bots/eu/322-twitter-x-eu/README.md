...
> X (Twitter): aged accounts with a replacement warranty

**ID:** `eu-322-twitter-x-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

X (Twitter) Accounts — a X (Twitter) social accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9821
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/322-twitter-x-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 322-twitter-x-eu.service /etc/systemd/system/
sudo systemctl enable --now 322-twitter-x-eu
```
