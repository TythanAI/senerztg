...
> Coinbase: verified accounts, ready to trade

**ID:** `eu-391-coinbase-acc-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** premium

Coinbase Accounts — a Coinbase exchange accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9890
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/391-coinbase-acc-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 391-coinbase-acc-eu.service /etc/systemd/system/
sudo systemctl enable --now 391-coinbase-acc-eu
```
