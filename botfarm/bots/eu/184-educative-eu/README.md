...
> Educative course access at a fraction of the price

**ID:** `eu-184-educative-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Educative Access — a Educative course access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9683
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/184-educative-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 184-educative-eu.service /etc/systemd/system/
sudo systemctl enable --now 184-educative-eu
```
