...
> Threads: aged accounts with a replacement warranty

**ID:** `eu-328-threads-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Threads Accounts — a Threads social accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9827
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/328-threads-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 328-threads-eu.service /etc/systemd/system/
sudo systemctl enable --now 328-threads-eu
```
