...
> Bluesky: aged accounts with a replacement warranty

**ID:** `eu-335-bluesky-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Bluesky Accounts — a Bluesky social accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9834
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/335-bluesky-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 335-bluesky-eu.service /etc/systemd/system/
sudo systemctl enable --now 335-bluesky-eu
```
