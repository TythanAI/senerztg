...
> ESEA: ready accounts, instant delivery

**ID:** `eu-214-esea-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

ESEA Accounts — a ESEA competitive accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9713
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/214-esea-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 214-esea-eu.service /etc/systemd/system/
sudo systemctl enable --now 214-esea-eu
```
