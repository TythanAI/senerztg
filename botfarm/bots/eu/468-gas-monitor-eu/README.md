# Gas Fee Watch Monitor

> Gas Fee Watch: real-time alerts straight into this chat

**ID:** `eu-468-gas-monitor-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** monitor · **Тариф цен:** standard

Gas Fee Watch Monitor — a Gas Fee Watch alerts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9967
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/468-gas-monitor-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 468-gas-monitor-eu.service /etc/systemd/system/
sudo systemctl enable --now 468-gas-monitor-eu
```
