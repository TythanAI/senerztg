# Roam Research Access

> Roam Research without a yearly commitment

**ID:** `eu-219-roam-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Roam Research Access — a Roam Research notes access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9718
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/219-roam-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 219-roam-eu.service /etc/systemd/system/
sudo systemctl enable --now 219-roam-eu
```
