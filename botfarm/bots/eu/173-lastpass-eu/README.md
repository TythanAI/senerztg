...
> LastPass licence keys, delivered instantly

**ID:** `eu-173-lastpass-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** keys · **Тариф цен:** budget

LastPass Keys — a LastPass password manager access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9672
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/173-lastpass-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 173-lastpass-eu.service /etc/systemd/system/
sudo systemctl enable --now 173-lastpass-eu
```
