...
> Wallet Setup: fast, fair rate, no runaround

**ID:** `eu-486-crypto-wallet-setup-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** premium

Wallet Setup — a Wallet Setup crypto services bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9985
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/486-crypto-wallet-setup-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 486-crypto-wallet-setup-eu.service /etc/systemd/system/
sudo systemctl enable --now 486-crypto-wallet-setup-eu
```
