...
> Uber: ready-to-use accounts for sellers

**ID:** `eu-400-uber-acc-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Uber Accounts — a Uber marketplace accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9899
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/400-uber-acc-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 400-uber-acc-eu.service /etc/systemd/system/
sudo systemctl enable --now 400-uber-acc-eu
```
