# Flight Price Watch Monitor

> Flight Price Watch: real-time alerts straight into this chat

**ID:** `eu-474-flight-monitor-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** monitor · **Тариф цен:** standard

Flight Price Watch Monitor — a Flight Price Watch alerts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9973
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/474-flight-monitor-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 474-flight-monitor-eu.service /etc/systemd/system/
sudo systemctl enable --now 474-flight-monitor-eu
```
