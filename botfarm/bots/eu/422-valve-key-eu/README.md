...
> Valve: keys and codes delivered instantly

**ID:** `eu-422-valve-key-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** keys · **Тариф цен:** standard

Valve Keys — a Valve keys and gift cards bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9921
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/422-valve-key-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 422-valve-key-eu.service /etc/systemd/system/
sudo systemctl enable --now 422-valve-key-eu
```
