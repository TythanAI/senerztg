...
> Genshin Impact accounts — instant delivery, warranty included

**ID:** `eu-247-genshin-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Genshin Impact Accounts — a Genshin Impact game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9746
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/247-genshin-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 247-genshin-eu.service /etc/systemd/system/
sudo systemctl enable --now 247-genshin-eu
```
