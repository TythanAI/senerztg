...
> Práctica diaria con correcciones

**ID:** `eu-052-spanish-es` · **Регион:** EU · **Язык:** es ·
**Модель:** club · **Тариф цен:** budget

Español Conversación — bot sobre español. Catálogo con tarifas, pago con tarjeta y cripto, entrega automática, programa de afiliados y panel de administración.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9251

## Каталог

| SKU | Название | Цена | Тип |
|-----|----------|------|-----|
| `month` | Un mes | 8 € | subscription · 30 дн. |
| `quarter` | Tres meses | 22 € | subscription · 90 дн. |
| `year` | Doce meses | 71 € | subscription · 365 дн. |
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/052-spanish-es/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 052-spanish-es.service /etc/systemd/system/
sudo systemctl enable --now 052-spanish-es
```
