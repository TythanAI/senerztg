...
> Whale Alerts: real-time alerts straight into this chat

**ID:** `eu-467-whale-alerts-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** monitor · **Тариф цен:** standard

Whale Alerts Monitor — a Whale Alerts alerts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9966
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/467-whale-alerts-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 467-whale-alerts-eu.service /etc/systemd/system/
sudo systemctl enable --now 467-whale-alerts-eu
```
