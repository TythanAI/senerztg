...
> Stripe Atlas: research and data access on demand

**ID:** `eu-159-stripe-atlas-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** premium

Stripe Atlas Access — a Stripe Atlas incorporation help bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9658
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/159-stripe-atlas-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 159-stripe-atlas-eu.service /etc/systemd/system/
sudo systemctl enable --now 159-stripe-atlas-eu
```
