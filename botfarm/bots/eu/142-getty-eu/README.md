...
> Getty Images for a fraction of the licence price

**ID:** `eu-142-getty-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Getty Images Access — a Getty Images stock media access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9641
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/142-getty-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 142-getty-eu.service /etc/systemd/system/
sudo systemctl enable --now 142-getty-eu
```
