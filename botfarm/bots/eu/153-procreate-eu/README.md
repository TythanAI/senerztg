...
> Procreate for a fraction of the licence price

**ID:** `eu-153-procreate-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Procreate Access — a Procreate app licences bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9652
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/153-procreate-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 153-procreate-eu.service /etc/systemd/system/
sudo systemctl enable --now 153-procreate-eu
```
