...
> Evernote without a yearly commitment

**ID:** `eu-216-evernote-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Evernote Access — a Evernote notes access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9715
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/216-evernote-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 216-evernote-eu.service /etc/systemd/system/
sudo systemctl enable --now 216-evernote-eu
```
