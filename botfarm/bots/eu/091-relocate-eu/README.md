...
> Visa route, taxes, and settling in

**ID:** `eu-091-relocate-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** consult · **Тариф цен:** elite

Relocation to Europe — a relocation bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/091-relocate-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 091-relocate-eu.service /etc/systemd/system/
sudo systemctl enable --now 091-relocate-eu
```
