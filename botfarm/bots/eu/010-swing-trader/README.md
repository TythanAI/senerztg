...
> Daily swing setups on US equities

**ID:** `eu-010-swing-trader` · **Регион:** EU · **Язык:** en ·
**Модель:** signals · **Тариф цен:** premium

Swing Trader Pro — a swing trading bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9509
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/010-swing-trader/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 010-swing-trader.service /etc/systemd/system/
sudo systemctl enable --now 010-swing-trader
```
