...
**ID:** `eu-087-garden-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** club · **Тариф цен:** budget

Urban Gardening — a urban gardening bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9586
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/087-garden-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 087-garden-eu.service /etc/systemd/system/
sudo systemctl enable --now 087-garden-eu
```
