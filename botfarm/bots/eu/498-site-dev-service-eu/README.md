...
> Web Development: delivered turnkey on a fixed timeline

**ID:** `eu-498-site-dev-service-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** standard

Web Development — a Web Development digital services bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9997
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/498-site-dev-service-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 498-site-dev-service-eu.service /etc/systemd/system/
sudo systemctl enable --now 498-site-dev-service-eu
```
