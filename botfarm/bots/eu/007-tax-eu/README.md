...
> Cross-border tax planning for freelancers

**ID:** `eu-007-tax-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** consult · **Тариф цен:** elite

EU Tax Advisor — a EU taxation bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/007-tax-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 007-tax-eu.service /etc/systemd/system/
sudo systemctl enable --now 007-tax-eu
```
