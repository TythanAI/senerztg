...
> Brawl Stars accounts — instant delivery, warranty included

**ID:** `eu-250-brawl-stars-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Brawl Stars Accounts — a Brawl Stars game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9749
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/250-brawl-stars-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 250-brawl-stars-eu.service /etc/systemd/system/
sudo systemctl enable --now 250-brawl-stars-eu
```
