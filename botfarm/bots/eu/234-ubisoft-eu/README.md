...
> Ubisoft accounts — instant delivery, warranty included

**ID:** `eu-234-ubisoft-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Ubisoft Accounts — a Ubisoft game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9733
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/234-ubisoft-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 234-ubisoft-eu.service /etc/systemd/system/
sudo systemctl enable --now 234-ubisoft-eu
```
