...
> Klaviyo access without a company card

**ID:** `eu-104-klaviyo-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Klaviyo Access — a Klaviyo email marketing access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9603
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/104-klaviyo-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 104-klaviyo-eu.service /etc/systemd/system/
sudo systemctl enable --now 104-klaviyo-eu
```
