...
> Binance: verified accounts, ready to trade

**ID:** `eu-382-binance-acc-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** premium

Binance Accounts — a Binance exchange accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9881
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/382-binance-acc-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 382-binance-acc-eu.service /etc/systemd/system/
sudo systemctl enable --now 382-binance-acc-eu
```
