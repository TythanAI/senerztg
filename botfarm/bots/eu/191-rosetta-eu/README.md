...
> Rosetta Stone course access at a fraction of the price

**ID:** `eu-191-rosetta-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Rosetta Stone Access — a Rosetta Stone language access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9690
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/191-rosetta-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 191-rosetta-eu.service /etc/systemd/system/
sudo systemctl enable --now 191-rosetta-eu
```
