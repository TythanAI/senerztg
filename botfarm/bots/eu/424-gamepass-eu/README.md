...
> Game Pass: keys and codes delivered instantly

**ID:** `eu-424-gamepass-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** keys · **Тариф цен:** standard

Game Pass Keys — a Game Pass keys and gift cards bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9923
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/424-gamepass-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 424-gamepass-eu.service /etc/systemd/system/
sudo systemctl enable --now 424-gamepass-eu
```
