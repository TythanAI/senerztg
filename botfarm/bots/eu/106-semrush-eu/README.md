...
> Semrush access without a company card

**ID:** `eu-106-semrush-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Semrush Access — a Semrush SEO tool access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9605
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/106-semrush-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 106-semrush-eu.service /etc/systemd/system/
sudo systemctl enable --now 106-semrush-eu
```
