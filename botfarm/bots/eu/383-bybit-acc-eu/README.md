...
> Bybit: verified accounts, ready to trade

**ID:** `eu-383-bybit-acc-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** premium

Bybit Accounts — a Bybit exchange accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9882
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/383-bybit-acc-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 383-bybit-acc-eu.service /etc/systemd/system/
sudo systemctl enable --now 383-bybit-acc-eu
```
