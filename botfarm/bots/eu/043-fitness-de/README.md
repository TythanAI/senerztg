...
> Training ohne Studio, täglich

**ID:** `eu-043-fitness-de` · **Регион:** EU · **Язык:** de ·
**Модель:** club · **Тариф цен:** budget

Fitness Zuhause — Bot rund um Heimtraining. Katalog mit Tarifen, Zahlung per Karte und Krypto, automatische Auslieferung, Partnerprogramm und Admin-Panel.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9542

## Каталог

| SKU | Название | Цена | Тип |
|-----|----------|------|-----|
| `month` | Ein Monat | 8 € | subscription · 30 дн. |
| `quarter` | Drei Monate | 22 € | subscription · 90 дн. |
| `year` | Zwölf Monate | 71 € | subscription · 365 дн. |
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/043-fitness-de/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 043-fitness-de.service /etc/systemd/system/
sudo systemctl enable --now 043-fitness-de
```
