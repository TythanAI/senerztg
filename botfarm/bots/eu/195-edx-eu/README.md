...
> edX course access at a fraction of the price

**ID:** `eu-195-edx-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

edX Access — a edX course access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9694
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/195-edx-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 195-edx-eu.service /etc/systemd/system/
sudo systemctl enable --now 195-edx-eu
```
