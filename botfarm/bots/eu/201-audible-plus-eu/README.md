...
> Audible Plus course access at a fraction of the price

**ID:** `eu-201-audible-plus-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Audible Plus Access — a Audible Plus audiobook access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9700
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/201-audible-plus-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 201-audible-plus-eu.service /etc/systemd/system/
sudo systemctl enable --now 201-audible-plus-eu
```
