...
> Discord: aged accounts with a replacement warranty

**ID:** `eu-319-discord-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Discord Accounts — a Discord social accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9818
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/319-discord-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 319-discord-eu.service /etc/systemd/system/
sudo systemctl enable --now 319-discord-eu
```
