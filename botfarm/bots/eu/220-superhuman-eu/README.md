...
> Superhuman without a yearly commitment

**ID:** `eu-220-superhuman-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Superhuman Access — a Superhuman email access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9719
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/220-superhuman-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 220-superhuman-eu.service /etc/systemd/system/
sudo systemctl enable --now 220-superhuman-eu
```
