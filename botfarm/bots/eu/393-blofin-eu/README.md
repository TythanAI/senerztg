...
> BloFin: verified accounts, ready to trade

**ID:** `eu-393-blofin-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** premium

BloFin Accounts — a BloFin exchange accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9892
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/393-blofin-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 393-blofin-eu.service /etc/systemd/system/
sudo systemctl enable --now 393-blofin-eu
```
