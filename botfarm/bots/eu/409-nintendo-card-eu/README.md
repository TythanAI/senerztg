...
> Nintendo eShop: keys and codes delivered instantly

**ID:** `eu-409-nintendo-card-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** keys · **Тариф цен:** standard

Nintendo eShop Keys — a Nintendo eShop keys and gift cards bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9908
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/409-nintendo-card-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 409-nintendo-card-eu.service /etc/systemd/system/
sudo systemctl enable --now 409-nintendo-card-eu
```
