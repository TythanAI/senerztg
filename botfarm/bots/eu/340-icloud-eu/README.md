...
> iCloud: clean accounts for work and sign-ups

**ID:** `eu-340-icloud-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

iCloud Accounts — a iCloud email and cloud accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9839
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/340-icloud-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 340-icloud-eu.service /etc/systemd/system/
sudo systemctl enable --now 340-icloud-eu
```
