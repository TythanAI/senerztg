...
> WSJ: research and data access on demand

**ID:** `eu-166-wsj-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** premium

WSJ Access — a WSJ news subscriptions bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9665
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/166-wsj-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 166-wsj-eu.service /etc/systemd/system/
sudo systemctl enable --now 166-wsj-eu
```
