...
> VLESS: fast delivery, replaced if anything fails

**ID:** `eu-378-vless-config-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

VLESS Access — a VLESS privacy access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9877
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/378-vless-config-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 378-vless-config-eu.service /etc/systemd/system/
sudo systemctl enable --now 378-vless-config-eu
```
