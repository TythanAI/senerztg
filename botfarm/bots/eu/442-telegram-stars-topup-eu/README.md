...
> Telegram Stars top-ups at a better rate, credited in 15 minutes

**ID:** `eu-442-telegram-stars-topup-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** topup · **Тариф цен:** budget

Telegram Stars Top-Up — a Telegram Stars in-game currency bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9941
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/442-telegram-stars-topup-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 442-telegram-stars-topup-eu.service /etc/systemd/system/
sudo systemctl enable --now 442-telegram-stars-topup-eu
```
