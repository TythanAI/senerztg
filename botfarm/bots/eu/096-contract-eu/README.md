...
**ID:** `eu-096-contract-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** premium

Contract Review — a contract review bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9595
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/096-contract-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 096-contract-eu.service /etc/systemd/system/
sudo systemctl enable --now 096-contract-eu
```
