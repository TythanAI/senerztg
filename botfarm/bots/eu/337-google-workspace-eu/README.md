...
> Google Workspace: clean accounts for work and sign-ups

**ID:** `eu-337-google-workspace-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Google Workspace Accounts — a Google Workspace email and cloud accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9836
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/337-google-workspace-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 337-google-workspace-eu.service /etc/systemd/system/
sudo systemctl enable --now 337-google-workspace-eu
```
