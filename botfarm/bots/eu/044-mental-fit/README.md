...
> Evidence-based habits for a calmer mind

**ID:** `eu-044-mental-fit` · **Регион:** EU · **Язык:** en ·
**Модель:** club · **Тариф цен:** standard

Mental Fitness — a mental fitness bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9243
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/044-mental-fit/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 044-mental-fit.service /etc/systemd/system/
sudo systemctl enable --now 044-mental-fit
```
