...
> Otter.ai access in under a minute, no card needed

**ID:** `eu-316-otter-ai-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Otter.ai Access — a Otter.ai AI access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9815
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/316-otter-ai-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 316-otter-ai-eu.service /etc/systemd/system/
sudo systemctl enable --now 316-otter-ai-eu
```
