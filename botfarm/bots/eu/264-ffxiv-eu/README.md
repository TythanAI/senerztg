...
> Final Fantasy XIV accounts — instant delivery, warranty included

**ID:** `eu-264-ffxiv-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Final Fantasy XIV Accounts — a Final Fantasy XIV game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9763
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/264-ffxiv-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 264-ffxiv-eu.service /etc/systemd/system/
sudo systemctl enable --now 264-ffxiv-eu
```
