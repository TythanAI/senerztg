...
> Your numbers, recalculated weekly

**ID:** `eu-037-macro-coach` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** premium

Macro Coaching — a nutrition coaching bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/037-macro-coach/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 037-macro-coach.service /etc/systemd/system/
sudo systemctl enable --now 037-macro-coach
```
