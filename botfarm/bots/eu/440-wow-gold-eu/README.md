...
> WoW top-ups at a better rate, credited in 15 minutes

**ID:** `eu-440-wow-gold-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** topup · **Тариф цен:** budget

WoW Top-Up — a WoW in-game currency bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9939
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/440-wow-gold-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 440-wow-gold-eu.service /etc/systemd/system/
sudo systemctl enable --now 440-wow-gold-eu
```
