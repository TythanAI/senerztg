...
> Stable Diffusion access in under a minute, no card needed

**ID:** `eu-300-stable-diffusion-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Stable Diffusion Access — a Stable Diffusion AI access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9799
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/300-stable-diffusion-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 300-stable-diffusion-eu.service /etc/systemd/system/
sudo systemctl enable --now 300-stable-diffusion-eu
```
