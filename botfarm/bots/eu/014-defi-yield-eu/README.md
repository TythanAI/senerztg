...
> Audited yield strategies, weekly

**ID:** `eu-014-defi-yield-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** club · **Тариф цен:** premium

DeFi Yield Room — a DeFi yield bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/014-defi-yield-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 014-defi-yield-eu.service /etc/systemd/system/
sudo systemctl enable --now 014-defi-yield-eu
```
