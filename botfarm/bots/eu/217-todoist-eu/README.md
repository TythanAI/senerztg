...
> Todoist without a yearly commitment

**ID:** `eu-217-todoist-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Todoist Access — a Todoist task manager access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9716
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/217-todoist-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 217-todoist-eu.service /etc/systemd/system/
sudo systemctl enable --now 217-todoist-eu
```
