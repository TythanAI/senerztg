...
> Senior review of your architecture

**ID:** `eu-024-code-mentor` · **Регион:** EU · **Язык:** en ·
**Модель:** consult · **Тариф цен:** premium

Code Mentor — a code mentoring bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/024-code-mentor/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 024-code-mentor.service /etc/systemd/system/
sudo systemctl enable --now 024-code-mentor
```
