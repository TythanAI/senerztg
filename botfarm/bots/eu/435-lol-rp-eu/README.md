...
> League of Legends top-ups at a better rate, credited in 15 minutes

**ID:** `eu-435-lol-rp-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** topup · **Тариф цен:** budget

League of Legends Top-Up — a League of Legends in-game currency bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9934
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/435-lol-rp-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 435-lol-rp-eu.service /etc/systemd/system/
sudo systemctl enable --now 435-lol-rp-eu
```
