...
> Market Data: research and data access on demand

**ID:** `eu-164-bloomberg-lite-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** premium

Market Data Access — a Market Data market data access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9663
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/164-bloomberg-lite-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 164-bloomberg-lite-eu.service /etc/systemd/system/
sudo systemctl enable --now 164-bloomberg-lite-eu
```
