...
> Build plans and part lists

**ID:** `eu-089-van-life` · **Регион:** EU · **Язык:** en ·
**Модель:** shop · **Тариф цен:** standard

Van Conversion Plans — a van conversion bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9288

## Каталог

| SKU | Название | Цена | Тип |
|-----|----------|------|-----|
| `pack-lite` | Mini pack | 16 € | digital |
| `pack-main` | Main pack | 40 € | digital |
| `pack-pro` | Pro pack | 80 € | digital |
| `pack-all` | Everything bundle | 129 € | digital |
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/089-van-life/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 089-van-life.service /etc/systemd/system/
sudo systemctl enable --now 089-van-life
```
