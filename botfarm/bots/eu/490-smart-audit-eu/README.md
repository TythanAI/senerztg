...
> Contract Audits: fast, fair rate, no runaround

**ID:** `eu-490-smart-audit-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** premium

Contract Audits — a Contract Audits crypto services bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9989
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/490-smart-audit-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 490-smart-audit-eu.service /etc/systemd/system/
sudo systemctl enable --now 490-smart-audit-eu
```
