...
> Add five figures to your offer

**ID:** `eu-049-salary-nego` · **Регион:** EU · **Язык:** en ·
**Модель:** course · **Тариф цен:** standard

Salary Negotiation — a salary negotiation bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9548
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/049-salary-nego/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 049-salary-nego.service /etc/systemd/system/
sudo systemctl enable --now 049-salary-nego
```
