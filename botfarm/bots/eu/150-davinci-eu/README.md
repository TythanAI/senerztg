# DaVinci Resolve Access

> DaVinci Resolve for a fraction of the licence price

**ID:** `eu-150-davinci-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

DaVinci Resolve Access — a DaVinci Resolve editor licences bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9649
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/150-davinci-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 150-davinci-eu.service /etc/systemd/system/
sudo systemctl enable --now 150-davinci-eu
```
