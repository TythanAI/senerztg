# Residential Proxies Access

> Residential Proxies: fast delivery, replaced if anything fails

**ID:** `eu-376-residential-proxy-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Residential Proxies Access — a Residential Proxies privacy access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9875

## Каталог

| SKU | Название | Цена | Тип |
|-----|----------|------|-----|
| `basic` | Basic | 8 € | account |
| `standard` | Standard | 15 € | account |
| `premium` | Premium | 30 € | account |
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/376-residential-proxy-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 376-residential-proxy-eu.service /etc/systemd/system/
sudo systemctl enable --now 376-residential-proxy-eu
```
