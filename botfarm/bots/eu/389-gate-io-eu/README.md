...
> Gate.io: verified accounts, ready to trade

**ID:** `eu-389-gate-io-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** premium

Gate.io Accounts — a Gate.io exchange accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9888
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/389-gate-io-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 389-gate-io-eu.service /etc/systemd/system/
sudo systemctl enable --now 389-gate-io-eu
```
