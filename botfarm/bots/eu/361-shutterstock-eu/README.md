...
> Shutterstock at a fraction of the subscription price

**ID:** `eu-361-shutterstock-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Shutterstock Access — a Shutterstock software access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9860
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/361-shutterstock-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 361-shutterstock-eu.service /etc/systemd/system/
sudo systemctl enable --now 361-shutterstock-eu
```
