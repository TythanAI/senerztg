...
> Ship a product without engineers

**ID:** `eu-032-no-code-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** course · **Тариф цен:** standard

No-Code Founder — a no-code bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9531
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/032-no-code-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 032-no-code-eu.service /etc/systemd/system/
sudo systemctl enable --now 032-no-code-eu
```
