...
> ATS-proof CV and LinkedIn profile

**ID:** `eu-047-cv-rewrite` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** standard

CV Rewrite — a CV writing bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/047-cv-rewrite/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 047-cv-rewrite.service /etc/systemd/system/
sudo systemctl enable --now 047-cv-rewrite
```
