...
> Kraken: verified accounts, ready to trade

**ID:** `eu-390-kraken-acc-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** premium

Kraken Accounts — a Kraken exchange accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9889
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/390-kraken-acc-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 390-kraken-acc-eu.service /etc/systemd/system/
sudo systemctl enable --now 390-kraken-acc-eu
```
