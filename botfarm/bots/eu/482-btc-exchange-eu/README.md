...
> BTC Exchange: fast, fair rate, no runaround

**ID:** `eu-482-btc-exchange-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** premium

BTC Exchange — a BTC Exchange crypto services bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9981
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/482-btc-exchange-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 482-btc-exchange-eu.service /etc/systemd/system/
sudo systemctl enable --now 482-btc-exchange-eu
```
