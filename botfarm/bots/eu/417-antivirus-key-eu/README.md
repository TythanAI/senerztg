...
> Antivirus: keys and codes delivered instantly

**ID:** `eu-417-antivirus-key-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** keys · **Тариф цен:** standard

Antivirus Keys — a Antivirus keys and gift cards bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9916
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/417-antivirus-key-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 417-antivirus-key-eu.service /etc/systemd/system/
sudo systemctl enable --now 417-antivirus-key-eu
```
