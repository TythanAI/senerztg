...
> Crunchyroll for less than the official plan, delivered instantly

**ID:** `eu-279-crunchyroll-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Crunchyroll Subs — a Crunchyroll subscriptions bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9778
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/279-crunchyroll-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 279-crunchyroll-eu.service /etc/systemd/system/
sudo systemctl enable --now 279-crunchyroll-eu
```
