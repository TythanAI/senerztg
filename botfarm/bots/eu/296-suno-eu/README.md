...
> Suno access in under a minute, no card needed

**ID:** `eu-296-suno-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Suno Access — a Suno AI access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9795
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/296-suno-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 296-suno-eu.service /etc/systemd/system/
sudo systemctl enable --now 296-suno-eu
```
