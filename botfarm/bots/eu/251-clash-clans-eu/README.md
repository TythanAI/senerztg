...
> Clash of Clans accounts — instant delivery, warranty included

**ID:** `eu-251-clash-clans-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Clash of Clans Accounts — a Clash of Clans game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9750
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/251-clash-clans-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 251-clash-clans-eu.service /etc/systemd/system/
sudo systemctl enable --now 251-clash-clans-eu
```
