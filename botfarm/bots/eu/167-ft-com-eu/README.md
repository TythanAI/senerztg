...
> Financial Times: research and data access on demand

**ID:** `eu-167-ft-com-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** premium

Financial Times Access — a Financial Times news subscriptions bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9666
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/167-ft-com-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 167-ft-com-eu.service /etc/systemd/system/
sudo systemctl enable --now 167-ft-com-eu
```
