# Crypto Alerts Monitor

> Crypto Alerts: real-time alerts straight into this chat

**ID:** `eu-466-crypto-alerts-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** monitor · **Тариф цен:** standard

Crypto Alerts Monitor — a Crypto Alerts alerts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9965
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/466-crypto-alerts-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 466-crypto-alerts-eu.service /etc/systemd/system/
sudo systemctl enable --now 466-crypto-alerts-eu
```
