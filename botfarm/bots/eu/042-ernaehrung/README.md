...
> Individueller Plan für Ihr Ziel

**ID:** `eu-042-ernaehrung` · **Регион:** EU · **Язык:** de ·
**Модель:** course · **Тариф цен:** standard

Ernährungsplan — Bot rund um Ernährung. Katalog mit Tarifen, Zahlung per Karte und Krypto, automatische Auslieferung, Partnerprogramm und Admin-Panel.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9241
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/042-ernaehrung/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 042-ernaehrung.service /etc/systemd/system/
sudo systemctl enable --now 042-ernaehrung
```
