# Grammarly Business Access

> Grammarly Business without a yearly commitment

**ID:** `eu-221-grammarly-biz-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Grammarly Business Access — a Grammarly Business writing access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9720
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/221-grammarly-biz-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 221-grammarly-biz-eu.service /etc/systemd/system/
sudo systemctl enable --now 221-grammarly-biz-eu
```
