...
> Luma AI access in under a minute, no card needed

**ID:** `eu-310-luma-ai-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Luma AI Access — a Luma AI AI access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9809
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/310-luma-ai-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 310-luma-ai-eu.service /etc/systemd/system/
sudo systemctl enable --now 310-luma-ai-eu
```
