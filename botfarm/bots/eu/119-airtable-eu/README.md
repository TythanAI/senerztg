...
> Airtable access without a company card

**ID:** `eu-119-airtable-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Airtable Access — a Airtable database access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9618
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/119-airtable-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 119-airtable-eu.service /etc/systemd/system/
sudo systemctl enable --now 119-airtable-eu
```
