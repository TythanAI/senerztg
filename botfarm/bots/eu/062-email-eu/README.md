...
> Flows that print money on autopilot

**ID:** `eu-062-email-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** course · **Тариф цен:** standard

Email Revenue — a email marketing bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9561
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/062-email-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 062-email-eu.service /etc/systemd/system/
sudo systemctl enable --now 062-email-eu
```
