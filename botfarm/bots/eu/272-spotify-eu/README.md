...
> Spotify for less than the official plan, delivered instantly

**ID:** `eu-272-spotify-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Spotify Subs — a Spotify subscriptions bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9771
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/272-spotify-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 272-spotify-eu.service /etc/systemd/system/
sudo systemctl enable --now 272-spotify-eu
```
