...
> Affinity for a fraction of the licence price

**ID:** `eu-151-affinity-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Affinity Access — a Affinity design licences bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9650
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/151-affinity-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 151-affinity-eu.service /etc/systemd/system/
sudo systemctl enable --now 151-affinity-eu
```
