# Green Man Gaming Accounts

> Green Man Gaming: ready accounts, instant delivery

**ID:** `eu-208-greenman-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Green Man Gaming Accounts — a Green Man Gaming store accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9707
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/208-greenman-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 208-greenman-eu.service /etc/systemd/system/
sudo systemctl enable --now 208-greenman-eu
```
