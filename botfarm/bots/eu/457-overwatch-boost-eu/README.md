...
> Overwatch Boosting: done for you, agreed before you pay

**ID:** `eu-457-overwatch-boost-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** standard

Overwatch Boosting — a Overwatch Boosting boosting services bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9956
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/457-overwatch-boost-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 457-overwatch-boost-eu.service /etc/systemd/system/
sudo systemctl enable --now 457-overwatch-boost-eu
```
