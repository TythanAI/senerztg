...
> Tenants, rent, and paperwork

**ID:** `eu-094-rental-mgmt` · **Регион:** EU · **Язык:** en ·
**Модель:** saas · **Тариф цен:** standard

Rental Manager — a rental management bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9593
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/094-rental-mgmt/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 094-rental-mgmt.service /etc/systemd/system/
sudo systemctl enable --now 094-rental-mgmt
```
