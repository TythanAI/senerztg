...
> Outlook: clean accounts for work and sign-ups

**ID:** `eu-338-outlook-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Outlook Accounts — a Outlook email and cloud accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9837
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/338-outlook-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 338-outlook-eu.service /etc/systemd/system/
sudo systemctl enable --now 338-outlook-eu
```
