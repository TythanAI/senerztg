...
> Proton VPN: fast delivery, replaced if anything fails

**ID:** `eu-370-protonvpn-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Proton VPN Access — a Proton VPN privacy access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9869
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/370-protonvpn-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 370-protonvpn-eu.service /etc/systemd/system/
sudo systemctl enable --now 370-protonvpn-eu
```
