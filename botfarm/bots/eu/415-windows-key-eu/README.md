...
> Windows: keys and codes delivered instantly

**ID:** `eu-415-windows-key-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** keys · **Тариф цен:** standard

Windows Keys — a Windows keys and gift cards bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9914
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/415-windows-key-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 415-windows-key-eu.service /etc/systemd/system/
sudo systemctl enable --now 415-windows-key-eu
```
