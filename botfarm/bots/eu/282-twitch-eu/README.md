...
> Twitch for less than the official plan, delivered instantly

**ID:** `eu-282-twitch-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Twitch Subs — a Twitch subscriptions bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9781
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/282-twitch-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 282-twitch-eu.service /etc/systemd/system/
sudo systemctl enable --now 282-twitch-eu
```
