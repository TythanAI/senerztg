...
> Verified B2B lots, weekly

**ID:** `eu-079-wholesale-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** club · **Тариф цен:** standard

Wholesale Deals — a wholesale bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9278
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/079-wholesale-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 079-wholesale-eu.service /etc/systemd/system/
sudo systemctl enable --now 079-wholesale-eu
```
