...
> Testnet checklists and wallet hygiene

**ID:** `eu-019-airdrop-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** club · **Тариф цен:** standard

Airdrop Farming — a airdrop farming bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9518
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/019-airdrop-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 019-airdrop-eu.service /etc/systemd/system/
sudo systemctl enable --now 019-airdrop-eu
```
