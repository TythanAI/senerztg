...
> Medium: aged accounts with a replacement warranty

**ID:** `eu-334-medium-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Medium Accounts — a Medium social accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9833
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/334-medium-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 334-medium-eu.service /etc/systemd/system/
sudo systemctl enable --now 334-medium-eu
```
