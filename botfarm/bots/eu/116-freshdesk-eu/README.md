...
> Freshdesk access without a company card

**ID:** `eu-116-freshdesk-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Freshdesk Access — a Freshdesk helpdesk access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9615
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/116-freshdesk-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 116-freshdesk-eu.service /etc/systemd/system/
sudo systemctl enable --now 116-freshdesk-eu
```
