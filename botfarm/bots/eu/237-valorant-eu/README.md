...
> Valorant accounts — instant delivery, warranty included

**ID:** `eu-237-valorant-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Valorant Accounts — a Valorant game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9736
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/237-valorant-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 237-valorant-eu.service /etc/systemd/system/
sudo systemctl enable --now 237-valorant-eu
```
