...
> Monday.com access without a company card

**ID:** `eu-120-monday-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Monday.com Access — a Monday.com project tool access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9619
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/120-monday-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 120-monday-eu.service /etc/systemd/system/
sudo systemctl enable --now 120-monday-eu
```
