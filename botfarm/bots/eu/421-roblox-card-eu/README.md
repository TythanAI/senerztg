...
> Roblox Card: keys and codes delivered instantly

**ID:** `eu-421-roblox-card-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** keys · **Тариф цен:** standard

Roblox Card Keys — a Roblox Card keys and gift cards bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9920
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/421-roblox-card-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 421-roblox-card-eu.service /etc/systemd/system/
sudo systemctl enable --now 421-roblox-card-eu
```
