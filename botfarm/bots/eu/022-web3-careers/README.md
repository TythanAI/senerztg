...
> Vetted roles and grant deadlines

**ID:** `eu-022-web3-careers` · **Регион:** EU · **Язык:** en ·
**Модель:** club · **Тариф цен:** standard

Web3 Careers — a web3 jobs bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9221
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/022-web3-careers/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 022-web3-careers.service /etc/systemd/system/
sudo systemctl enable --now 022-web3-careers
```
