# Obsidian Sync Access

> Obsidian Sync without a yearly commitment

**ID:** `eu-218-obsidian-sync-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Obsidian Sync Access — a Obsidian Sync sync access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9717
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/218-obsidian-sync-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 218-obsidian-sync-eu.service /etc/systemd/system/
sudo systemctl enable --now 218-obsidian-sync-eu
```
