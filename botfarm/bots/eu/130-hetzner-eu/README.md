...
> Hetzner: ready accounts and credits

**ID:** `eu-130-hetzner-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Hetzner Accounts — a Hetzner server accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9629
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/130-hetzner-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 130-hetzner-eu.service /etc/systemd/system/
sudo systemctl enable --now 130-hetzner-eu
```
