...
> Game Keys: keys and codes delivered instantly

**ID:** `eu-418-game-keys-shop-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** keys · **Тариф цен:** standard

Game Keys Keys — a Game Keys keys and gift cards bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9917
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/418-game-keys-shop-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 418-game-keys-shop-eu.service /etc/systemd/system/
sudo systemctl enable --now 418-game-keys-shop-eu
```
