...
> Amazon: ready-to-use accounts for sellers

**ID:** `eu-394-amazon-acc-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Amazon Accounts — a Amazon marketplace accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9893
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/394-amazon-acc-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 394-amazon-acc-eu.service /etc/systemd/system/
sudo systemctl enable --now 394-amazon-acc-eu
```
