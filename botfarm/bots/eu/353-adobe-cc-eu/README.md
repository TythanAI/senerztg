...
> Adobe Creative Cloud at a fraction of the subscription price

**ID:** `eu-353-adobe-cc-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Adobe Creative Cloud Access — a Adobe Creative Cloud software access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9852
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/353-adobe-cc-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 353-adobe-cc-eu.service /etc/systemd/system/
sudo systemctl enable --now 353-adobe-cc-eu
```
