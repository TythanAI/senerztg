...
> Trello at a fraction of the subscription price

**ID:** `eu-357-trello-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Trello Access — a Trello software access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9856
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/357-trello-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 357-trello-eu.service /etc/systemd/system/
sudo systemctl enable --now 357-trello-eu
```
