...
> Zoho Mail: clean accounts for work and sign-ups

**ID:** `eu-345-zoho-mail-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Zoho Mail Accounts — a Zoho Mail email and cloud accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9844
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/345-zoho-mail-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 345-zoho-mail-eu.service /etc/systemd/system/
sudo systemctl enable --now 345-zoho-mail-eu
```
