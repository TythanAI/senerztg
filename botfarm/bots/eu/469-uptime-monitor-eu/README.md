# Uptime Watch Monitor

> Uptime Watch: real-time alerts straight into this chat

**ID:** `eu-469-uptime-monitor-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** monitor · **Тариф цен:** standard

Uptime Watch Monitor — a Uptime Watch alerts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9968
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/469-uptime-monitor-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 469-uptime-monitor-eu.service /etc/systemd/system/
sudo systemctl enable --now 469-uptime-monitor-eu
```
