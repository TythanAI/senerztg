...
> Notion at a fraction of the subscription price

**ID:** `eu-354-notion-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Notion Access — a Notion software access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9853
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/354-notion-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 354-notion-eu.service /etc/systemd/system/
sudo systemctl enable --now 354-notion-eu
```
