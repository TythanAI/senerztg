...
> Mobile Legends accounts — instant delivery, warranty included

**ID:** `eu-253-mobile-legends-acc-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Mobile Legends Accounts — a Mobile Legends game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9752
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/253-mobile-legends-acc-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 253-mobile-legends-acc-eu.service /etc/systemd/system/
sudo systemctl enable --now 253-mobile-legends-acc-eu
```
