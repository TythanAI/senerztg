# Property Alerts Monitor

> Property Alerts: real-time alerts straight into this chat

**ID:** `eu-478-realty-monitor-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** monitor · **Тариф цен:** standard

Property Alerts Monitor — a Property Alerts alerts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9977
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/478-realty-monitor-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 478-realty-monitor-eu.service /etc/systemd/system/
sudo systemctl enable --now 478-realty-monitor-eu
```
