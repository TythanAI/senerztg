...
> Loom without a yearly commitment

**ID:** `eu-222-loom-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Loom Access — a Loom video messaging access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9721
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/222-loom-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 222-loom-eu.service /etc/systemd/system/
sudo systemctl enable --now 222-loom-eu
```
