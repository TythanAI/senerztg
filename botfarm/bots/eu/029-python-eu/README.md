...
**ID:** `eu-029-python-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** course · **Тариф цен:** standard

Python for Work — a Python automation bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/029-python-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 029-python-eu.service /etc/systemd/system/
sudo systemctl enable --now 029-python-eu
```
