...
> Khan Academy course access at a fraction of the price

**ID:** `eu-194-khan-plus-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Khan Academy Access — a Khan Academy course access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9693
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/194-khan-plus-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 194-khan-plus-eu.service /etc/systemd/system/
sudo systemctl enable --now 194-khan-plus-eu
```
