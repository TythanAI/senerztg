...
> Red Dead Redemption 2 accounts — instant delivery, warranty included

**ID:** `eu-246-rdr2-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Red Dead Redemption 2 Accounts — a Red Dead Redemption 2 game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9745
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/246-rdr2-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 246-rdr2-eu.service /etc/systemd/system/
sudo systemctl enable --now 246-rdr2-eu
```
