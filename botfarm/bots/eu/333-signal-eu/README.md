...
> Signal: aged accounts with a replacement warranty

**ID:** `eu-333-signal-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Signal Accounts — a Signal social accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9832
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/333-signal-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 333-signal-eu.service /etc/systemd/system/
sudo systemctl enable --now 333-signal-eu
```
