...
> Destiny 2 accounts — instant delivery, warranty included

**ID:** `eu-262-destiny2-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Destiny 2 Accounts — a Destiny 2 game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9761
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/262-destiny2-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 262-destiny2-eu.service /etc/systemd/system/
sudo systemctl enable --now 262-destiny2-eu
```
