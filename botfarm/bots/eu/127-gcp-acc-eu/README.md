...
> Google Cloud: ready accounts and credits

**ID:** `eu-127-gcp-acc-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Google Cloud Accounts — a Google Cloud cloud accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9626
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/127-gcp-acc-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 127-gcp-acc-eu.service /etc/systemd/system/
sudo systemctl enable --now 127-gcp-acc-eu
```
