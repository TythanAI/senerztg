# ActiveCampaign Access

> ActiveCampaign access without a company card

**ID:** `eu-105-activecampaign-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

ActiveCampaign Access — a ActiveCampaign automation access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9604
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/105-activecampaign-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 105-activecampaign-eu.service /etc/systemd/system/
sudo systemctl enable --now 105-activecampaign-eu
```
