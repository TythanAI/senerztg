# Restock Alerts Monitor

> Restock Alerts: real-time alerts straight into this chat

**ID:** `eu-477-stock-monitor-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** monitor · **Тариф цен:** standard

Restock Alerts Monitor — a Restock Alerts alerts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9976
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/477-stock-monitor-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 477-stock-monitor-eu.service /etc/systemd/system/
sudo systemctl enable --now 477-stock-monitor-eu
```
