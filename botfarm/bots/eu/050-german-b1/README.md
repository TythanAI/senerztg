...
> Pass B1 in four months

**ID:** `eu-050-german-b1` · **Регион:** EU · **Язык:** de ·
**Модель:** course · **Тариф цен:** standard

German B1 Sprint — Bot rund um German language. Katalog mit Tarifen, Zahlung per Karte und Krypto, automatische Auslieferung, Partnerprogramm und Admin-Panel.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9549
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/050-german-b1/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 050-german-b1.service /etc/systemd/system/
sudo systemctl enable --now 050-german-b1
```
