...
> LeetCode Premium course access at a fraction of the price

**ID:** `eu-186-leetcode-premium-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

LeetCode Premium Access — a LeetCode Premium practice access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9685
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/186-leetcode-premium-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 186-leetcode-premium-eu.service /etc/systemd/system/
sudo systemctl enable --now 186-leetcode-premium-eu
```
