# Amazon Gift Card Keys

> Amazon Gift Card: keys and codes delivered instantly

**ID:** `eu-412-amazon-card-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** keys · **Тариф цен:** standard

Amazon Gift Card Keys — a Amazon Gift Card keys and gift cards bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9911
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/412-amazon-card-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 412-amazon-card-eu.service /etc/systemd/system/
sudo systemctl enable --now 412-amazon-card-eu
```
