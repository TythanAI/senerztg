...
> Fix your sleep in three weeks

**ID:** `eu-040-sleep-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** course · **Тариф цен:** budget

Sleep Protocol — a sleep improvement bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/040-sleep-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 040-sleep-eu.service /etc/systemd/system/
sudo systemctl enable --now 040-sleep-eu
```
