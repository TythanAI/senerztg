...
> Destiny 2 Carries: done for you, agreed before you pay

**ID:** `eu-453-destiny-carry-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** standard

Destiny 2 Carries — a Destiny 2 Carries boosting services bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9952
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/453-destiny-carry-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 453-destiny-carry-eu.service /etc/systemd/system/
sudo systemctl enable --now 453-destiny-carry-eu
```
