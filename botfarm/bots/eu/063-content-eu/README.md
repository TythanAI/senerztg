...
> Plan a quarter in an afternoon

**ID:** `eu-063-content-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** saas · **Тариф цен:** standard

Content Calendar — a content marketing bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9562

## Каталог

| SKU | Название | Цена | Тип |
|-----|----------|------|-----|
| `lite` | Lite | 22 € | subscription · 30 дн. |
| `pro` | Pro | 57 € | subscription · 30 дн. |
| `team` | Team | 144 € | subscription · 30 дн. |
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/063-content-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 063-content-eu.service /etc/systemd/system/
sudo systemctl enable --now 063-content-eu
```
