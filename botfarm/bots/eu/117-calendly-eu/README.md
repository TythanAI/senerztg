...
> Calendly access without a company card

**ID:** `eu-117-calendly-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Calendly Access — a Calendly scheduling access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9616
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/117-calendly-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 117-calendly-eu.service /etc/systemd/system/
sudo systemctl enable --now 117-calendly-eu
```
