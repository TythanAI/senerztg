...
> Freepik at a fraction of the subscription price

**ID:** `eu-362-freepik-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Freepik Access — a Freepik software access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9861
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/362-freepik-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 362-freepik-eu.service /etc/systemd/system/
sudo systemctl enable --now 362-freepik-eu
```
