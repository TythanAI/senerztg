...
> Bitget: verified accounts, ready to trade

**ID:** `eu-387-bitget-acc-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** premium

Bitget Accounts — a Bitget exchange accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9886
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/387-bitget-acc-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 387-bitget-acc-eu.service /etc/systemd/system/
sudo systemctl enable --now 387-bitget-acc-eu
```
