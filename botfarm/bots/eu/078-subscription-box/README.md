...
> Recurring revenue from physical goods

**ID:** `eu-078-subscription-box` · **Регион:** EU · **Язык:** en ·
**Модель:** course · **Тариф цен:** standard

Subscription Box — a subscription boxes bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/078-subscription-box/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 078-subscription-box.service /etc/systemd/system/
sudo systemctl enable --now 078-subscription-box
```
