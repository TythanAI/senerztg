...
> Amazon Prime for less than the official plan, delivered instantly

**ID:** `eu-278-amazon-prime-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Amazon Prime Subs — a Amazon Prime subscriptions bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9777
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/278-amazon-prime-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 278-amazon-prime-eu.service /etc/systemd/system/
sudo systemctl enable --now 278-amazon-prime-eu
```
