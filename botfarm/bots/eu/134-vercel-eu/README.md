...
> Vercel: ready accounts and credits

**ID:** `eu-134-vercel-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Vercel Accounts — a Vercel hosting accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9633
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/134-vercel-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 134-vercel-eu.service /etc/systemd/system/
sudo systemctl enable --now 134-vercel-eu
```
