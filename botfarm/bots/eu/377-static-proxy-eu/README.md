# Static Proxies Access

> Static Proxies: fast delivery, replaced if anything fails

**ID:** `eu-377-static-proxy-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Static Proxies Access — a Static Proxies privacy access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9876
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/377-static-proxy-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 377-static-proxy-eu.service /etc/systemd/system/
sudo systemctl enable --now 377-static-proxy-eu
```
