...
> World of Warcraft accounts — instant delivery, warranty included

**ID:** `eu-263-wow-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

World of Warcraft Accounts — a World of Warcraft game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9762
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/263-wow-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 263-wow-eu.service /etc/systemd/system/
sudo systemctl enable --now 263-wow-eu
```
