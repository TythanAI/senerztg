...
> Vetted suppliers and samples

**ID:** `eu-074-product-source` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** premium

Product Sourcing — a product sourcing bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/074-product-source/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 074-product-source.service /etc/systemd/system/
sudo systemctl enable --now 074-product-source
```
