...
> Microsoft Office: keys and codes delivered instantly

**ID:** `eu-416-office-key-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** keys · **Тариф цен:** standard

Microsoft Office Keys — a Microsoft Office keys and gift cards bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9915
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/416-office-key-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 416-office-key-eu.service /etc/systemd/system/
sudo systemctl enable --now 416-office-key-eu
```
