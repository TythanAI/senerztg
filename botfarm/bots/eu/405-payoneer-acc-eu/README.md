...
> Payoneer: ready-to-use accounts for sellers

**ID:** `eu-405-payoneer-acc-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Payoneer Accounts — a Payoneer marketplace accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9904
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/405-payoneer-acc-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 405-payoneer-acc-eu.service /etc/systemd/system/
sudo systemctl enable --now 405-payoneer-acc-eu
```
