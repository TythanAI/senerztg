...
> Clash of Clans top-ups at a better rate, credited in 15 minutes

**ID:** `eu-437-clash-gems-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** topup · **Тариф цен:** budget

Clash of Clans Top-Up — a Clash of Clans in-game currency bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9936
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/437-clash-gems-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 437-clash-gems-eu.service /etc/systemd/system/
sudo systemctl enable --now 437-clash-gems-eu
```
