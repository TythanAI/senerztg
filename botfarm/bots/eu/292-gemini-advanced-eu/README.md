...
> Gemini Advanced access in under a minute, no card needed

**ID:** `eu-292-gemini-advanced-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Gemini Advanced Access — a Gemini Advanced AI access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9791
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/292-gemini-advanced-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 292-gemini-advanced-eu.service /etc/systemd/system/
sudo systemctl enable --now 292-gemini-advanced-eu
```
