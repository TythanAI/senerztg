...
> Figma at a fraction of the subscription price

**ID:** `eu-351-figma-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Figma Access — a Figma software access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9850
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/351-figma-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 351-figma-eu.service /etc/systemd/system/
sudo systemctl enable --now 351-figma-eu
```
