...
> Clash Royale accounts — instant delivery, warranty included

**ID:** `eu-252-clash-royale-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Clash Royale Accounts — a Clash Royale game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9751
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/252-clash-royale-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 252-clash-royale-eu.service /etc/systemd/system/
sudo systemctl enable --now 252-clash-royale-eu
```
