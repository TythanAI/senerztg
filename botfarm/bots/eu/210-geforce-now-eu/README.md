# GeForce Now Accounts

> GeForce Now: ready accounts, instant delivery

**ID:** `eu-210-geforce-now-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

GeForce Now Accounts — a GeForce Now cloud gaming access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9709
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/210-geforce-now-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 210-geforce-now-eu.service /etc/systemd/system/
sudo systemctl enable --now 210-geforce-now-eu
```
