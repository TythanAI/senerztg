# Skin Price Watch Monitor

> Skin Price Watch: real-time alerts straight into this chat

**ID:** `eu-465-skins-monitor-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** monitor · **Тариф цен:** standard

Skin Price Watch Monitor — a Skin Price Watch alerts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9964
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/465-skins-monitor-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 465-skins-monitor-eu.service /etc/systemd/system/
sudo systemctl enable --now 465-skins-monitor-eu
```
