...
> Udemy at a fraction of the subscription price

**ID:** `eu-364-udemy-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Udemy Access — a Udemy software access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9863
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/364-udemy-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 364-udemy-eu.service /etc/systemd/system/
sudo systemctl enable --now 364-udemy-eu
```
