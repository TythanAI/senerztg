...
> ClickUp access without a company card

**ID:** `eu-121-clickup-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

ClickUp Access — a ClickUp project tool access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9620
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/121-clickup-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 121-clickup-eu.service /etc/systemd/system/
sudo systemctl enable --now 121-clickup-eu
```
