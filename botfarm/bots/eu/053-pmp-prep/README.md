...
**ID:** `eu-053-pmp-prep` · **Регион:** EU · **Язык:** en ·
**Модель:** course · **Тариф цен:** premium

PMP Exam Prep — a PMP certification bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9552
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/053-pmp-prep/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 053-pmp-prep.service /etc/systemd/system/
sudo systemctl enable --now 053-pmp-prep
```
