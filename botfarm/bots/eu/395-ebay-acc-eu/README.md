...
> eBay: ready-to-use accounts for sellers

**ID:** `eu-395-ebay-acc-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

eBay Accounts — a eBay marketplace accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9894
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/395-ebay-acc-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 395-ebay-acc-eu.service /etc/systemd/system/
sudo systemctl enable --now 395-ebay-acc-eu
```
