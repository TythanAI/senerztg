...
> Brawl Stars top-ups at a better rate, credited in 15 minutes

**ID:** `eu-430-brawl-gems-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** topup · **Тариф цен:** budget

Brawl Stars Top-Up — a Brawl Stars in-game currency bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9929
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/430-brawl-gems-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 430-brawl-gems-eu.service /etc/systemd/system/
sudo systemctl enable --now 430-brawl-gems-eu
```
