...
> ExpressVPN: fast delivery, replaced if anything fails

**ID:** `eu-368-expressvpn-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

ExpressVPN Access — a ExpressVPN privacy access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9867
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/368-expressvpn-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 368-expressvpn-eu.service /etc/systemd/system/
sudo systemctl enable --now 368-expressvpn-eu
```
