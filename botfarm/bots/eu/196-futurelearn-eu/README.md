...
> FutureLearn course access at a fraction of the price

**ID:** `eu-196-futurelearn-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

FutureLearn Access — a FutureLearn course access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9695
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/196-futurelearn-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 196-futurelearn-eu.service /etc/systemd/system/
sudo systemctl enable --now 196-futurelearn-eu
```
