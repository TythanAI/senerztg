...
> Words that sell without hype

**ID:** `eu-066-copywriting-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** course · **Тариф цен:** premium

Conversion Copywriting — a copywriting bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9265
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/066-copywriting-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 066-copywriting-eu.service /etc/systemd/system/
sudo systemctl enable --now 066-copywriting-eu
```
