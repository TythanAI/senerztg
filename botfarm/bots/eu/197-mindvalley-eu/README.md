...
> Mindvalley course access at a fraction of the price

**ID:** `eu-197-mindvalley-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Mindvalley Access — a Mindvalley course access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9696
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/197-mindvalley-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 197-mindvalley-eu.service /etc/systemd/system/
sudo systemctl enable --now 197-mindvalley-eu
```
