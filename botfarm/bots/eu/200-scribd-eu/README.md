...
> Scribd course access at a fraction of the price

**ID:** `eu-200-scribd-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Scribd Access — a Scribd library access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9699
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/200-scribd-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 200-scribd-eu.service /etc/systemd/system/
sudo systemctl enable --now 200-scribd-eu
```
