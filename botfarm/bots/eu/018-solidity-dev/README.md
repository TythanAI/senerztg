...
> Ship audited smart contracts

**ID:** `eu-018-solidity-dev` · **Регион:** EU · **Язык:** en ·
**Модель:** course · **Тариф цен:** elite

Solidity Developer — a Solidity bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/018-solidity-dev/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 018-solidity-dev.service /etc/systemd/system/
sudo systemctl enable --now 018-solidity-dev
```
