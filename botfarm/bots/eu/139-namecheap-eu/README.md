...
> Namecheap: ready accounts and credits

**ID:** `eu-139-namecheap-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Namecheap Accounts — a Namecheap domain accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9638
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/139-namecheap-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 139-namecheap-eu.service /etc/systemd/system/
sudo systemctl enable --now 139-namecheap-eu
```
