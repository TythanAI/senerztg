...
> Every euro assigned, every month

**ID:** `eu-006-budget-app` · **Регион:** EU · **Язык:** en ·
**Модель:** saas · **Тариф цен:** budget

Zero-Based Budget — a budgeting bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9205

## Каталог

| SKU | Название | Цена | Тип |
|-----|----------|------|-----|
| `lite` | Lite | 6 € | subscription · 30 дн. |
| `pro` | Pro | 17 € | subscription · 30 дн. |
| `team` | Team | 44 € | subscription · 30 дн. |
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/006-budget-app/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 006-budget-app.service /etc/systemd/system/
sudo systemctl enable --now 006-budget-app
```
