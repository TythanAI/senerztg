...
> FACEIT: ready accounts, instant delivery

**ID:** `eu-213-faceit-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

FACEIT Accounts — a FACEIT competitive accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9712
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/213-faceit-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 213-faceit-eu.service /etc/systemd/system/
sudo systemctl enable --now 213-faceit-eu
```
