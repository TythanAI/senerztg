...
> Krea AI access in under a minute, no card needed

**ID:** `eu-313-krea-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Krea AI Access — a Krea AI AI access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9812
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/313-krea-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 313-krea-eu.service /etc/systemd/system/
sudo systemctl enable --now 313-krea-eu
```
