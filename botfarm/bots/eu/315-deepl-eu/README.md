...
> DeepL Pro access in under a minute, no card needed

**ID:** `eu-315-deepl-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

DeepL Pro Access — a DeepL Pro AI access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9814
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/315-deepl-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 315-deepl-eu.service /etc/systemd/system/
sudo systemctl enable --now 315-deepl-eu
```
