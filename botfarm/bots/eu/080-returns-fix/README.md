...
> Cut return rates by a third

**ID:** `eu-080-returns-fix` · **Регион:** EU · **Язык:** en ·
**Модель:** consult · **Тариф цен:** premium

Returns Reduction — a returns management bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9579
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/080-returns-fix/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 080-returns-fix.service /etc/systemd/system/
sudo systemctl enable --now 080-returns-fix
```
