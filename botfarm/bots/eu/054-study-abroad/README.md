...
> Applications, visas, and funding

**ID:** `eu-054-study-abroad` · **Регион:** EU · **Язык:** en ·
**Модель:** consult · **Тариф цен:** elite

Study Abroad EU — a studying abroad bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9553
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/054-study-abroad/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 054-study-abroad.service /etc/systemd/system/
sudo systemctl enable --now 054-study-abroad
```
