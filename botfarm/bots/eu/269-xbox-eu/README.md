...
> Xbox accounts — instant delivery, warranty included

**ID:** `eu-269-xbox-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Xbox Accounts — a Xbox game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9768
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/269-xbox-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 269-xbox-eu.service /etc/systemd/system/
sudo systemctl enable --now 269-xbox-eu
```
