...
> ElevenLabs access in under a minute, no card needed

**ID:** `eu-295-elevenlabs-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

ElevenLabs Access — a ElevenLabs AI access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9794
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/295-elevenlabs-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 295-elevenlabs-eu.service /etc/systemd/system/
sudo systemctl enable --now 295-elevenlabs-eu
```
