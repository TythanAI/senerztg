...
> WhatsApp: aged accounts with a replacement warranty

**ID:** `eu-331-whatsapp-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

WhatsApp Accounts — a WhatsApp social accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9830
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/331-whatsapp-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 331-whatsapp-eu.service /etc/systemd/system/
sudo systemctl enable --now 331-whatsapp-eu
```
