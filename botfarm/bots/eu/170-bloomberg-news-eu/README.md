...
> Bloomberg: research and data access on demand

**ID:** `eu-170-bloomberg-news-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** premium

Bloomberg Access — a Bloomberg news subscriptions bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9669
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/170-bloomberg-news-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 170-bloomberg-news-eu.service /etc/systemd/system/
sudo systemctl enable --now 170-bloomberg-news-eu
```
