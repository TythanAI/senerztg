...
> BingX: verified accounts, ready to trade

**ID:** `eu-392-bingx-acc-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** premium

BingX Accounts — a BingX exchange accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9891
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/392-bingx-acc-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 392-bingx-acc-eu.service /etc/systemd/system/
sudo systemctl enable --now 392-bingx-acc-eu
```
