...
> Vetted remote roles, daily

**ID:** `eu-056-remote-jobs` · **Регион:** EU · **Язык:** en ·
**Модель:** signals · **Тариф цен:** budget

Remote Job Feed — a remote work bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9255
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/056-remote-jobs/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 056-remote-jobs.service /etc/systemd/system/
sudo systemctl enable --now 056-remote-jobs
```
