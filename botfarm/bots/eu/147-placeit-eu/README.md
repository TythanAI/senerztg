...
> Placeit for a fraction of the licence price

**ID:** `eu-147-placeit-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Placeit Access — a Placeit mockup access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9646
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/147-placeit-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 147-placeit-eu.service /etc/systemd/system/
sudo systemctl enable --now 147-placeit-eu
```
