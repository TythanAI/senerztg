...
> GitHub at a fraction of the subscription price

**ID:** `eu-348-github-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

GitHub Access — a GitHub software access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9847
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/348-github-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 348-github-eu.service /etc/systemd/system/
sudo systemctl enable --now 348-github-eu
```
