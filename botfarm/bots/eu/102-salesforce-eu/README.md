...
> Salesforce access without a company card

**ID:** `eu-102-salesforce-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Salesforce Access — a Salesforce CRM access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9601
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/102-salesforce-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 102-salesforce-eu.service /etc/systemd/system/
sudo systemctl enable --now 102-salesforce-eu
```
