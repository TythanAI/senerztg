...
> Descript Pro without a yearly commitment

**ID:** `eu-223-descript-pro-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Descript Pro Access — a Descript Pro editor access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9722
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/223-descript-pro-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 223-descript-pro-eu.service /etc/systemd/system/
sudo systemctl enable --now 223-descript-pro-eu
```
