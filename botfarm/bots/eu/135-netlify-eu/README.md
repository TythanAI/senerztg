...
> Netlify: ready accounts and credits

**ID:** `eu-135-netlify-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Netlify Accounts — a Netlify hosting accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9634
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/135-netlify-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 135-netlify-eu.service /etc/systemd/system/
sudo systemctl enable --now 135-netlify-eu
```
