...
> Hulu for less than the official plan, delivered instantly

**ID:** `eu-285-hulu-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Hulu Subs — a Hulu subscriptions bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9784
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/285-hulu-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 285-hulu-eu.service /etc/systemd/system/
sudo systemctl enable --now 285-hulu-eu
```
