# Marketplace Price Watch Monitor

> Marketplace Price Watch: real-time alerts straight into this chat

**ID:** `eu-461-wb-monitor-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** monitor · **Тариф цен:** standard

Marketplace Price Watch Monitor — a Marketplace Price Watch alerts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9960

## Каталог

| SKU | Название | Цена | Тип |
|-----|----------|------|-----|
| `week` | One week | 13 € | subscription · 7 дн. |
| `month` | One month | 43 € | subscription · 30 дн. |
| `quarter` | Three months | 103 € | subscription · 90 дн. |
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/461-wb-monitor-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 461-wb-monitor-eu.service /etc/systemd/system/
sudo systemctl enable --now 461-wb-monitor-eu
```
