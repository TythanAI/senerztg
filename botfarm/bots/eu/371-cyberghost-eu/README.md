...
> CyberGhost: fast delivery, replaced if anything fails

**ID:** `eu-371-cyberghost-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

CyberGhost Access — a CyberGhost privacy access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9870
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/371-cyberghost-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 371-cyberghost-eu.service /etc/systemd/system/
sudo systemctl enable --now 371-cyberghost-eu
```
