...
> Hashrate Rental: fast, fair rate, no runaround

**ID:** `eu-488-mining-rent-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** premium

Hashrate Rental — a Hashrate Rental crypto services bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9987
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/488-mining-rent-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 488-mining-rent-eu.service /etc/systemd/system/
sudo systemctl enable --now 488-mining-rent-eu
```
