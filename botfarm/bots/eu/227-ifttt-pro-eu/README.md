...
> IFTTT Pro without a yearly commitment

**ID:** `eu-227-ifttt-pro-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

IFTTT Pro Access — a IFTTT Pro automation access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9726
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/227-ifttt-pro-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 227-ifttt-pro-eu.service /etc/systemd/system/
sudo systemctl enable --now 227-ifttt-pro-eu
```
