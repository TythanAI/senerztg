...
> Ahrefs access without a company card

**ID:** `eu-107-ahrefs-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Ahrefs Access — a Ahrefs SEO tool access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9606
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/107-ahrefs-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 107-ahrefs-eu.service /etc/systemd/system/
sudo systemctl enable --now 107-ahrefs-eu
```
