...
> MasterClass course access at a fraction of the price

**ID:** `eu-188-masterclass-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

MasterClass Access — a MasterClass course access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9687
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/188-masterclass-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 188-masterclass-eu.service /etc/systemd/system/
sudo systemctl enable --now 188-masterclass-eu
```
