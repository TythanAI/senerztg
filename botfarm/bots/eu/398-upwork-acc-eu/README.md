...
> Upwork: ready-to-use accounts for sellers

**ID:** `eu-398-upwork-acc-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Upwork Accounts — a Upwork marketplace accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9897
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/398-upwork-acc-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 398-upwork-acc-eu.service /etc/systemd/system/
sudo systemctl enable --now 398-upwork-acc-eu
```
