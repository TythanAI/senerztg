...
> Motion Array for a fraction of the licence price

**ID:** `eu-146-motion-array-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Motion Array Access — a Motion Array template access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9645
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/146-motion-array-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 146-motion-array-eu.service /etc/systemd/system/
sudo systemctl enable --now 146-motion-array-eu
```
