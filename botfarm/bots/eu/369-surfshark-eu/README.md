...
> Surfshark: fast delivery, replaced if anything fails

**ID:** `eu-369-surfshark-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Surfshark Access — a Surfshark privacy access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9868
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/369-surfshark-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 369-surfshark-eu.service /etc/systemd/system/
sudo systemctl enable --now 369-surfshark-eu
```
