...
> P2P Desk: fast, fair rate, no runaround

**ID:** `eu-485-p2p-service-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** premium

P2P Desk — a P2P Desk crypto services bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9984
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/485-p2p-service-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 485-p2p-service-eu.service /etc/systemd/system/
sudo systemctl enable --now 485-p2p-service-eu
```
