...
> Notion AI access in under a minute, no card needed

**ID:** `eu-307-notion-ai-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Notion AI Access — a Notion AI AI access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9806
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/307-notion-ai-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 307-notion-ai-eu.service /etc/systemd/system/
sudo systemctl enable --now 307-notion-ai-eu
```
