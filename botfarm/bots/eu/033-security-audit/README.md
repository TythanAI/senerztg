...
> Pen-test report for your web app

**ID:** `eu-033-security-audit` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** elite

Security Audit — a security auditing bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/033-security-audit/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 033-security-audit.service /etc/systemd/system/
sudo systemctl enable --now 033-security-audit
```
