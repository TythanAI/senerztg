...
**ID:** `eu-023-api-monitor` · **Регион:** EU · **Язык:** en ·
**Модель:** saas · **Тариф цен:** standard

API Uptime Monitor — a API monitoring bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9522
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/023-api-monitor/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 023-api-monitor.service /etc/systemd/system/
sudo systemctl enable --now 023-api-monitor
```
