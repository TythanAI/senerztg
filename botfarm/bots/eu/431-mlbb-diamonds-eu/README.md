...
> Mobile Legends top-ups at a better rate, credited in 15 minutes

**ID:** `eu-431-mlbb-diamonds-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** topup · **Тариф цен:** budget

Mobile Legends Top-Up — a Mobile Legends in-game currency bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9930
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/431-mlbb-diamonds-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 431-mlbb-diamonds-eu.service /etc/systemd/system/
sudo systemctl enable --now 431-mlbb-diamonds-eu
```
