# Telegram Premium Keys

> Telegram Premium: keys and codes delivered instantly

**ID:** `eu-420-telegram-premium-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** keys · **Тариф цен:** standard

Telegram Premium Keys — a Telegram Premium keys and gift cards bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9919

## Каталог

| SKU | Название | Цена | Тип |
|-----|----------|------|-----|
| `key1` | 1 key | 28 € | account |
| `key3` | 3 keys | 77 € | account |
| `key10` | 10 keys | 231 € | account |
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/420-telegram-premium-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 420-telegram-premium-eu.service /etc/systemd/system/
sudo systemctl enable --now 420-telegram-premium-eu
```
