...
> A new technique every week

**ID:** `eu-083-cooking-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** club · **Тариф цен:** budget

Cooking Club — a cooking bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/083-cooking-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 083-cooking-eu.service /etc/systemd/system/
sudo systemctl enable --now 083-cooking-eu
```
