...
> Replit access in under a minute, no card needed

**ID:** `eu-309-replit-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Replit Access — a Replit AI access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9808
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/309-replit-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 309-replit-eu.service /etc/systemd/system/
sudo systemctl enable --now 309-replit-eu
```
