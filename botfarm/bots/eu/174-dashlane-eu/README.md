...
> Dashlane licence keys, delivered instantly

**ID:** `eu-174-dashlane-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** keys · **Тариф цен:** budget

Dashlane Keys — a Dashlane password manager access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9673
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/174-dashlane-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 174-dashlane-eu.service /etc/systemd/system/
sudo systemctl enable --now 174-dashlane-eu
```
