...
**ID:** `eu-039-yoga-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** club · **Тариф цен:** budget

Daily Yoga — a yoga bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9538
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/039-yoga-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 039-yoga-eu.service /etc/systemd/system/
sudo systemctl enable --now 039-yoga-eu
```
