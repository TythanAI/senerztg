...
> EA Play: keys and codes delivered instantly

**ID:** `eu-423-ea-play-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** keys · **Тариф цен:** standard

EA Play Keys — a EA Play keys and gift cards bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9922
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/423-ea-play-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 423-ea-play-eu.service /etc/systemd/system/
sudo systemctl enable --now 423-ea-play-eu
```
