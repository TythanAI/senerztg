...
> HTTP Proxies: fast delivery, replaced if anything fails

**ID:** `eu-374-http-proxy-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

HTTP Proxies Access — a HTTP Proxies privacy access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9873
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/374-http-proxy-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 374-http-proxy-eu.service /etc/systemd/system/
sudo systemctl enable --now 374-http-proxy-eu
```
