...
> GOG accounts — instant delivery, warranty included

**ID:** `eu-235-gog-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

GOG Accounts — a GOG game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9734
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/235-gog-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 235-gog-eu.service /etc/systemd/system/
sudo systemctl enable --now 235-gog-eu
```
