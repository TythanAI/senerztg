...
> Synthesia access in under a minute, no card needed

**ID:** `eu-302-synthesia-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Synthesia Access — a Synthesia AI access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9801
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/302-synthesia-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 302-synthesia-eu.service /etc/systemd/system/
sudo systemctl enable --now 302-synthesia-eu
```
