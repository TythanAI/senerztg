...
> Brilliant course access at a fraction of the price

**ID:** `eu-187-brilliant-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Brilliant Access — a Brilliant course access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9686
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/187-brilliant-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 187-brilliant-eu.service /etc/systemd/system/
sudo systemctl enable --now 187-brilliant-eu
```
