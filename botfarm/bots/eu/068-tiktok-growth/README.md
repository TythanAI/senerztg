...
> Reels and TikTok that compound

**ID:** `eu-068-tiktok-growth` · **Регион:** EU · **Язык:** en ·
**Модель:** course · **Тариф цен:** standard

Short-Form Growth — a short-form video bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9567
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/068-tiktok-growth/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 068-tiktok-growth.service /etc/systemd/system/
sudo systemctl enable --now 068-tiktok-growth
```
