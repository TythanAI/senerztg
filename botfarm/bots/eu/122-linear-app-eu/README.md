...
> Linear access without a company card

**ID:** `eu-122-linear-app-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Linear Access — a Linear issue tracker access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9621
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/122-linear-app-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 122-linear-app-eu.service /etc/systemd/system/
sudo systemctl enable --now 122-linear-app-eu
```
