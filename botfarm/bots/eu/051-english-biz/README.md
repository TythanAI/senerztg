...
**ID:** `eu-051-english-biz` · **Регион:** EU · **Язык:** en ·
**Модель:** club · **Тариф цен:** standard

Business English — a business English bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9250
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/051-english-biz/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 051-english-biz.service /etc/systemd/system/
sudo systemctl enable --now 051-english-biz
```
