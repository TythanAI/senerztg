...
> Seeking Alpha: research and data access on demand

**ID:** `eu-162-seeking-alpha-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** premium

Seeking Alpha Access — a Seeking Alpha research access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9661
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/162-seeking-alpha-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 162-seeking-alpha-eu.service /etc/systemd/system/
sudo systemctl enable --now 162-seeking-alpha-eu
```
