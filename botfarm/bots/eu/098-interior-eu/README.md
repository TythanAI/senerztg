...
**ID:** `eu-098-interior-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** elite

Interior Design — a interior design bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9597
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/098-interior-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 098-interior-eu.service /etc/systemd/system/
sudo systemctl enable --now 098-interior-eu
```
