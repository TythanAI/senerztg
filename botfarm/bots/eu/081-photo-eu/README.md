...
> Manual mode to paid shoots

**ID:** `eu-081-photo-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** course · **Тариф цен:** standard

Photography Craft — a photography bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9580
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/081-photo-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 081-photo-eu.service /etc/systemd/system/
sudo systemctl enable --now 081-photo-eu
```
