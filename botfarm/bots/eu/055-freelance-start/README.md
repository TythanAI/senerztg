...
> First five clients in 90 days

**ID:** `eu-055-freelance-start` · **Регион:** EU · **Язык:** en ·
**Модель:** course · **Тариф цен:** standard

Freelance Launch — a freelancing bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/055-freelance-start/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 055-freelance-start.service /etc/systemd/system/
sudo systemctl enable --now 055-freelance-start
```
