...
> Battle.net accounts — instant delivery, warranty included

**ID:** `eu-232-battlenet-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Battle.net Accounts — a Battle.net game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9731
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/232-battlenet-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 232-battlenet-eu.service /etc/systemd/system/
sudo systemctl enable --now 232-battlenet-eu
```
