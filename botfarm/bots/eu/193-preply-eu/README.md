...
> Preply course access at a fraction of the price

**ID:** `eu-193-preply-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Preply Access — a Preply tutoring credits bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9692
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/193-preply-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 193-preply-eu.service /etc/systemd/system/
sudo systemctl enable --now 193-preply-eu
```
