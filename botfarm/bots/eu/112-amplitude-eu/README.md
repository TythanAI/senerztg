...
> Amplitude access without a company card

**ID:** `eu-112-amplitude-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Amplitude Access — a Amplitude product analytics access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9611
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/112-amplitude-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 112-amplitude-eu.service /etc/systemd/system/
sudo systemctl enable --now 112-amplitude-eu
```
