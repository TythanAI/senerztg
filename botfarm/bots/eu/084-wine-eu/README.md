...
> Tastings and bottles worth buying

**ID:** `eu-084-wine-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** club · **Тариф цен:** standard

Wine Discovery — a wine bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9583
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/084-wine-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 084-wine-eu.service /etc/systemd/system/
sudo systemctl enable --now 084-wine-eu
```
