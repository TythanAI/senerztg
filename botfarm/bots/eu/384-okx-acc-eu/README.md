...
> OKX: verified accounts, ready to trade

**ID:** `eu-384-okx-acc-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** premium

OKX Accounts — a OKX exchange accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9883
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/384-okx-acc-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 384-okx-acc-eu.service /etc/systemd/system/
sudo systemctl enable --now 384-okx-acc-eu
```
