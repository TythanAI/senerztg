...
> Gmail: clean accounts for work and sign-ups

**ID:** `eu-336-gmail-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Gmail Accounts — a Gmail email and cloud accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9835
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/336-gmail-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 336-gmail-eu.service /etc/systemd/system/
sudo systemctl enable --now 336-gmail-eu
```
