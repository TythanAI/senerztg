...
> PayPal: ready-to-use accounts for sellers

**ID:** `eu-402-paypal-acc-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

PayPal Accounts — a PayPal marketplace accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9901
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/402-paypal-acc-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 402-paypal-acc-eu.service /etc/systemd/system/
sudo systemctl enable --now 402-paypal-acc-eu
```
