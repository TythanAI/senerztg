...
> Epic Games accounts — instant delivery, warranty included

**ID:** `eu-231-epic-games-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Epic Games Accounts — a Epic Games game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9730
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/231-epic-games-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 231-epic-games-eu.service /etc/systemd/system/
sudo systemctl enable --now 231-epic-games-eu
```
