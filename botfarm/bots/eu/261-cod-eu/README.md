...
> Call of Duty accounts — instant delivery, warranty included

**ID:** `eu-261-cod-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Call of Duty Accounts — a Call of Duty game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9760
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/261-cod-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 261-cod-eu.service /etc/systemd/system/
sudo systemctl enable --now 261-cod-eu
```
