...
> Codecademy course access at a fraction of the price

**ID:** `eu-183-codecademy-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Codecademy Access — a Codecademy course access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9682
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/183-codecademy-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 183-codecademy-eu.service /etc/systemd/system/
sudo systemctl enable --now 183-codecademy-eu
```
