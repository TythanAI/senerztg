...
> ChatGPT Plus access in under a minute, no card needed

**ID:** `eu-288-chatgpt-plus-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

ChatGPT Plus Access — a ChatGPT Plus AI access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9787
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/288-chatgpt-plus-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 288-chatgpt-plus-eu.service /etc/systemd/system/
sudo systemctl enable --now 288-chatgpt-plus-eu
```
