...
> DigitalOcean: ready accounts and credits

**ID:** `eu-129-digitalocean-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

DigitalOcean Accounts — a DigitalOcean VPS accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9628
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/129-digitalocean-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 129-digitalocean-eu.service /etc/systemd/system/
sudo systemctl enable --now 129-digitalocean-eu
```
