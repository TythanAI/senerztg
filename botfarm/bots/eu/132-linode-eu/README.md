# Akamai Linode Accounts

> Akamai Linode: ready accounts and credits

**ID:** `eu-132-linode-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Akamai Linode Accounts — a Akamai Linode VPS accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9631
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/132-linode-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 132-linode-eu.service /etc/systemd/system/
sudo systemctl enable --now 132-linode-eu
```
