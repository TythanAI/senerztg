...
> Mailchimp access without a company card

**ID:** `eu-103-mailchimp-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Mailchimp Access — a Mailchimp email marketing access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9602
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/103-mailchimp-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 103-mailchimp-eu.service /etc/systemd/system/
sudo systemctl enable --now 103-mailchimp-eu
```
