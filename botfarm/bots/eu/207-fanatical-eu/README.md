...
> Fanatical: ready accounts, instant delivery

**ID:** `eu-207-fanatical-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Fanatical Accounts — a Fanatical store accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9706
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/207-fanatical-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 207-fanatical-eu.service /etc/systemd/system/
sudo systemctl enable --now 207-fanatical-eu
```
