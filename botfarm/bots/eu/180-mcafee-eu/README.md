...
> McAfee licence keys, delivered instantly

**ID:** `eu-180-mcafee-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** keys · **Тариф цен:** budget

McAfee Keys — a McAfee security licences bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9679
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/180-mcafee-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 180-mcafee-eu.service /etc/systemd/system/
sudo systemctl enable --now 180-mcafee-eu
```
