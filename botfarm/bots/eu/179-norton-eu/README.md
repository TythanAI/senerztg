...
> Norton licence keys, delivered instantly

**ID:** `eu-179-norton-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** keys · **Тариф цен:** budget

Norton Keys — a Norton security licences bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9678
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/179-norton-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 179-norton-eu.service /etc/systemd/system/
sudo systemctl enable --now 179-norton-eu
```
