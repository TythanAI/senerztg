...
> YouTube Premium for less than the official plan, delivered instantly

**ID:** `eu-273-youtube-premium-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

YouTube Premium Subs — a YouTube Premium subscriptions bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9772
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/273-youtube-premium-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 273-youtube-premium-eu.service /etc/systemd/system/
sudo systemctl enable --now 273-youtube-premium-eu
```
