...
> Tidal for less than the official plan, delivered instantly

**ID:** `eu-281-tidal-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Tidal Subs — a Tidal subscriptions bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9780
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/281-tidal-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 281-tidal-eu.service /etc/systemd/system/
sudo systemctl enable --now 281-tidal-eu
```
