...
> Gamivo: ready accounts, instant delivery

**ID:** `eu-204-gamivo-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Gamivo Accounts — a Gamivo game key marketplace accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9703
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/204-gamivo-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 204-gamivo-eu.service /etc/systemd/system/
sudo systemctl enable --now 204-gamivo-eu
```
