...
> Blinkist course access at a fraction of the price

**ID:** `eu-198-blinkist-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Blinkist Access — a Blinkist summaries access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9697
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/198-blinkist-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 198-blinkist-eu.service /etc/systemd/system/
sudo systemctl enable --now 198-blinkist-eu
```
