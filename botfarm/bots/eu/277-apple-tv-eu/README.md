...
> Apple TV+ for less than the official plan, delivered instantly

**ID:** `eu-277-apple-tv-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Apple TV+ Subs — a Apple TV+ subscriptions bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9776
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/277-apple-tv-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 277-apple-tv-eu.service /etc/systemd/system/
sudo systemctl enable --now 277-apple-tv-eu
```
