...
> Viber: aged accounts with a replacement warranty

**ID:** `eu-332-viber-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Viber Accounts — a Viber social accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9831
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/332-viber-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 332-viber-eu.service /etc/systemd/system/
sudo systemctl enable --now 332-viber-eu
```
