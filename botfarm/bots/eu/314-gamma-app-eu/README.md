...
> Gamma access in under a minute, no card needed

**ID:** `eu-314-gamma-app-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Gamma Access — a Gamma AI access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9813
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/314-gamma-app-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 314-gamma-app-eu.service /etc/systemd/system/
sudo systemctl enable --now 314-gamma-app-eu
```
