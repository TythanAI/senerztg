...
> Templates that run your work

**ID:** `eu-058-notion-systems` · **Регион:** EU · **Язык:** en ·
**Модель:** shop · **Тариф цен:** budget

Notion Systems — a Notion templates bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9257
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/058-notion-systems/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 058-notion-systems.service /etc/systemd/system/
sudo systemctl enable --now 058-notion-systems
```
