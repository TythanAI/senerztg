...
> Claude Pro access in under a minute, no card needed

**ID:** `eu-289-claude-pro-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Claude Pro Access — a Claude Pro AI access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9788
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/289-claude-pro-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 289-claude-pro-eu.service /etc/systemd/system/
sudo systemctl enable --now 289-claude-pro-eu
```
