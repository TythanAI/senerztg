...
> World of Tanks top-ups at a better rate, credited in 15 minutes

**ID:** `eu-436-wot-gold-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** topup · **Тариф цен:** budget

World of Tanks Top-Up — a World of Tanks in-game currency bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9935
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/436-wot-gold-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 436-wot-gold-eu.service /etc/systemd/system/
sudo systemctl enable --now 436-wot-gold-eu
```
