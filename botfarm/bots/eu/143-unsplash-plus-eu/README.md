...
> Unsplash+ for a fraction of the licence price

**ID:** `eu-143-unsplash-plus-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Unsplash+ Access — a Unsplash+ stock media access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9642
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/143-unsplash-plus-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 143-unsplash-plus-eu.service /etc/systemd/system/
sudo systemctl enable --now 143-unsplash-plus-eu
```
