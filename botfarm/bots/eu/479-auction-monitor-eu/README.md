# Auction Alerts Monitor

> Auction Alerts: real-time alerts straight into this chat

**ID:** `eu-479-auction-monitor-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** monitor · **Тариф цен:** standard

Auction Alerts Monitor — a Auction Alerts alerts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9978
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/479-auction-monitor-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 479-auction-monitor-eu.service /etc/systemd/system/
sudo systemctl enable --now 479-auction-monitor-eu
```
