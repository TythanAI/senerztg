...
> GamersClub: ready accounts, instant delivery

**ID:** `eu-215-gamersclub-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

GamersClub Accounts — a GamersClub competitive accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9714
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/215-gamersclub-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 215-gamersclub-eu.service /etc/systemd/system/
sudo systemctl enable --now 215-gamersclub-eu
```
