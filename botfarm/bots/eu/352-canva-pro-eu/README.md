...
> Canva Pro at a fraction of the subscription price

**ID:** `eu-352-canva-pro-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Canva Pro Access — a Canva Pro software access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9851
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/352-canva-pro-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 352-canva-pro-eu.service /etc/systemd/system/
sudo systemctl enable --now 352-canva-pro-eu
```
