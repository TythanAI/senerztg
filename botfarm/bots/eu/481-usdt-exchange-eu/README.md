...
> USDT Exchange: fast, fair rate, no runaround

**ID:** `eu-481-usdt-exchange-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** premium

USDT Exchange — a USDT Exchange crypto services bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9980
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/481-usdt-exchange-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 481-usdt-exchange-eu.service /etc/systemd/system/
sudo systemctl enable --now 481-usdt-exchange-eu
```
