...
> italki course access at a fraction of the price

**ID:** `eu-192-italki-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

italki Access — a italki tutoring credits bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9691
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/192-italki-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 192-italki-eu.service /etc/systemd/system/
sudo systemctl enable --now 192-italki-eu
```
