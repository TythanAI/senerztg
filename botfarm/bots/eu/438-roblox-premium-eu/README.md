# Roblox Premium Top-Up

> Roblox Premium top-ups at a better rate, credited in 15 minutes

**ID:** `eu-438-roblox-premium-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** topup · **Тариф цен:** budget

Roblox Premium Top-Up — a Roblox Premium in-game currency bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9937
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/438-roblox-premium-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 438-roblox-premium-eu.service /etc/systemd/system/
sudo systemctl enable --now 438-roblox-premium-eu
```
