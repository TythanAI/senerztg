...
> Copy.ai access in under a minute, no card needed

**ID:** `eu-305-copy-ai-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Copy.ai Access — a Copy.ai AI access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9804
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/305-copy-ai-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 305-copy-ai-eu.service /etc/systemd/system/
sudo systemctl enable --now 305-copy-ai-eu
```
