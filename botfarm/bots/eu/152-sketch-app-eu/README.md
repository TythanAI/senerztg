...
> Sketch for a fraction of the licence price

**ID:** `eu-152-sketch-app-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Sketch Access — a Sketch design access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9651
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/152-sketch-app-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 152-sketch-app-eu.service /etc/systemd/system/
sudo systemctl enable --now 152-sketch-app-eu
```
