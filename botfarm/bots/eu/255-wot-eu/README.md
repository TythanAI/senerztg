...
> World of Tanks accounts — instant delivery, warranty included

**ID:** `eu-255-wot-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

World of Tanks Accounts — a World of Tanks game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9754
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/255-wot-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 255-wot-eu.service /etc/systemd/system/
sudo systemctl enable --now 255-wot-eu
```
