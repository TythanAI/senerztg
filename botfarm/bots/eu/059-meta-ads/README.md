...
> Profitable campaigns, not vanity metrics

**ID:** `eu-059-meta-ads` · **Регион:** EU · **Язык:** en ·
**Модель:** course · **Тариф цен:** premium

Meta Ads Mastery — a Meta advertising bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9558
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/059-meta-ads/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 059-meta-ads.service /etc/systemd/system/
sudo systemctl enable --now 059-meta-ads
```
