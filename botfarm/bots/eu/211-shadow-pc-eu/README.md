...
> Shadow PC: ready accounts, instant delivery

**ID:** `eu-211-shadow-pc-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Shadow PC Accounts — a Shadow PC cloud PC access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9710
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/211-shadow-pc-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 211-shadow-pc-eu.service /etc/systemd/system/
sudo systemctl enable --now 211-shadow-pc-eu
```
