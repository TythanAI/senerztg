...
> Source, list, and scale across the EU

**ID:** `eu-065-amazon-fba` · **Регион:** EU · **Язык:** en ·
**Модель:** course · **Тариф цен:** premium

Amazon FBA EU — a Amazon FBA bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9564
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/065-amazon-fba/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 065-amazon-fba.service /etc/systemd/system/
sudo systemctl enable --now 065-amazon-fba
```
