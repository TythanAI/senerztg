...
> Live ROI for your rigs

**ID:** `eu-020-mining-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** saas · **Тариф цен:** standard

Mining Profit Tracker — a crypto mining bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/020-mining-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 020-mining-eu.service /etc/systemd/system/
sudo systemctl enable --now 020-mining-eu
```
