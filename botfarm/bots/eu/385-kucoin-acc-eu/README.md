...
> KuCoin: verified accounts, ready to trade

**ID:** `eu-385-kucoin-acc-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** premium

KuCoin Accounts — a KuCoin exchange accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9884
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/385-kucoin-acc-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 385-kucoin-acc-eu.service /etc/systemd/system/
sudo systemctl enable --now 385-kucoin-acc-eu
```
