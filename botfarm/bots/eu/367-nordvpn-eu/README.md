...
> NordVPN: fast delivery, replaced if anything fails

**ID:** `eu-367-nordvpn-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

NordVPN Access — a NordVPN privacy access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9866
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/367-nordvpn-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 367-nordvpn-eu.service /etc/systemd/system/
sudo systemctl enable --now 367-nordvpn-eu
```
