...
> Boosteroid: ready accounts, instant delivery

**ID:** `eu-212-boosteroid-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Boosteroid Accounts — a Boosteroid cloud gaming access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9711
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/212-boosteroid-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 212-boosteroid-eu.service /etc/systemd/system/
sudo systemctl enable --now 212-boosteroid-eu
```
