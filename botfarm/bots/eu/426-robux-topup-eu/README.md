...
> Robux top-ups at a better rate, credited in 15 minutes

**ID:** `eu-426-robux-topup-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** topup · **Тариф цен:** budget

Robux Top-Up — a Robux in-game currency bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9925
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/426-robux-topup-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 426-robux-topup-eu.service /etc/systemd/system/
sudo systemctl enable --now 426-robux-topup-eu
```
