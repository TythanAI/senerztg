...
> Runway access in under a minute, no card needed

**ID:** `eu-294-runway-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Runway Access — a Runway AI access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9793
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/294-runway-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 294-runway-eu.service /etc/systemd/system/
sudo systemctl enable --now 294-runway-eu
```
