...
> QuickBooks: research and data access on demand

**ID:** `eu-156-quickbooks-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** premium

QuickBooks Access — a QuickBooks accounting access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9655
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/156-quickbooks-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 156-quickbooks-eu.service /etc/systemd/system/
sudo systemctl enable --now 156-quickbooks-eu
```
