# Epidemic Sound Access

> Epidemic Sound for a fraction of the licence price

**ID:** `eu-145-epidemic-sound-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Epidemic Sound Access — a Epidemic Sound music licence access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9644
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/145-epidemic-sound-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 145-epidemic-sound-eu.service /etc/systemd/system/
sudo systemctl enable --now 145-epidemic-sound-eu
```
