...
> DAZN for less than the official plan, delivered instantly

**ID:** `eu-286-dazn-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

DAZN Subs — a DAZN subscriptions bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9785
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/286-dazn-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 286-dazn-eu.service /etc/systemd/system/
sudo systemctl enable --now 286-dazn-eu
```
