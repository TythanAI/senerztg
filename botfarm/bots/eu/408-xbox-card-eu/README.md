...
> Xbox Gift Card: keys and codes delivered instantly

**ID:** `eu-408-xbox-card-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** keys · **Тариф цен:** standard

Xbox Gift Card Keys — a Xbox Gift Card keys and gift cards bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9907
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/408-xbox-card-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 408-xbox-card-eu.service /etc/systemd/system/
sudo systemctl enable --now 408-xbox-card-eu
```
