...
> Dota 2 MMR Boosting: done for you, agreed before you pay

**ID:** `eu-446-dota-mmr-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** standard

Dota 2 MMR Boosting — a Dota 2 MMR Boosting boosting services bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9945
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/446-dota-mmr-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 446-dota-mmr-eu.service /etc/systemd/system/
sudo systemctl enable --now 446-dota-mmr-eu
```
