...
> New York Times: research and data access on demand

**ID:** `eu-169-nyt-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** premium

New York Times Access — a New York Times news subscriptions bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9668
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/169-nyt-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 169-nyt-eu.service /etc/systemd/system/
sudo systemctl enable --now 169-nyt-eu
```
