...
> Bitwarden licence keys, delivered instantly

**ID:** `eu-172-bitwarden-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** keys · **Тариф цен:** budget

Bitwarden Keys — a Bitwarden password manager access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9671
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/172-bitwarden-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 172-bitwarden-eu.service /etc/systemd/system/
sudo systemctl enable --now 172-bitwarden-eu
```
