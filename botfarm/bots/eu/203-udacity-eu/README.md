...
> Udacity course access at a fraction of the price

**ID:** `eu-203-udacity-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Udacity Access — a Udacity nanodegree access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9702
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/203-udacity-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 203-udacity-eu.service /etc/systemd/system/
sudo systemctl enable --now 203-udacity-eu
```
