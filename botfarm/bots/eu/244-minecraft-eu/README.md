...
> Minecraft accounts — instant delivery, warranty included

**ID:** `eu-244-minecraft-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Minecraft Accounts — a Minecraft game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9743
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/244-minecraft-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 244-minecraft-eu.service /etc/systemd/system/
sudo systemctl enable --now 244-minecraft-eu
```
