...
> Genshin Farming: done for you, agreed before you pay

**ID:** `eu-452-genshin-farm-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** service · **Тариф цен:** standard

Genshin Farming — a Genshin Farming boosting services bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9951
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/452-genshin-farm-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 452-genshin-farm-eu.service /etc/systemd/system/
sudo systemctl enable --now 452-genshin-farm-eu
```
