...
> HBO Max for less than the official plan, delivered instantly

**ID:** `eu-275-hbo-max-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

HBO Max Subs — a HBO Max subscriptions bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9774
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/275-hbo-max-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 275-hbo-max-eu.service /etc/systemd/system/
sudo systemctl enable --now 275-hbo-max-eu
```
