...
**ID:** `eu-072-print-demand` · **Регион:** EU · **Язык:** en ·
**Модель:** course · **Тариф цен:** standard

Print on Demand — a print on demand bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/072-print-demand/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 072-print-demand.service /etc/systemd/system/
sudo systemctl enable --now 072-print-demand
```
