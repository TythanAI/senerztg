...
> Jira access without a company card

**ID:** `eu-123-jira-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Jira Access — a Jira issue tracker access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9622
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/123-jira-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 123-jira-eu.service /etc/systemd/system/
sudo systemctl enable --now 123-jira-eu
```
