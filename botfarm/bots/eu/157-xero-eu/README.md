...
> Xero: research and data access on demand

**ID:** `eu-157-xero-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** premium

Xero Access — a Xero accounting access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9656
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/157-xero-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 157-xero-eu.service /etc/systemd/system/
sudo systemctl enable --now 157-xero-eu
```
