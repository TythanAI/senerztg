...
> Escape from Tarkov accounts — instant delivery, warranty included

**ID:** `eu-267-tarkov-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** budget

Escape from Tarkov Accounts — a Escape from Tarkov game accounts bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9766
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/267-tarkov-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 267-tarkov-eu.service /etc/systemd/system/
sudo systemctl enable --now 267-tarkov-eu
```
