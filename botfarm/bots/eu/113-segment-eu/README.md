...
> Segment access without a company card

**ID:** `eu-113-segment-eu` · **Регион:** EU · **Язык:** en ·
**Модель:** accounts · **Тариф цен:** standard

Segment Access — a Segment data pipeline access bot. Tiered catalog, card and crypto checkout, automatic delivery, an affiliate programme and an admin panel with analytics.
...
- Веб-эндпоинты `/healthz` и `/metrics` на порту 9612
...
```bash
cd ../../..                   # botfarm/
docker compose -f bots/eu/113-segment-eu/docker-compose.yml up -d
```

Как systemd-сервис на VPS:

```bash
sudo cp 113-segment-eu.service /etc/systemd/system/
sudo systemctl enable --now 113-segment-eu
```
